...
# Each run uses its own SUPERCLI_HOME so rows can run in parallel.
set -u

CONC="$1"
LABEL="$2"

ROOT="$HOME/workspace/muse-harness/supercli"
CRATES="$ROOT/crates"
SUPERCLI="$CRATES/target/release/supercli"
LOAD="$CRATES/target/release/supercli-load"
...
# Phase 14 (0c): Tokens via env, NOT argv. Passing bearer tokens on the
# command line exposes them in `ps` output. supercli-load reads
# SUPERCLI_MCP_TOKEN and SUPERCLI_MOBILE_TOKEN from the environment.
SUPERCLI_MCP_TOKEN="$MCP_TOKEN" SUPERCLI_MOBILE_TOKEN="$MOBILE_TOKEN" \
"$LOAD" \
    --hook-port "$HOOK_PORT" \
