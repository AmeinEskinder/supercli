...
#
# `supercli-core` must build without the Host: `--no-default-features
...
# `crates/supercli-core/src/portable_gating_tests.rs` catches the same class
...
cargo test --manifest-path "$manifest" -p supercli-core \
...
cargo clippy --manifest-path "$manifest" -p supercli-core \
