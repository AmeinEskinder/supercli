...
/// S2: Migrate grants from app-state.json to grants.json (sharding).
/// Idempotent: if grants.json already has the grants, or app-state.json
/// has no grants, it's a no-op. On --apply, moves the grants and removes
/// them from app-state.json.
fn step_grant_shard(apply: bool) -> StepOutcome {
    let mut step = StepOutcome {
        title: "grant shard (app-state.json -> grants.json)",
        lines: Vec::new(),
        changed: false,
        error: None,
    };
    ...
    for key in to_migrate {
        if let Some(value) = state_obj.remove(key) {
            // If grants.json already has this key, merge (grants.json wins on conflict)
            // Actually, per user requirement: migrate moves and deletes from app-state.
            // If both have it, we need to define precedence. Let's merge with
    ...
    if step.changed {
        ...
        let body = serde_json::to_vec_pretty(&grants_value).unwrap();
