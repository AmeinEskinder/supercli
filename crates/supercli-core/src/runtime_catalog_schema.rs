...
pub struct RuntimeDisplay {
    /// What this CLI is for presentation: an agent, an Supercli App, an
...
pub struct RuntimeLifecycle {
    ...
    /// Escape interrupts the foreground turn without a Stop hook (verified
    /// per provider). The Host owns the cancellation fence; this only opts
    /// the runtime into it.
    #[serde(default)]
...
/// What `supercli integrations install` edits for this runtime, in the user's
...
pub struct RuntimeIntegrationInfo {
    ...
    /// Files, relative to the machine home, that only a pre-0.7 Supercli
    ...
    for discovered in &descriptors {
        ...
        if let Some(integration) = &descriptor.integration {
            for evidence in &integration.legacy_evidence {
                ...
                {
                    errors.push(format!(
                        "{prefix}: [integration] legacy_evidence entries must be relative paths inside the Supercli home (got {evidence:?})"
                    ));
                }
            }
        }
...
pub fn generated_catalog_source(descriptors: &[DiscoveredRuntimeDescriptor]) -> String {
    ...
    let mut output = String::from(
        "// @generated by supercli-core/build.rs; do not edit.\n\
...
pub fn generated_integration_source(descriptors: &[DiscoveredRuntimeDescriptor]) -> String {
    ...
    // Each package module is generated from what the package ships: an
    // optional installer (`adapter/setup.rs`, exposing `pub fn install`), an
    // optional resume recipe (`adapter/resume.rs`, exposing `ADAPTER`), and
    // optional package tests (`adapter/tests.rs`). Nothing in a package
    // touches launching; the descriptor carries the only per-runtime flags.
    let mut output = String::from("// @generated by supercli-core/build.rs; do not edit.\n");
    for runtime in &adapters {
        let descriptor = &runtime.descriptor;
        let module = rust_module_identifier(&descriptor.legacy_slug);
        let package = &descriptor.slug;
        let adapter_dir = runtime
            .source_path
            .parent()
            .map(|dir| dir.join("adapter"))
            .unwrap_or_default();
        let has = |file: &str| adapter_dir.join(file).is_file();
        let include = |file: &str| {
            format!(
                "include!(concat!(env!(\"CARGO_MANIFEST_DIR\"), \"/../../runtimes/{package}/adapter/{file}\"));"
            )
        };
        ...
        if descriptor.lifecycle.escape_cancels_turn {
...
"##
    ...
    fn generated_presets_include_adapter_free_runtime_metadata() {
        ...
        // The package module is generated from the descriptor: a fixture with
        // no adapter files on disk still gets its module and an installer-less
        // Integration; a metadata-only runtime gets no module at all.
        assert!(generated.contains("pub(crate) mod adapted {"));
        assert!(generated.contains("Integration::new(None)"));
        assert!(!generated.contains("adapter/mod.rs"));
        assert!(!generated.contains("pub(crate) mod metadata {"));
