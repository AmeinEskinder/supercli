...
if [ "${SUPERCLI_HOOK_TRACE_VERBOSE:-}" = "1" ]; then
  ...
  TRACE_RAW="<redacted; SUPERCLI_HOOK_TRACE_VERBOSE=1 to log>"
fi
[ -z "$EVENT_TYPE" ] && exit 0
...
if [ -n "$SUPERCLI_SESSION_ID" ]; then
  post_to_unpeel() {
    ...
    post_hook_payload "$HOOK_PAYLOAD" "$SUPERCLI_SESSION_ID" "$SUPERCLI_APP_PORT" || true
    post_hook_payload_to_current_ports "$HOOK_PAYLOAD" "$SUPERCLI_SESSION_ID" "$SUPERCLI_APP_PORT" || true
  }
