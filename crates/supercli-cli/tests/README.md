...
End-to-end coverage for `supercli`: each case drives the **real binaries**
(`supercli serve`, the one-shot verbs, `supercli-host`) against a **private
`SUPERCLI_HOME`**, and asserts on what lands on disk, what the Host publishes
...
SUPERCLI_TUI_SKIP_BUILD=1 ./run.sh   # skip the cargo build
```

`cargo test -p supercli-cli` runs the unit tests and the in-process
...
(The `SUPERCLI_TUI_*` variable names predate the removal of the terminal UI
and are kept so existing scripts and CI keep working: `SUPERCLI_TUI_BINARY`,
`SUPERCLI_TUI_SKIP_BUILD`, `SUPERCLI_TUI_TEST_BASE`, `SUPERCLI_TUI_TEST_HOME`,
`SUPERCLI_TUI_PTY_TESTS`.)

## Why it is built this way

**Fixtures are per-case and built from scratch.** Cases used to share one
`SUPERCLI_HOME` and mutate each other's state, which produced both false passes
...
| | |
|---|---|
| `case.serve(env=…)` | a foreground scoped `supercli serve` (`ready`, `wait_for`, `read_for`, `status`, `log`, `close`) |
...
| `McpClient(home, session_id)` | a `supercli-host __mcp__` child speaking JSON-RPC for one caller Session (`tool_names`, `call`, `McpClient.text`) |
...
|---|---|
| `standalone` | `supercli serve` is a complete persistent Host with no app: verbs, restart, lease |
| `mobile` | bootstrap, output, organization, polite-guest port rules for a paired phone |
| `pairing` | QR + pairing against the **shipped Swift crypto**, `supercli pair` through the worker |
...
| `serve_install` | `supercli serve install|uninstall|status` unit rendering (shim-tested) |
...
| `compat_serve` | **version skew**: the shipped 0.4.3 `supercli` beside this tree's binaries over one home, both directions |
...
archive: it is cached under `~/Library/Caches/supercli-matrix/<version>/`
(sha256 pinned in the case), `SUPERCLI_MATRIX_COMPAT_ARCHIVE=<path>` overrides
