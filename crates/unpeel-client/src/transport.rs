...
pub enum AnswerUiState {
    ...
    Failed,
    /// Phase 14 (0a) follow-up: the Host reported the approval was already
    /// resolved (idempotent retry). Carries the original decision so the UI
    /// can show "Already approved" / "Already denied" instead of a generic
    /// success. Terminal state; the card is removed on next bootstrap.
    AlreadyResolved { approved: bool },
    /// Phase 14 follow-up 3: the Host returned 409 "approval no longer
    /// pending" after a restart — the phone cannot know the decision.
    /// Renders as "Resolved — see activity log", never as approved/denied
    /// and never as a retryable failure. Terminal state.
    ResolvedUnknown,
}
...
impl HostClient {
    ...
    pub fn answer_approval_with_progress(
        ...
        let auth = format!("Bearer {}", self.token);
        // Phase 14 (0a): Generate a unique nonce per answer attempt for
        // idempotency. If the response is lost and the client retries,
        // the server returns already_resolved with the original decision.
        let nonce = format!(
            "{}-{}",
