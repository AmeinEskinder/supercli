...
private enum SessionSheetAction: String, Identifiable {
    ...
    func confirmationMessage(for session: RemoteSessionSummary) -> String {
        ...
        case .remove:
            if session.status == .running {
                return "This stops the running process and removes the session from Supercli. It does not delete the agent’s conversation."
            }
            return "This only removes the session from Supercli. It does not delete the agent’s conversation."
        }
    }
}
