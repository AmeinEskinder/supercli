...
//
//  Two modes:
//  - paired: the production path. The QR in Supercli's Settings encodes a
...
//    token is honored by the Rust `supercli-host __remote__` server, so
...
enum RemoteKeychain {
    private static let service = "com.supercli.ios.remote"
    ...
    private static func saveData(_ data: Data, account: String) -> Bool {
        ...
        if status != errSecSuccess {
            NSLog("[SupercliIOS] keychain save failed: \(status)")
            return false
        }
        return true
    }
    ...
    private static func loadData(account: String) -> KeychainReadResult<Data> {
        ...
        case errSecSuccess:
            guard let data = result as? Data else {
                NSLog("[SupercliIOS] keychain read returned unexpected data")
        ...
        default:
            ...
            NSLog("[SupercliIOS] keychain read temporarily unavailable: \(status)")
    ...
    static func saveToken(_ token: String, macID: String) -> Bool {
        guard !token.isEmpty else {
            NSLog("[SupercliIOS] refusing empty Mac bearer token")
            return false
        }
    ...
    /// Supercli Remote credentials (relay token + E2E key) — same protection
    ...
    static func persistRelayCredentials(
        ...
        guard isValid(credentials, expectedMacID: expectedMacID) else {
            NSLog("[SupercliIOS] refusing invalid relay credentials")
            return false
        }
        guard let data = try? JSONEncoder().encode(credentials) else {
            NSLog("[SupercliIOS] relay credential encoding failed")
            return false
        }
        guard writer(data) else {
            NSLog("[SupercliIOS] relay credential persistence failed")
            return false
        }
        return true
    }
...
enum RelayCredentialRefreshMarker {
    ...
    private static let keyPrefix = "supercli.ios.relayCredentialVersion."
...
enum RemoteDirectRestore {
    ...
    static func attempt(
        ...
        guard snapshot.macID == generation.macID else {
            NSLog("[SupercliIOS] discarded Direct restore response from unexpected Host")
            return false
        }
        guard isStillCurrent(generation) else {
            NSLog("[SupercliIOS] discarded Direct restore for superseded Relay client")
            return false
        }
        adopt()
        return true
...
enum RelayCredentialRepair {
    ...
    private static func evaluating(
        ...
        case .failure(let error as RemoteMacClientError) where error.statusCode == 404:
            NSLog("[SupercliIOS] selected Host does not support relay credential recovery")
            return .recoveryUnavailable
        case .failure(let error):
            NSLog("[SupercliIOS] relay credential refresh failed: \(error.localizedDescription)")
            return .fetchFailed
        case .success(let credentials):
            guard RemoteKeychain.isValid(credentials, expectedMacID: expectedMacID) else {
                NSLog("[SupercliIOS] relay credential refresh returned invalid credentials")
            ...
            guard persist(credentials) else {
                ...
                NSLog("[SupercliIOS] refreshed relay credentials could not be persisted")
    ...
    static func attemptIfBoundToActiveDirectClient(
        ...
        guard let generation = RemoteDirectClientGeneration.capture(
            ...
            NSLog("[SupercliIOS] skipped relay credential refresh for stale Direct client")
            return nil
        }
        ...
        guard isStillCurrent(generation) else {
            ...
            NSLog("[SupercliIOS] discarded relay credential response for superseded Direct client")
            return nil
        }
...
final class RemoteConnectionStore: ObservableObject {
    ...
    nonisolated private static let recordsKey = "supercli.ios.pairedMacs"
    nonisolated private static let activeMacIDKey = "supercli.ios.activeMacID"
    nonisolated private static let legacyRecordKey = "supercli.ios.pairedMac"
    nonisolated private static let deviceIDKey = "supercli.ios.deviceID"

    /// True while `client` tunnels through Supercli Remote instead of the LAN.
    ...
    private func hydrateStoredPairings(
        ...
        case .available, .missing, .invalid:
            ...
            if case .invalid = credentialState {
                NSLog("[SupercliIOS] stored relay credentials are invalid; Direct will repair them")
            }
        }
        keychainHydrationPending = remainsPending
    }
    ...
    /// `~/.supercli/dev-bridge-token` on the Mac, and the Simulator shares the
    ...
    private static func devBridgeToken() -> String? {
        ...
        let path = hostHome + "/.supercli/dev-bridge-token"
    ...
    func completePairing(with payload: RemotePairingPayload) async throws {
        ...
        // per `supercli-host __remote__` run).
    ...
    // MARK: - Supercli Remote (relay) fallback
    ...
    func activateRelayFallback() async -> Bool {
        ...
        do {
            ...
            NSLog("[SupercliIOS] relay fallback failed; credentials queued for Direct repair: \(error.localizedDescription)")
            return false
        }
        guard relaySnapshot.macID == expectedMacID else {
            ...
            NSLog("[SupercliIOS] discarded Relay bootstrap from unexpected Host identity")
            return false
        }
    ...
    func adoptDirectTransport(after poll: RemoteConnectionPollProof) -> Bool {
        ...
        NSLog("[SupercliIOS] Direct /mobile transport for \(record.macID): \(updated.directTLSFingerprint == nil ? "plaintext" : "pinned HTTPS")")
    ...
    func upgradeDirectTransportAfterPlaintextRefusal(connectionEpoch: Int) -> Bool {
        ...
        guard let fingerprint = RemoteDirectTransportPolicy.normalizedFingerprint(
            ...
            NSLog("[SupercliIOS] Host refused plaintext /mobile but advertised no certificate to pin; waiting for an authenticated bootstrap")
            return false
        }
        var updated = record
        ...
        Self.saveRecords(pairedMacs)
        mode = .paired(updated)
        NSLog("[SupercliIOS] Host refused plaintext /mobile; switching Direct to pinned HTTPS")
        client = updated.directClient(token: token)
        epoch &+= 1
        return true
    }
    ...
    func ensureRelayCredentials(
        ...
        guard isCurrentDirectGeneration(result.generation) else {
            NSLog("[SupercliIOS] discarded relay credential state for superseded Direct client")
            return nil
        }
    ...
    nonisolated static func migrateLegacyStorageIfNeeded(
        ...
        if let status = unavailableStatuses.first {
            NSLog("[SupercliIOS] legacy pairing migration deferred until protected data is available: \(status)")
            return .temporarilyUnavailable(status)
        }
        ...
        if scopedToken == nil,
           ...
           !saveToken(legacyToken, record.macID) {
            NSLog("[SupercliIOS] legacy pairing migration deferred after token write failure")
            return .retryNeeded
        }
        ...
        if case .available(let scopedRelay) = scopedRelayState,
           RemoteKeychain.isValid(scopedRelay, expectedMacID: record.macID) {
            ...
            if RemoteKeychain.isValid(legacyRelay, expectedMacID: record.macID) {
                ...
                NSLog("[SupercliIOS] dropping invalid legacy relay credentials")
            }
        } else if case .invalid = legacyRelayState {
            NSLog("[SupercliIOS] dropping undecodable legacy relay credentials")
        }
        ...
        if retainLegacyCredentialsForRetry {
            ...
            NSLog("[SupercliIOS] legacy relay credential migration deferred after scoped write failure")
            return .retryNeeded
        }
