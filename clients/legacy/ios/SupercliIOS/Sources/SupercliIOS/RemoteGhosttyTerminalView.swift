        ...
        .onChange(of: selectedPhotoItem) { item in
            ...
            Task { @MainActor in
                ...
                do {
                    ...
                    NSLog("[SupercliIOS] photo load failed: \(error)")
                    raw = nil
                }
                guard let raw else {
...
final class RemoteGhosttyRenderer: ObservableObject {
    ...
    /// `supercli-attach`). Keep this separate from local surface alignment:
    ...
    /// then try the `supercli-host __remote__` WebSocket when the
    ...
    private func streamOutput() async throws {
        // On Supercli Remote the advertised WSS endpoint is a LAN address the
    ...
    /// One WebSocket streaming attempt against `supercli-host __remote__`.
        ...
        while !Task.isCancelled {
            ...
            case .string(let text):
                ...
                if case .error(let detail) = RemoteTerminalWSServerMessage.parse(text: text) {
                    NSLog("[SupercliIOS] ws output stream error: \(detail)")
                }
            case .data(let raw):
    ...
    func attachImage(
        ...
        Task { @MainActor in
            do {
                ...
                clearLastError()
                NSLog("[SupercliIOS] attached image at \(path)")
            } catch {
                NSLog("[SupercliIOS] image upload failed: \(error)")
    ...
    private func autoFitToPhone() async {
        ...
        do {
            ...
            NSLog("[SupercliIOS] auto-fit failed: \(error)")
        }
    }
