...
public struct RemoteMacClient: Sendable {
    ...
    /// Non-nil when this client tunnels through Supercli Remote (the relay)
    ...
    /// timeout: the Mac shells out to `supercli-host __transcript__ markdown`
    /// per request.
    ...
    /// an Supercli Browser screenshot artifact. The Host owns terminal encoding;
    ...
    /// Rotate + fetch this device's Supercli Remote credentials over the LAN
