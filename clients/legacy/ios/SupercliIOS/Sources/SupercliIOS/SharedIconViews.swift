...
struct SharedToolIconView: View {
    let icon: SupercliToolIcon
    var size: CGFloat = 16

    init(appID: String? = nil, providerID: String?, command: String, size: CGFloat = 16) {
        icon = SupercliToolIcon.resolving(appID: appID, providerID: providerID, command: command)
        self.size = size
    }
...
/// Parsed form of one of the app's own bundled icon SVGs (`SupercliChromeIcon` /
/// `SupercliToolIcon` in SupercliShared): paths with solid or linear-gradient
