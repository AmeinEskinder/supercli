...
public final class RemotePreviewStore {
    ...
    nonisolated static func lastSessionScopeKey(
        ...
        "supercli.ios.lastSession.\(macID ?? "unpaired").\(workspaceID ?? "local")"
    }
    ...
    private static let legacyLastSessionKey = "supercli.ios.lastSelectedSession"
    ...
    /// through Supercli Remote (the E2E relay). Automatic Bonjour endpoint
    ...
    /// credentials for phones paired before Supercli Remote (cheap no-op once
    ...
    /// Supercli Remote can be exercised without leaving the LAN. Set with the
    /// Xcode scheme launch arg `-supercli.ios.forceRelay YES`.
    static let forceRelayForDev = UserDefaults.standard.bool(forKey: "supercli.ios.forceRelay")
    ...
    nonisolated static func replacementRuntimeID(
        ...
        if let providerID = session.providerID?
           ...
           !providerID.isEmpty {
            return SupercliRuntimeCatalog.runtime(id: providerID)?.stableID
                ?? providerID.lowercased()
        }
        if let runtime = SupercliRuntimeCatalog.runtime(command: session.command) {
    ...
    init(
        ...
        SupercliAppIconCatalog.update(snapshot.availableApps ?? [])
    ...
    private func publishCreatedSession(
        ...
        if !Self.snapshotContentEqual(snapshot, next) {
            SupercliAppIconCatalog.update(next.availableApps ?? [])
            snapshot = next
    ...
    public func loadFromBridge() async -> RemoteConnectionPollResult {
        ...
        do {
            ...
            // Every bootstrap carries the current `supercli-host __remote__`
            ...
            if !Self.snapshotContentEqual(snapshot, remote) {
                ...
                SupercliAppIconCatalog.update(remote.availableApps ?? [])
                snapshot = remote
            ...
            setConnectionState(
                status: client.isRelay ? "Connected via Supercli Remote" : directStatus,
                error: nil,
                disconnected: false,
                unreachable: false
            )
...
extension RemoteSessionSummary {
    ...
    var presentationProviderID: String? {
        ...
        return SupercliRuntimeCatalog.runtime(id: activeRuntimeID)?.legacySlug
            ?? activeRuntimeID
    }
    ...
    private var launchRuntime: SupercliRuntimeMetadata? {
        if let providerID = providerID?.trimmingCharacters(in: .whitespacesAndNewlines),
           !providerID.isEmpty,
           let runtime = SupercliRuntimeCatalog.runtime(id: providerID) {
            return runtime
        }
        return SupercliRuntimeCatalog.runtime(command: command)
    }

    /// True only for sessions launched as a recognized agent runtime — not
    /// plain shells, custom commands, or Supercli App sessions. Gates the
...
extension RemoteBootstrapSnapshot {
    ...
    static let mock = RemoteBootstrapSnapshot(
        ...
        projects: [
            .init(id: "project-supercli", name: "Supercli", path: "~/Dev/supercli", folderID: "folder-product", mcpBlocked: false, sortOrder: 0),
            .init(id: "project-site", name: "Website", path: "~/Dev/supercli/apps/website", folderID: "folder-product", mcpBlocked: false, sortOrder: 1),
