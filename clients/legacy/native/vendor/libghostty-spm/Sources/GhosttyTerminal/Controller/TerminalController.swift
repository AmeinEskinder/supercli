...
public final class TerminalController {
    ...
    nonisolated(unsafe) var callbackContext: TerminalControllerCallbackContext?
    nonisolated(unsafe) var retainedCallbackContextAddress: UInt?
    ...
    deinit {
        ...
        if let app {
            let retainedCallbackContextAddress = retainedCallbackContextAddress
            TerminalResourceFreeQueue.freeApp(app) {
                releaseRetainedTerminalControllerCallbackContext(
                    address: retainedCallbackContextAddress
                )
            }
        } else {
            releaseRetainedTerminalControllerCallbackContext(
        ...
        if let config { TerminalResourceFreeQueue.freeConfig(config) }
