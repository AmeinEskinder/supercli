...
final class HostServiceIdentityTests: XCTestCase {
    private let own = HostServiceIdentity.Own(
        executable: "/Applications/Supercli.app/Contents/MacOS/supercli-host",
        version: "0.4.0",
        buildId: "1788338230.000000001:4242"
    )

    private func record(
        executable: String? = "/Applications/Supercli.app/Contents/MacOS/supercli-host",
    ...
    func testVersionSkewRestartsInEitherDirection() {
        for version in ["0.3.1", "0.5.0"] {
            guard case .restart = HostServiceIdentity.decide(
                record: record(executable: "/opt/supercli/bin/supercli-host", version: version, buildId: "x"),
    ...
    func testForeignSameVersionServiceIsLeftAlone() {
        XCTAssertEqual(
            HostServiceIdentity.decide(
                record: record(executable: "/usr/local/bin/supercli-host", buildId: "other"),
                own: own, restartedThisLaunch: false
            ),
    ...
    func testImageTestFallsBackToProcessNameWhenThePathIsGone() {
        ...
        XCTAssertTrue(HostServiceIdentity.isSupercliHostImage(path: nil, processName: "supercli-host"))
        XCTAssertTrue(HostServiceIdentity.isSupercliHostImage(path: "", processName: "supercli-host"))
        XCTAssertFalse(HostServiceIdentity.isSupercliHostImage(path: nil, processName: "zsh"))
        XCTAssertFalse(HostServiceIdentity.isSupercliHostImage(path: nil, processName: nil))
        // A resolvable path decides on its own.
        XCTAssertTrue(HostServiceIdentity.isSupercliHostImage(
            path: "/Applications/Supercli.app/Contents/MacOS/supercli-host", processName: nil))
        XCTAssertFalse(HostServiceIdentity.isSupercliHostImage(path: "/bin/zsh", processName: "supercli-host"))
    }
    ...
    func testBuildIDMatchesTheHostStampFormat() throws {
        let file = FileManager.default.temporaryDirectory
            .appendingPathComponent("supercli-build-id-\(UUID().uuidString.prefix(8))")
    ...
    func testStaleWorkerIsRestartedOnceNotInALoop() throws {
        ...
        for _ in 0..<6 { repo.deleteLastPathComponent() } // …/clients/native/SupercliNative/Tests/SupercliNativeTests/X.swift
        let realHost = repo.appendingPathComponent("crates/target/debug/supercli-host")
        try XCTSkipUnless(
            FileManager.default.isExecutableFile(atPath: realHost.path),
            "needs crates/target/debug/supercli-host (cargo build -p supercli-host)"
        )
        ...
        let staleHost = scratch.appendingPathComponent("supercli-host")
        ...
        env["SUPERCLI_HOME"] = home.path
        env = env.filter { !$0.key.hasPrefix("SUPERCLI_TEST_") && !$0.key.hasPrefix("SUPERCLI_SNAPSHOT") }
