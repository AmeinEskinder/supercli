...
final class RemoteHostRuntimeTests: XCTestCase {
    ...
    func testPluginOrderingUsesSelectedHostAndDedicatedCapability() async throws {
        ...
        await waitUntil { runtime.connectionState == .connected(name: "Host") }
        try await runtime.setPluginOrder(["codex", "supercli.app.markdown", "claude"])
        let patches = await backend.workspaceSettingsPatches
        XCTAssertEqual(patches, [RemoteWorkspaceSettingsPatch(pluginOrder: ["codex", "supercli.app.markdown", "claude"])])
        runtime.disconnect()
    }
    ...
    func testPluginActivationUsesSelectedHostAndDedicatedCapability() async throws {
        ...
        await waitUntil { runtime.connectionState == .connected(name: "Host") }
        try await runtime.setPluginActive(id: "supercli.app.markdown", active: false)
        let patches = await backend.workspaceSettingsPatches
        XCTAssertEqual(patches, [RemoteWorkspaceSettingsPatch(
            pluginActivation: .init(id: "supercli.app.markdown", active: false)
        )])
        runtime.disconnect()
    }
    ...
    func testLocalGatewayTransportReachesFactoryAndBootstrapsLikeAnyHost() async {
        ...
        runtime.connectLocalWorkspace(
            home: "/Users/me/.supercli/profiles/writing",
            name: "Writing",
            expectedHostID: nil
        )
        ...
        XCTAssertEqual(home, "/Users/me/.supercli/profiles/writing")
    ...
    func testLocalServiceUsesSemanticHostVerbsButNeverTerminalEffects() async throws {
        ...
        XCTAssertEqual(home, "/Users/me/.supercli")
    ...
    /// A Host removed from the Supercli Link enrollment list
