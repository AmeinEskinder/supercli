...
final class HostedSessionManifestTests: XCTestCase {
    func testLifecycleLockPathAndLeaseMatchRustContract() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("supercli-lifecycle-lock-\(UUID().uuidString)")
        ...
        XCTAssertEqual(
            SupercliStore.sessionLifecycleLockURL(
                supercliDir: root,
        ...
        let first = try XCTUnwrap(SupercliStore.acquireSessionLifecycleLease(
            supercliDir: root,
            sessionID: "session/../one"
        ))
        XCTAssertNil(SupercliStore.acquireSessionLifecycleLease(
            supercliDir: root,
            sessionID: "session/../one"
        ), "a second process/file description must not enter concurrently")
        first.release()
        XCTAssertNotNil(SupercliStore.acquireSessionLifecycleLease(
            supercliDir: root,
            sessionID: "session/../one"
        ))
    }
    ...
    func testReplacementResumeIsStoppedOnlyButReloadCanReplaceLiveTerminal() {
        XCTAssertTrue(SupercliStore.replacementRestartAllowsState(
            "exited", stoppedOnly: true,
            childProcessExists: true, pidIdentity: .matches
        ))
        XCTAssertFalse(SupercliStore.replacementRestartAllowsState(
    ...
    func testDecodesProviderSessionIDFromManifest() throws {
        let json = """
        {
          ...
          "managed_storage_path": "/Users/test/.supercli/runtime-storage/session",
        ...
        XCTAssertEqual(
            ...
            "/Users/test/.supercli/runtime-storage/session"
        )
    ...
    func testDurableResumeEvidenceAcceptsARealProviderTranscript() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("supercli-resume-evidence-\(UUID().uuidString)")
        ...
        XCTAssertTrue(SupercliStore.hasDurableResumeEvidence(
            manifest: manifest, dirPath: directory.path
        ))
        try FileManager.default.removeItem(at: transcript)
        XCTAssertFalse(SupercliStore.hasDurableResumeEvidence(
            manifest: manifest, dirPath: directory.path
    ...
    func testRuntimeLaunchOutputWindowExcludesPreservedScrollback() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("supercli-runtime-output-\(UUID().uuidString)")
        ...
        let window = try XCTUnwrap(SupercliStore.readFileWindow(
    ...
    func testResumeAgentFailureClassifierKeepsInfrastructureFailuresAt500() {
        XCTAssertEqual(SupercliStore.resumeAgentFailureHTTPStatus(
        ...
        XCTAssertEqual(SupercliStore.normalizedResumeAgentFailureMessage(
    ...
    func testQueuedOldHookReceiptPredatesCommittedRuntimeLaunch() {
        ...
        XCTAssertTrue(SupercliStore.hookReceiptPredatesRuntimeLaunch(
            receivedAt: launch.addingTimeInterval(-0.001),
            launchedAt: launch
        ))
        XCTAssertFalse(SupercliStore.hookReceiptPredatesRuntimeLaunch(
            receivedAt: launch,
            launchedAt: launch
        ))
        XCTAssertFalse(SupercliStore.hookReceiptPredatesRuntimeLaunch(
    ...
    func testDeferredStopEffectsRequireUnchangedRuntimeGeneration() {
        XCTAssertTrue(SupercliStore.shouldPublishDeferredStopEffects(
            observedGeneration: 4,
            currentGeneration: 4
        ))
        XCTAssertFalse(SupercliStore.shouldPublishDeferredStopEffects(
            observedGeneration: 4,
            currentGeneration: 5
        ))
    ...
    func testPostCommitDepartingStopCannotOwnReplacementGeneration() {
        ...
        XCTAssertEqual(SupercliStore.hookRuntimeDecision(
            eventGeneration: 1,
            hookEventName: "Stop",
    ...
    func testStaleDurableHookSeedGenerationIsRejected() throws {
        let seed = try XCTUnwrap(LastHookEvent.parse(Data(
            #"{"hook_event_name":"Stop","supercli_runtime_generation":3}"#.utf8
        )))
