...
struct HookRestartTests {
    ...
    @Test func expiryOnlySuppressesOlderOpenersFromTheSameLaunch() throws {
        let start = try #require(LastHookEvent.parse(Data(
            #"{"hook_event_name":"UserPromptSubmit","supercli_runtime_generation":7}"#.utf8
        )))
...
final class SessionActivityTests: XCTestCase {
    ...
    func testSharedReadReceiptRefreshOnlyAdvancesPastNewActivity() {
        XCTAssertTrue(SupercliStore.sharedReadReceiptNeedsRefresh(
            readAt: nil, settledAt: nil
        ))
        XCTAssertFalse(SupercliStore.sharedReadReceiptNeedsRefresh(
        ...
        XCTAssertEqual(
            SupercliStore.latestUnreadActivityAt(lifecycleAt: 90, alertAt: 120),
            120
        )
        XCTAssertEqual(
            SupercliStore.latestUnreadActivityAt(lifecycleAt: 130, alertAt: 120),
            130
        )
        XCTAssertNil(SupercliStore.latestUnreadActivityAt(lifecycleAt: nil, alertAt: nil))
    }
    ...
    func testLastHookEventParsesLifecycleEvents() {
        ...
        let owned = parseLastHookEvent(
            #"{"hook_event_name":"Stop","supercli_runtime_generation":7}"#
        )
