...
final class NotificationDeliveryTests: XCTestCase {
    func testMacObservationSuppressesOnlyLocalEffects() {
        let policy = SupercliStore.notificationDeliveryPolicy(
            macIsObserving: true,
    ...
    func testMenuPromptNotifiesOncePerFalseToTrueEdge() {
        var decision = SupercliStore.menuPromptNotificationDecision(
            previous: nil,
            runtimeGeneration: 1,
            active: false,
        ...
        decision = SupercliStore.menuPromptNotificationDecision(
    ...
    func testInitialActiveMenuSeedsWithoutNotification() {
        let decision = SupercliStore.menuPromptNotificationDecision(
            previous: nil,
            runtimeGeneration: 1,
            active: true,
            initialAppScan: true,
        ...
        let hook = SupercliStore.permissionRequestNotificationDecision(
            previous: decision.state,
            runtimeGeneration: 1
        )
    ...
    func testMenuAndPermissionRequestDeduplicateInEitherOrder() {
        let armed = SupercliStore.menuPromptNotificationDecision(
            previous: nil,
            runtimeGeneration: 4,
            active: false,
            initialAppScan: false,
        ...
        let menuFirst = SupercliStore.menuPromptNotificationDecision(
            previous: armed,
            runtimeGeneration: 4,
            active: true,
            initialAppScan: false,
        ...
        XCTAssertFalse(SupercliStore.permissionRequestNotificationDecision(
            previous: menuFirst.state,
            runtimeGeneration: 4
        ).sendNotification)

        let hookFirstMenu = SupercliStore.menuPromptNotificationDecision(
            previous: armed,
            runtimeGeneration: 4,
            active: true,
            initialAppScan: false,
    ...
    func testPermissionHookBeforeFirstMenuSampleSuppressesBothDuplicates() {
        ...
        let menu = SupercliStore.menuPromptNotificationDecision(
            previous: hook.state,
            runtimeGeneration: 7,
            active: true,
            initialAppScan: false,
    ...
    func testRuntimeGenerationChangeRearmsAnActiveMenu() {
        let previous = SupercliStore.MenuPromptNotificationState(
            runtimeGeneration: 2,
            active: true,
    ...
    func testDisabledMenuDetectionDoesNotSuppressLaterPermissionHook() {
        ...
        XCTAssertTrue(SupercliStore.permissionRequestNotificationDecision(
            previous: menu.state,
            runtimeGeneration: 1
        ).sendNotification)
    }
