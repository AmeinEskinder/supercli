...
//  from the Tauri app's <project>/.supercli.json project presets).
//
//  Data source (READ ONLY):
//  - global presets: ~/.supercli/app-state.json `presets`
//
...
extension Preset {
    ...
    static let builtinGlobalPresets: [Preset] = SupercliRuntimeCatalog
        .runtimes(for: .macos)
        .flatMap { runtime in
...
struct QuickPresetTool: RawRepresentable, CaseIterable, Hashable, Identifiable, Sendable {
    ...
    init?(rawValue: String) {
        guard let runtime = SupercliRuntimeCatalog.runtime(id: rawValue, for: .macos),
              runtime.supportsQuickLaunch
        else { return nil }
        self.rawValue = runtime.legacySlug
    }

    private init(runtime: SupercliRuntimeMetadata) {
    ...
    static var allCases: [QuickPresetTool] {
        SupercliRuntimeCatalog.runtimes(for: .macos)
    ...
    var metadata: SupercliRuntimeMetadata? { SupercliRuntimeCatalog.runtime(id: rawValue) }
    ...
    static func detect(in command: String) -> QuickPresetTool? {
        guard let runtime = SupercliRuntimeCatalog.runtime(command: command, for: .macos),
              runtime.supportsQuickLaunch
        else { return nil }
...
/// CLIs Supercli knows how to detect during setup. This intentionally includes
...
struct SetupTool: RawRepresentable, CaseIterable, Identifiable, Hashable, Sendable {
    ...
    init?(rawValue: String) {
        guard let runtime = SupercliRuntimeCatalog.runtime(id: rawValue, for: .macos) else {
            return nil
        }
        self.rawValue = runtime.legacySlug
    }
    ...
    static var allCases: [SetupTool] {
        SupercliRuntimeCatalog.runtimes(for: .macos).map(SetupTool.init(runtime:))
    }
    ...
    static func detect(in command: String) -> SetupTool? {
        guard let runtime = SupercliRuntimeCatalog.runtime(command: command, for: .macos) else {
            return nil
        }
...
/// that predate the Tauri app's migration to <project>/.supercli.json — the
...
/// predates Supercli — exactly what first-run seeding wants to rank by.
...
final class ToolAvailability: @unchecked Sendable {
    ...
    private static func searchDirs() -> [URL] {
        ...
        for suffix in SupercliRuntimeCatalog.runtimes(for: .macos).flatMap(\.searchPathSuffixes) {
    ...
    private static func shellPath() -> String {
        ...
        let start = "__SUPERCLI_PATH_START__"
        let end = "__SUPERCLI_PATH_END__"
