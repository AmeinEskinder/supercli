...
//  local workspace home to another. Workspaces are isolated SUPERCLI_HOME
...
enum ProjectWorkspaceMove {
    ...
    /// Same bucket keying as SupercliStore.rebuildTree / removeProject: a
    ...
    static func sessionIsLive(home: URL, sessionID: String) -> Bool {
        ...
        let exists = SupercliStore.hostedChildProcessExists(manifest.pid)
        if exists == false { return false }
        return SupercliStore.manifestPidIdentity(manifest) != .notOurs
    }
