...
//  SupercliNative
//
//  Resolves agent TUI themes enough for Supercli's terminal frame. OpenCode and
//  Grok paint most of their UI with truecolor escape sequences, but the
//  surrounding Ghostty padding/titlebar background is still owned by Supercli.
//
...
struct TerminalFrameStyle {
    ...
    /// Provider-resolved background, if any. Nil means Supercli's default.
    let background: Background?

    /// True when this session's frame is driven by a provider TUI theme
    /// (OpenCode / Grok) rather than Supercli's default terminal surface.
...
/// — that is the ground truth for Supercli's titlebar / Ghostty default bg,
...
enum ProviderThemeWatchPaths {
    ...
    static func roots(workingDirectories: [String]) -> [String] {
        ...
        append(OpenCodeThemeResolver.supercliOpenCodeConfigDirectory())
...
enum OpenCodeThemeResolver {
    ...
    private static func configCandidates(workingDirectory: String?) -> [URL] {
        ...
        appendConfigFiles(in: supercliOpenCodeConfigDirectory(), to: &urls, seen: &seen)
    ...
    private static func themeCandidates(
        ...
        appendThemeFiles(names: names, in: supercliOpenCodeConfigDirectory(), to: &urls, seen: &seen)
    ...
    static func supercliOpenCodeConfigDirectory() -> URL {
        // Prefer the active workspace home when SUPERCLI_HOME is set, matching
        // hook install paths for blank/workspace instances.
        if let home = nonEmptyString(ProcessInfo.processInfo.environment["SUPERCLI_HOME"]) {
...
enum GrokThemeResolver {
    ...
    /// CLI canvas colors so Supercli's titlebar / Ghostty default background
