...
//  SupercliNative
//
//  Plain data types decoded from the existing Supercli on-disk contracts
//  (~/.supercli/app-state.json and app-sessions/*/manifest.json). These are
...
/// The capability half of a session's Supercli Sessions MCP grant. Mirrors
...
struct AppStateFile: Decodable {
    ...
    /// Supercli-managed worktrees. Absent ⇒ false.
    ...
    /// `profile_display_name` / `profile_avatar` — the Supercli Link profile:
    /// the nickname and emoji avatar presence surfaces show for this person.
    /// The TUI edits the same keys (Settings ▸ Remote ▸ Supercli Link). Absent
...
struct BrowserSettings: Equatable, Codable {
    ...
    var allowedDomains = ""
    /// "project" (default) = one Supercli-managed browser window/profile per
...
/// Host-resolved installed Supercli App identity (`active_app` in the
...
struct HostedSessionManifest: Decodable {
    ...
    /// only removes it after re-validating that it remains beneath SUPERCLI_HOME.
    ...
    /// Host-resolved installed Supercli App identity; nil for non-App sessions
...
struct SessionEntry: Codable, Identifiable, Hashable, Sendable {
    ...
    /// the pane's cwd (`SupercliStore.paneWorkingDirectory`); nil on rows from
    ...
    var supportsTranscriptCopy: Bool {
        guard let runtime = SupercliRuntimeCatalog.runtime(command: command),
              runtime.kind == .agent
        else { return false }
...
enum RuntimePresentation {
    static func command(for runtimeID: String?) -> String? {
        SupercliRuntimeCatalog.presentationCommand(forRuntimeID: runtimeID)
    }
}
