...
struct MobilePairedDeviceRecord: Codable, Equatable {
    ...
    /// Per-device Supercli Link scope: false keeps this device Direct/LAN-only —
...
final class MobileE2EKeychainStore: MobileE2EKeyStoring {
    private let service = "com.supercli.mobile.e2e"
    /// Keychain items are bundle-scoped, not SUPERCLI_HOME-scoped, so the
...
final class MobilePairingStore: @unchecked Sendable {
    ...
    init(
        ...
        do {
            ...
            NSLog("[SupercliNative] failed to reconcile mobile E2E keys: \(error)")
        }
    }
    ...
    func pair(_ request: RemotePairingRequest, now: Date = Date()) throws -> RemotePairingResponse {
        ...
        NotificationCenter.default.post(name: .supercliMobileDevicesChanged, object: nil)
        return response
    }
    ...
    private func pairLocked(
        ...
        try withFreshRecords { directoryDescriptor in
            ...
            // Supercli Remote credentials ride the same pairing exchange: the
    ...
    func setRelayAllowed(deviceID: String, allowed: Bool) {
        ...
        do {
            ...
            NSLog("[SupercliNative] failed to change device relay scope: \(error)")
            changed = false
        }
        if changed {
            NotificationCenter.default.post(name: .supercliMobileDevicesChanged, object: nil)
    ...
    func setWorkspaceScopedForCompatTesting(_ scoped: Bool?, deviceID: String) {
        do {
            ...
            NSLog("[SupercliNative] failed to change device workspace scope: \(error)")
        }
    }
    ...
    func rotateRelayCredentials(deviceID: String) -> RelayCredentials? {
        ...
        do {
            ...
            NSLog("[SupercliNative] failed to rotate relay credentials: \(error)")
            credentials = nil
        }
        if credentials != nil {
    ...
    func clearPushToken(deviceID: String) {
        do {
            ...
            NSLog("[SupercliNative] failed to clear push token: \(error)")
        }
    }
    ...
    func pushTargets() -> [(deviceID: String, token: String, environment: String)] {
        (try? withFreshRecords { _ in
            records.compactMap { record in
                // Pushes travel through the Supercli Link service, so a device
            ...
            return try withFreshRecords { directoryDescriptor in
                ...
                if nowMs - previous > 60_000 {
                    ...
                    do {
                        ...
                        NSLog("[SupercliNative] failed to persist device lastSeen: \(error)")
                    }
                }
                return id
            }
        ...
        do {
            changed = try withFreshRecords { directoryDescriptor in
                ...
                do {
                    ...
                    NSLog("[SupercliNative] failed to clean revoked shared E2E key: \(error)")
                }
                e2eKeyStore.delete(deviceID: id)
                return true
            }
        } catch {
            NSLog("[SupercliNative] failed to revoke mobile device: \(error)")
            changed = false
        }
        if changed {
    ...
    private func restoreKeychain(_ previousKey: Data?, deviceID: String) {
        if let previousKey {
            do {
                ...
                NSLog("[SupercliNative] failed to roll back Keychain E2E key: \(error)")
            }
        } else {
            e2eKeyStore.delete(deviceID: deviceID)
        }
    ...
    private func restoreSharedE2EKeyValue(
        ...
        do {
            ...
            NSLog("[SupercliNative] failed to roll back shared E2E key: \(error)")
        }
    }
    ...
    static func defaultMacID() -> String {
        let url = LaunchConfig.supercliDir
            .appendingPathComponent("mobile")
    ...
    /// sharing an SUPERCLI_HOME launch for the first time together. The lock is
    ...
    static func stableHostID(at url: URL) -> String {
        ...
        do {
            ...
            NSLog("[SupercliNative] failed to create Host identity directory: \(error)")
        ...
        guard fd >= 0 else {
            ...
            NSLog("[SupercliNative] failed to lock Host identity file")
            return fallback
        }
        defer { close(fd) }
        guard flock(fd, LOCK_EX) == 0 else {
            if let existing = persistedHostID(at: url) { return existing }
            let fallback = UUID().uuidString.lowercased()
            NSLog("[SupercliNative] failed to acquire Host identity lock")
            return fallback
        }
        ...
        do {
            ...
            NSLog("[SupercliNative] failed to persist mobile mac id: \(error)")
        }
        return id
    }
    ...
    static func randomToken(byteCount: Int) -> String {
        ...
        return Data(bytes).supercliBase64URLString()
    }
...
extension Notification.Name {
    ...
    static let supercliMobileDevicesChanged = Notification.Name(
        "supercli.native.mobileDevicesChanged"
    )
}
...
private extension Data {
    func supercliBase64URLString() -> String {
        base64EncodedString()
            .replacingOccurrences(of: "+", with: "-")
