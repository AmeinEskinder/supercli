...
enum WorkspaceSwitching {
    ...
    static func orderedRows(store: SupercliStore) -> [WorkspaceListRowModel] {
    ...
    static func isScoped(_ row: WorkspaceListRowModel, store: SupercliStore) -> Bool {
        switch row.kind {
    ...
    static func select(_ row: WorkspaceListRowModel, store: SupercliStore) {
        switch row.kind {
        case let .local(_, _, isCurrentInstance, _):
            if isCurrentInstance {
    ...
    /// row already scoped.
    static func selectSliding(_ row: WorkspaceListRowModel, store: SupercliStore) {
    ...
    static func cycle(by delta: Int, store: SupercliStore) {
        let rows = orderedRows(store: store)
...
extension SidebarListPage where LiveTree == EmptyView {
    ...
    init(source: SidebarListSource, store: SupercliStore) {
...
struct SidebarListContent: Equatable {
    ...
    struct SessionRow: Equatable, Identifiable {
        ...
        let paneItems: [SupercliStore.PaneSidebarItem]
...
struct SidebarPaneProjection {
    ...
    let itemsByRepresentative: [String: [SupercliStore.PaneSidebarItem]]
    ...
    init(
        ...
        var items: [String: [SupercliStore.PaneSidebarItem]] = [:]
        ...
        for group in reconciled.groups {
            ...
            let paneItems: [SupercliStore.PaneSidebarItem] = group.panes
                .compactMap { pane in
                    guard let sessionID = pane.content.sessionID,
                          let session = sessionsByID[sessionID]
                    else { return nil }
                    return SupercliStore.paneSidebarItem(
                        paneID: pane.id,
                        session: session,
...
enum SidebarListContentBuilder {
    ...
    private static func localContent(store: SupercliStore) -> SidebarListContent {
        ...
        let expanded: Set<String> = store.selectedHostScope == .local
            ...
            : Set(AppDefaults.shared.stringArray(
                forKey: SupercliStore.expandedProjectsKey
            ) ?? [])
    ...
    private static func snapshotContent(
        ...
        let sidebarGroupIDs: Set<String> = Set(
            snapshot.projects.compactMap { summary in
                guard summary.isGroup == true,
                      ...
                      summary.id == SupercliStore.projectSidebarGroupID(forRoot: parent)
                else { return nil }
                return summary.id
            }
        )
        ...
        for summary in snapshot.sessions {
            ...
            if knownProjectIDs.contains(summary.projectID) {
                entriesByProject[summary.projectID, default: []].append(
                    SupercliStore.sessionEntry(fromRemote: summary)
                )
            }
        }
        ...
        if isLocalScope {
            ...
            expanded = AppDefaults.shared.stringArray(
                forKey: SupercliStore.expandedProjectsKey + "." + hostKey
        ...
        /// SupercliStore.renderedDisplayedItems for a remote projection.
        func displayedItems(
            _ n: ProjectNode,
