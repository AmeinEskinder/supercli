...
//  ContentArea render remote state through SupercliStore's display projection.
...
struct RemoteTerminalPaneHostView: NSViewRepresentable {
    ...
    func updateNSView(_ container: SwapContainer, context _: Context) {
        ...
        pane.dropStabilizeHome = dropStabilizeHome ?? LaunchConfig.supercliDir
    ...
    init(
        ...
        case let .incompatible(message):
            ...
            contentBanner = Banner(
                ...
                message: "This Host uses an incompatible protocol. Update Supercli on both devices.",
                tint: Theme.danger,
                showsProgress: false
            )
...
/// Small non-modal status for the bundled `supercli serve` service. A healthy
