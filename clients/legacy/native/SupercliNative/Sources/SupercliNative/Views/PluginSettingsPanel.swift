...
/// per-agent Supercli integration (Install integration / Reinstall, what it edits,
/// what the agent can do with and without it) and the manual setup recipe; the Plugins
/// scope is the Supercli Apps catalog under its user-facing name.
struct PluginSettingsPanel: View {
    ...
    static let bannerDismissedKey = "supercli.native.agentsConnectBannerDismissed"
    ...
    private var description: String {
        switch scope {
        case .agents:
            return "Install agent CLIs on this Host, install their Supercli integrations, and choose what each one launches."
        case .plugins:
            return "Supercli Apps that open files and resources in a pane beside your agents. "
                + "Install and update them here; develop your own with `supercli apps link`."
        }
    }
    ...
    /// Installed agent CLIs whose Supercli integration the Host can install
    ...
    var body: some View {
            ...
            if runtime.snapshot == nil {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 5) {
                        if !canReorder {
                            Label("Update Supercli on this Host to reorder \(title.lowercased()).", systemImage: "info.circle")
        ...
        .task {
            ...
            guard environment["SUPERCLI_SNAPSHOT"] != nil,
                  let command = environment["SUPERCLI_TEST_SETTINGS_COMMAND"], !command.isEmpty else { return }
            for _ in 0..<100 {
    ...
    // MARK: Supercli integration (Agents scope)

    /// The row-level integration state. Installing runs the Host's
    /// `integrations.install` verb: hooks + the supercli MCP server registered
        ...
        if scope == .agents, item.installed, !item.isCustom, item.integrationInstallable {
            if pendingIntegrations.contains(item.id) {
                ...
                Label("Integration installed", systemImage: "checkmark.circle")
                    ...
                    .foregroundStyle(Theme.mutedForeground)
                    .frame(width: 22, height: 24)
                    .help("Integration installed: Supercli's hooks and MCP server are registered in \(item.name)'s own config.")
            } else {
                Button("Install integration") { connect(item) }
                    .buttonStyle(.bordered).controlSize(.small)
                    .disabled(!canConnect)
                    .help(canConnect
                        ? "Register Supercli's hooks and MCP server in \(item.name)'s own config"
    ...
    /// First-run nudge: agents were found on the Host that Supercli could
    ...
    private var connectBanner: some View {
        ...
        return HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(count == 1
                    ? "\(unconnectedAgents[0].name) is installed without its Supercli integration"
                    : "\(count) agents are installed without their Supercli integration")
                    .font(.system(size: 12, weight: .semibold))
                Text("Install it for exact busy and idle, done notifications, transcripts, precise resume, "
                    + "and Supercli's tools inside each agent. Each integration registers Supercli's hooks and "
    ...
    private func agentDetails(_ item: PluginSettingsItem) -> some View {
        let meta = SupercliRuntimeCatalog.runtime(id: item.id)
    ...
    private func integrationTitle(_ item: PluginSettingsItem) -> String {
        guard item.integrationInstallable else { return "No Supercli integration" }
        return item.integrationInstalled ? "Supercli integration installed" : "Supercli integration not installed"
    }
    private func integrationSummary(_ item: PluginSettingsItem) -> String {
        guard item.integrationInstallable else {
            return "\(item.name) has no hooks or MCP configuration Supercli can register; Supercli still recognizes it by name."
        }
        return item.integrationSummary
            ?? "Registers Supercli's hooks and the supercli MCP server in \(item.name)'s own configuration."
    }
    ...
    private func capabilitiesLine(_ item: PluginSettingsItem, meta: SupercliRuntimeMetadata?) -> String {
        ...
        DisclosureGroup("Manual setup", isExpanded: $showManualSetup) {
            VStack(alignment: .leading, spacing: 6) {
                Text("Every integration points the agent's MCP config at one launcher on this Host. "
                    ...
                    + "get Supercli's tools without the hooks; Install integration does both.")
                    .font(.system(size: 11)).foregroundStyle(Theme.mutedForeground)
                    .fixedSize(horizontal: false, vertical: true)
                ...
                Text("Or from any terminal: supercli integrations install <agent>")
