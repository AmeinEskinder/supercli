...
extension Notification.Name {
    ...
    static let supercliCloseActivePane = Notification.Name(
        "supercli.terminal-pane.close-active"
    )
}
...
struct TerminalPaneContainer: View {
    ...
    /// `supercli-attach` surface on its `session.sock`; its home is on this
    ...
    private struct PaneConfirmationOverlay: View {
        ...
        private var message: String {
            switch confirmation.action {
            case .archive:
                return "This stops “\(confirmation.label)” and files it in Supercli so you can restore and resume later. Screenshots and other session files stay."
            case .remove:
                return "This only removes “\(confirmation.label)” from Supercli. It does not delete the agent’s conversation."
            }
        }
    ...
    /// rather than removing the Supercli session.
        ...
        if let entry {
            ...
            menu.addItem(controller.item("Copy session ID") {
                ...
                NSPasteboard.general.setString(
                    "Supercli Session ID: \(entry.id)", forType: .string
                )
            })
            ...
            // Agents, with their manage entry at the bottom of the list.
            menu.addItem(.separator())
    ...
    private func presetMenuItem(
        ...
        let runtime = SupercliRuntimeCatalog.runtime(command: preset.command)
        let icon = runtime.map(SupercliToolIcon.forRuntime) ?? .terminal
