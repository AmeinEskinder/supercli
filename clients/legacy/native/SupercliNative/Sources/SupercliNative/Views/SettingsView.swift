...
//  - Agent access → the supercli MCP policies (Sessions + Browser)
...
enum SettingsTab: String, CaseIterable, Identifiable {
    ...
    case agents
    /// Supercli Apps under their user-facing name.
    case plugins
    /// What agents may do through Supercli's tools (the Sessions and Browser
...
struct SettingsSidebarPanel: View {
    ...
    private static let feedbackURL = URL(string: "https://github.com/orgs/supercli-com/discussions")!
...
private struct HostAppearanceSettingsPanel: View {
    ...
    private var suite: UserDefaults { AppDefaults.suite(forSupercliHome: home) }
    ...
    var body: some View {
        ...
        .onChange(of: mode) { newMode in
            ...
            suite.set(newMode.rawValue, forKey: SupercliStore.nativeThemeKey)
            notifyTarget()
        ...
        .onChange(of: tint) { newTint in
            guard loaded else { return }
            suite.set(newTint.rawValue, forKey: SupercliStore.nativeAppTintKey)
            notifyTarget()
            // Repaint this window's chrome + the pickers' dots.
            store.applyScopeTint()
            NotificationCenter.default.post(
                name: .supercliWorkspaceTintChanged, object: nil
            )
        }
    ...
    private var hasOverrides: Bool {
        suite.string(forKey: SupercliStore.nativeThemeKey) != nil
    ...
    private func revertToDefault() {
        suite.removeObject(forKey: SupercliStore.nativeThemeKey)
    ...
    private func load() {
        mode = SupercliStore.workspaceThemePreference(home: home)
        tint = SupercliStore.workspaceTint(home: home)
                ...
                if let settings {
                    Section {
                        Picker(
                            ...
                            ForEach(SupercliStore.autoStopArchiveMinuteOptions, id: \.self) { minutes in
                                Text(SupercliStore.autoStopArchiveLabel(for: minutes)).tag(minutes)
                            }
                        }
                        ...
                        Picker(
                            ...
                            ForEach(SupercliStore.sidebarStoppedLimitOptions, id: \.self) { limit in
                                Text(SupercliStore.sidebarStoppedLimitLabel(for: limit)).tag(limit)
                            }
                        }
                    } header: {
                        SettingsSectionHeader(
...
private struct HostNotificationsSettingsPanel: View {
                    ...
                    LabeledContent {
                        Toggle(
                            ...
                            isOn: Binding(
                                ...
                                set: { newValue in
                                    ...
                                    suite.set(
                                        newValue,
                                        forKey: SupercliStore.menuAttentionDetectionKey
                                    )
                        ...
                        VStack(alignment: .leading, spacing: 1) {
                            ...
                            Text("Show the yellow attention dot when an agent draws a "
                                ...
                                + "their own, so Supercli reads them off the screen.")
                                .font(.system(size: 11))
                                .foregroundStyle(Theme.mutedForeground)
                                .fixedSize(horizontal: false, vertical: true)
                        }
    ...
    private func load() {
        let own = suite.object(forKey: SupercliStore.menuAttentionDetectionKey) as? Bool
        hasOverride = own != nil
    ...
    private func revertToDefault() {
        suite.removeObject(forKey: SupercliStore.menuAttentionDetectionKey)
        loaded = false
        load()
        WorkspacesSettingsPanel.pingReloadAppearance(
            homeDir: URL(fileURLWithPath: home, isDirectory: true)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            Form {
                ...
                Section {
                    ForEach(SupercliFeatureFlags.availableShippedFeatures) { feature in
                        featureRow(feature)
                    }
                }

                if !SupercliFeatureFlags.availableExperimentalFeatures.isEmpty {
                    Section {
                        ForEach(SupercliFeatureFlags.availableExperimentalFeatures) { feature in
                            featureRow(feature)
                        }
                    } header: {
                        SettingsSectionHeader(
            ...
            Toggle("", isOn: Binding(
                ...
                set: { newValue in
                    ...
                    if feature == .computerUse {
                        ...
                        SupercliStore.writeComputerUseExperiment(
                            newValue,
        ...
        VStack(alignment: .leading, spacing: 0) {
            SettingsPaneHeader(
                ...
                description: runtime.snapshot == nil
                    ...
                    : "\(scopeName) is running an older Supercli Host that "
            ...
            if runtime.snapshot == nil {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("Update Supercli on \(scopeName), then reconnect this "
...
private struct SettingsScopePickerControl: View {
    ...
    var body: some View {
        ...
        .onReceive(
            NotificationCenter.default.publisher(for: .supercliWorkspaceTintChanged)
        ) { _ in refreshActiveRow() }
        .onReceive(
            NotificationCenter.default.publisher(for: .supercliWorkspaceListChanged)
        ) { _ in refreshActiveRow() }
    ...
    private var activeName: String {
        activeRow?.name
            ...
            ?? SupercliWorkspaceContext.advertisedHostName
    }
...
struct SettingsContentHost: View {
    ...
    private var localPanel: some View {
        ...
        case .mobile:
            RemoteSettingsPanel(store: store)
        case .workspaces:
            // Supercli Link (license + enrollment) lives on the Remote tab.
...
/// visibleCases.
@MainActor
func resolvedSettingsTab(_ store: SupercliStore) -> SettingsTab {
...
struct SettingsTitleStrip: View {
    ...
    private func refreshEditedWorkspaceName() {
        if let name = SupercliWorkspaceContext.displayName {
            editedWorkspaceName = name
            return
        }
        let otherScopesExist = store.selectedHostScope != .local
            || !SupercliWorkspaceRegistry.load().isEmpty
            || !store.remoteHostStore.records.isEmpty
        editedWorkspaceName =
            otherScopesExist ? SupercliWorkspaceContext.advertisedHostName : nil
    }
}
        ...
        VStack(alignment: .leading, spacing: 0) {
            Form {
                ...
                if !SupercliWorkspaceContext.isDefaultInstance {
                    Section {
                        Button(
                            "Use \(SupercliWorkspaceContext.defaultWorkspaceName ?? "Personal")'s appearance"
                        ) {
                            store.revertAppearanceToInheritedBaseline()
                        }
                    } header: {
                        SettingsSectionHeader(
                            title: "Inherits from \(SupercliWorkspaceContext.defaultWorkspaceName ?? "Personal")",
...
struct RemoteSettingsPanel: View {
    ...
    private var remoteDescription: String {
        if let scope = scopedLocalWorkspace {
            return "Let another Supercli device control \(scope.name). Devices "
        ...
        VStack(alignment: .leading, spacing: 0) {
            Form {
                ...
                if let scope = scopedLocalWorkspace {
                    ...
                    testflightSection
                    // Supercli Link: the enrollment list (which replaced the
    ...
    private func scopedLinkFootnote(_ scope: (home: String, name: String)) -> some View {
        Section {
            Text("Supercli Link enrollment for \(scope.name)'s devices lives in "
                + "\(scope.name)'s own Remote Control settings; the license "
                + "covers every workspace on this Mac and is managed from "
                + "\(SupercliWorkspaceContext.advertisedHostName)'s scope.")
                .font(.system(size: 12))
                .foregroundStyle(Theme.mutedForeground)
                .fixedSize(horizontal: false, vertical: true)
        }
        ...
        Section {
            if store.remoteHostRuntime.supportsHostOperation(
                ...
                Text("\(scopeName) cannot mint pairing invitations over this "
                    + "connection. Pair devices from its own running Supercli "
    ...
    private var testflightSection: some View {
        Section {
            HStack(alignment: .center, spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("Supercli for iPhone is in beta")
        ...
        Section {
            ...
            if management.value.devices.isEmpty {
                ...
                ForEach(management.value.devices) { device in
                    LabeledContent {
                        HStack(spacing: 12) {
                            // Link scope is managed in the Supercli Link
...
/// list and the Supercli Link enrollment list so both describe a device the
/// same way.
...
struct ShareThisMacSheet: View {
    ...
    init(store: SupercliStore, usesWorkspaceLanguage: Bool = false) {
        self.store = store
        self.management = store.hostManagement
    ...
    /// The terminal counterpart of the QR code: the `supercli` CLI controls
    ...
    private var cliCommand: String {
        "supercli --host ssh://\(Self.sshHostName)"
    }
            ...
            if store.hostPairingCompleted {
                ...
                HStack(alignment: .top, spacing: 18) {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        Text("Scan or paste this code into another Supercli device.")
                            .font(.system(size: 13, weight: .semibold))
                            .foregroundStyle(Theme.foreground)
...
/// Settings ▸ Remote ▸ On Supercli Link — the enrollment list that replaced
...
private struct LinkEnrollmentSection: View {
    ...
    init(store: SupercliStore, hosts: RemoteHostStore, usesWorkspaceLanguage: Bool) {
        self.store = store
        self.hosts = hosts
        self.management = store.hostManagement
        self.usesWorkspaceLanguage = usesWorkspaceLanguage
    ...
    var body: some View {
        Section {
            ...
            .help(addCandidatesEmpty
                ...
                : "Enroll a paired device or Host on Supercli Link.")
        } header: {
            SettingsSectionHeader(
                title: "Reachable outside your network (Supercli Link)",
    ...
    private func enrollmentRow(
        ...
        LabeledContent {
            ...
            Button("Remove from Link", action: remove)
                ...
                .controlSize(.small)
                .help("Take this device off Supercli Link — it then connects only over your own network. Re-add it any time with \u{201C}Add to Link…\u{201D}.")
        } label: {
            HStack(spacing: 10) {
...
// MARK: - Supercli Sessions MCP panel


/// How Supercli gets the user's attention: the menu-waiting attention badge and
...
struct NotificationsSettingsPanel: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            Form {
                Section {} header: {
                    SettingsPaneHeader(
                        title: "Notifications",
                        description: "How Supercli flags a session that needs you and "
                ...
                if !SupercliWorkspaceContext.isDefaultInstance {
                    Section {
                        Button(
                            "Use \(SupercliWorkspaceContext.defaultWorkspaceName ?? "Personal")'s notifications"
                        ) {
                            store.revertNotificationsToInheritedBaseline()
                        }
                        .disabled(!SupercliStore.hasOwnMenuAttentionSetting)
                    } header: {
                        SettingsSectionHeader(
                            title: "Inherits from \(SupercliWorkspaceContext.defaultWorkspaceName ?? "Personal")",
                            description: "This workspace uses the default "
    ...
    private var notificationsSection: some View {
        Section {
            ...
            SettingsValueRow(label: "Supercli Link", value: uplink.status.label)
        ...
        VStack(alignment: .leading, spacing: 0) {
            Form {
                Section {} header: {
                    SettingsPaneHeader(
                        title: "Features",
                        description: "Turn Supercli's optional features on or off — no "
                            + "restart needed."
                    )
                    .padding(.bottom, 4)
                }
                ...
                if !SupercliWorkspaceContext.isDefaultInstance {
                    Section {
                        Button(
                            "Use \(SupercliWorkspaceContext.defaultWorkspaceName ?? "Personal")'s features"
                        ) {
                            store.revertExperimentalToInheritedBaseline()
                        }
                        .disabled(
                            !AppFeature.all.contains {
                                SupercliFeatureFlags.hasOwnSetting($0)
                            }
                        )
                    } header: {
                        SettingsSectionHeader(
                ...
                if SupercliFeatureFlags.availableFeatures.isEmpty {
                    Section {
...
struct AdvancedSettingsPanel: View {
    ...
    private var diagnosticsSection: some View {
        Section {
            ...
            LabeledContent {
                Button("Show in Finder") {
                    let trace = LaunchConfig.supercliDir
                    ...
                    if FileManager.default.fileExists(atPath: trace.path) {
                        ...
                        NSWorkspace.shared.open(LaunchConfig.supercliDir)
                    }
                }
                .controlSize(.small)
            } label: {
...
enum AdvancedDiagnostics {
    ...
    static func collect() -> Snapshot {
        ...
        // Native renames win over the manifest label, same as SupercliStore.
