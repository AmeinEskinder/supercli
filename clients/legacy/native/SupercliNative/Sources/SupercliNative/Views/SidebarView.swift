...
final class SidebarDragState: ObservableObject {
    ...
    /// drag (gated by `SupercliStore.canMoveSession`). Worktree rows never
...
struct SidebarView: View {
    ...
    private static let treeTopScrollID = "supercli.sidebar.tree-top"
...
struct ProjectNodeView: View {
    ...
    private var projectRow: some View {
        ...
        let isGitProject = isLocalMachine
            && node.project.isFolder != true
            && SupercliStore.isGitRepo(path: node.project.path)
        ...
        if isProjectDraggable {
            row
                ...
                // drop through `SupercliStore.canMoveSession`.
...
struct ProjectRowView: View {
    ...
    var activeBranchStore: SupercliStore?
    let onToggle: () -> Void
    ...
    /// SUPERCLI_DEBUG_HOVER_PROJECT=<name|id> forces this row's hover state
    /// and the expanded strip, so snapshots can photograph hover-only UI.
    private static let debugHoverProject =
        ProcessInfo.processInfo.environment["SUPERCLI_DEBUG_HOVER_PROJECT"]
    ...
    /// (SupercliStore.removeProject), so the confirm names the count.
    ...
    private var normalRow: some View {
        ...
        // exited). Supercli Sessions MCP access is set per session.
        .contextMenu {
...
func sessionRowActivitySpinnerCommand(
    session: SessionEntry,
    paneItems: [SupercliStore.PaneSidebarItem]
) -> String? {
...
struct SessionRowView: View {
    ...
    var paneItems: [SupercliStore.PaneSidebarItem] = []
    ...
    /// SUPERCLI_DEBUG_HOVER_SESSION=<session-id> forces this row's hover
    /// state so snapshots can photograph its hover actions.
    private static let debugHoverSession =
        ProcessInfo.processInfo.environment["SUPERCLI_DEBUG_HOVER_SESSION"]
    ...
    private var regularContextMenuItems: some View {
        ...
        // Every Supercli Session has an id. It is useful beyond agent/MCP
        // features, so keep it as a direct session-level action.
        Button("Copy session ID") {
            copyToPasteboard("Supercli Session ID: \(session.id)")
        }
        Divider()
        ...
        // Remove stays the explicit destructive verb: it deletes Supercli's
...
private struct SessionCommandIconPresentation: View {
    var appID: String? = nil
    let command: String
    let items: [SupercliStore.PaneSidebarItem]
...
private struct SessionCommandIconStack: View {
    ...
    private var visibleItems: [SupercliStore.PaneSidebarItem] {
...
struct ActiveProjectBranchLabel: View {
    ...
    init(store: SupercliStore, projectID: String) {
        self.store = store
