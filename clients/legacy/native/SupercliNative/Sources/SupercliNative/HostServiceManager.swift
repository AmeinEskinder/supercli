...
//  SupercliNative
//
//  Starts the canonical Rust Host service embedded in `supercli-host`. The
...
final class HostServiceManager: ObservableObject {
    ...
    private var platformAdapterHandle: supercli_native_bridge_platform_adapter_handle_t?
    ...
    func ensureStarted(now: Date = Date(), force: Bool = false) {
        ...
        guard FileManager.default.isExecutableFile(atPath: binary) else {
            NSLog("[SupercliNative] Host service: supercli-host not found at %@", binary)
            return
        }
        ...
        // dev-blank/test-style private instance, so preserve its SUPERCLI_HOME
        ...
        if SupercliWorkspaceContext.isDefaultInstance
            || SupercliWorkspaceContext.currentWorkspace() != nil
        {
            environment.removeValue(forKey: "SUPERCLI_HOME")
            ...
            NSLog("[SupercliNative] Host service: %@=direct, forking the service", HostServiceAgent.launcherOverrideEnvVar)
        }
    ...
    private static let launchdQueue = DispatchQueue(
        label: "com.supercli.native.host-service-agent",
        qos: .userInitiated
    )
    ...
    /// The pre-launchd launcher: fork `supercli-host __serve__` detached from
    ...
    private nonisolated static func spawnDirectly(binary: String, environment: [String: String]) {
        ...
        do {
            ...
            NSLog("[SupercliNative] Host service launch requested (pid %d)", process.processIdentifier)
        } catch {
            NSLog("[SupercliNative] Host service failed to launch: %@", error.localizedDescription)
        }
    }
    ...
    func startPlatformAdapter(on server: HookServer) {
        ...
        do {
            config = try JSONEncoder().encode(Config(
                supercliHome: LaunchConfig.supercliDir.standardizedFileURL.path,
            ...
            NSLog("[SupercliNative] Host platform adapter config failed: %@", error.localizedDescription)
            return
        }

        var handle: supercli_native_bridge_platform_adapter_handle_t = 0
        var outputPointer: UnsafeMutablePointer<UInt8>?
        var outputLength = 0
        let result = config.withUnsafeBytes { bytes in
            supercli_native_bridge_platform_adapter_start(
        ...
        guard result == SUPERCLI_NATIVE_BRIDGE_OK, handle != 0 else {
            server.platformAdapterToken = nil
            let message = String(data: output, encoding: .utf8) ?? "unknown bridge error"
            NSLog("[SupercliNative] Host platform adapter failed: %@", message)
            return
        }
    ...
    func stopPlatformAdapter() {
        ...
        var outputPointer: UnsafeMutablePointer<UInt8>?
        var outputLength = 0
        let result = supercli_native_bridge_platform_adapter_stop(
            handle,
            &outputPointer,
            &outputLength
        )
        let output = Self.takeBridgeOutput(outputPointer, length: outputLength)
        if result != SUPERCLI_NATIVE_BRIDGE_OK {
            let message = String(data: output, encoding: .utf8) ?? "unknown bridge error"
            NSLog("[SupercliNative] Host platform adapter stop failed: %@", message)
        }
    }
