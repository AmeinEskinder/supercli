...
//  SupercliNative
//
//  Preset writes to the shared ~/.supercli/app-state.json contract. Since the
...
//  with the terminal UI (`supercli`), which edits the same file. Edits happen
...
enum PresetStateFile {
    ...
    /// Edit this instance's own `~/.supercli/app-state.json`.
    @discardableResult
    ...
    private static func lockedEdit(
        ...
        do {
            ...
            NSLog("[SupercliNative] preset state write failed: \(error)")
            return false
        }
    }
