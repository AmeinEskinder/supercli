...
final class NativeRemoteOutputPage: @unchecked Sendable {
    ...
    private var resolution: (
        parent: supercli_native_bridge_remote_handle_t,
        page: supercli_native_bridge_remote_output_page_handle_t
    )?

    init(
        metadata: NativeRemoteOutputPageMetadata,
        bytes: Data,
        parentHandle: supercli_native_bridge_remote_handle_t,
        pageHandle: supercli_native_bridge_remote_output_page_handle_t
    ) {
...
extension NativeRemoteBackendProtocol {
    func setOpener(
        ...
        throw NativeRemoteBackendError(
            result: Int32(SUPERCLI_NATIVE_BRIDGE_ERROR_REMOTE),
    ...
    func pluginUpdates() async throws -> RemotePluginUpdates {
        throw NativeRemoteBackendError(result: Int32(SUPERCLI_NATIVE_BRIDGE_ERROR_REMOTE),
...
final class NativeRemoteBackend: @unchecked Sendable {
    ...
    private var handle: supercli_native_bridge_remote_handle_t
    ...
    private var identityValidatedHandle: supercli_native_bridge_remote_handle_t?
    ...
    static func installSupercli(
        ...
        try await runBlocking(priority: .userInitiated) {
            ...
            let result = config.withUnsafeBytes { bytes in
                supercli_native_bridge_remote_ssh_install(
            ...
            try Task.checkCancellation()
            guard result == SUPERCLI_NATIVE_BRIDGE_OK else {
                throw bridgeError(
                    result: result,
                    output: output,
                    fallbackCode: "ssh_install_failed",
                    fallbackMessage: "Could not install Supercli on the SSH Host."
                )
            }
        }
    }
    ...
    init(
        ...
        guard supercli_native_bridge_abi_version() == Self.supportedABIVersion else {
            throw NativeRemoteBackendError(
                result: Int32(SUPERCLI_NATIVE_BRIDGE_ERROR_INVALID_INPUT),
                code: "unsupported_bridge_abi",
                message: "This Supercli build has an incompatible remote Host bridge."
            )
        }
        ...
        var openedHandle: supercli_native_bridge_remote_handle_t = 0
        var outputPointer: UnsafeMutablePointer<UInt8>?
        var outputLength = 0
        let result = config.withUnsafeBytes { bytes in
        ...
        guard result == SUPERCLI_NATIVE_BRIDGE_OK, openedHandle != 0 else {
    ...
    /// app's bundled `supercli-host __remote_stdio__` against the workspace's
            ...
            bearer.withUnsafeBytes { bearerBytes in
                if let certificateFingerprint {
                    return Data(certificateFingerprint.utf8).withUnsafeBytes { pinBytes in
                        supercli_native_bridge_remote_direct_open_pinned(
                ...
                return supercli_native_bridge_remote_direct_open(
                    endpointBytes.bindMemory(to: UInt8.self).baseAddress,
                    endpointBytes.count,
                    bearerBytes.bindMemory(to: UInt8.self).baseAddress,
                    bearerBytes.count,
    ...
    init(
        ...
        guard !expectedHostID.isEmpty,
            ...
            throw Self.invalidInput(
                ...
                message: "Supercli Link credentials do not match the paired Host. Pair it again."
            )
        }
    ...
    func bootstrap() async throws -> RemoteBootstrapSnapshot {
        ...
        let snapshot = try await Self.runBlocking(priority: .userInitiated) {
            ...
            var outputPointer: UnsafeMutablePointer<UInt8>?
            var outputLength = 0
            let result = supercli_native_bridge_remote_bootstrap(
                handle,
                &outputPointer,
                &outputLength
            )
            ...
            do {
                ...
                throw NativeRemoteBackendError(
                    result: Int32(SUPERCLI_NATIVE_BRIDGE_ERROR_SERIALIZATION),
    ...
    private func pollOutput(
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            try Task.checkCancellation()
            var pageHandle: supercli_native_bridge_remote_output_page_handle_t = 0
            ...
            let result = session.withUnsafeBytes { sessionBytes in
                switch cursor {
                case .current:
                    supercli_native_bridge_remote_output_poll(
                        handle,
                ...
                case let .requested(requestedOffset):
                    supercli_native_bridge_remote_output_poll_from(
                        handle,
                        sessionBytes.bindMemory(to: UInt8.self).baseAddress,
                        sessionBytes.count,
                        requestedOffset ?? 0,
            ...
            guard result == SUPERCLI_NATIVE_BRIDGE_OK, pageHandle != 0 else {
                throw Self.bridgeError(
                    result: result,
                    output: metadataData,
    ...
    func commitOutput(_ page: NativeRemoteOutputPage) async throws {
        ...
        try await Self.runBlocking(priority: .userInitiated) {
            var outputPointer: UnsafeMutablePointer<UInt8>?
            var outputLength = 0
            let result = supercli_native_bridge_remote_output_commit(
                handle,
                resolution.page,
                &outputPointer,
                &outputLength
    ...
    private typealias SessionEffectFFI = @convention(c) (
        supercli_native_bridge_remote_handle_t,
        UnsafePointer<UInt8>?,
        Int,
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            ...
            let result = session.withUnsafeBytes { sessionBytes in
                titleData.withUnsafeBytes { titleBytes in
                    supercli_native_bridge_remote_session_title_set(
                        handle,
                        sessionBytes.bindMemory(to: UInt8.self).baseAddress,
                        sessionBytes.count,
    ...
    func setSessionPinned(
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            ...
            var outputLength = 0
            let result = session.withUnsafeBytes { bytes in
                supercli_native_bridge_remote_session_pinned_set(
                    handle,
                    bytes.bindMemory(to: UInt8.self).baseAddress,
                    bytes.count,
                    pinnedFlag,
    ...
    func setSessionNotifyWhenDone(
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            ...
            var outputLength = 0
            let result = session.withUnsafeBytes { bytes in
                supercli_native_bridge_remote_session_notify_when_done_set(
                    handle,
                    bytes.bindMemory(to: UInt8.self).baseAddress,
                    bytes.count,
                    enabledFlag,
    ...
    func answerApproval(
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            ...
            let result = approval.withUnsafeBytes { bytes in
                supercli_native_bridge_remote_approval_answer(
                    handle,
                    bytes.bindMemory(to: UInt8.self).baseAddress,
                    bytes.count,
                    approvedFlag,
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            ...
            let result = session.withUnsafeBytes { sessionBytes in
                project.withUnsafeBytes { projectBytes in
                    supercli_native_bridge_remote_session_project_set(
                        handle,
                        sessionBytes.bindMemory(to: UInt8.self).baseAddress,
                        sessionBytes.count,
    ...
    func archiveSession(sessionID: String) async throws -> NativeRemoteEffectReceipt {
        try await performSessionEffect(
            ...
            call: supercli_native_bridge_remote_session_archive
        )
    }
    ...
    func restoreSession(sessionID: String) async throws -> NativeRemoteEffectReceipt {
        try await performSessionEffect(
            ...
            call: supercli_native_bridge_remote_session_restore
        )
    }
    ...
    func stopSession(sessionID: String) async throws -> NativeRemoteEffectReceipt {
        try await performSessionEffect(
            ...
            call: supercli_native_bridge_remote_session_stop
        )
    }
    ...
    func removeSession(sessionID: String) async throws -> NativeRemoteEffectReceipt {
        try await performSessionEffect(
            ...
            call: supercli_native_bridge_remote_session_remove
        )
    }
    ...
    func restartSession(sessionID: String) async throws -> NativeRemoteEffectReceipt {
        try await performSessionEffect(
            ...
            call: supercli_native_bridge_remote_session_restart
        )
    }
    ...
    func reloadSession(sessionID: String) async throws -> NativeRemoteEffectReceipt {
        try await performSessionEffect(
            ...
            call: supercli_native_bridge_remote_session_reload
        )
    }
    ...
    func resumeAgent(sessionID: String) async throws -> NativeRemoteEffectReceipt {
        try await performSessionEffect(
            ...
            call: supercli_native_bridge_remote_session_resume_agent
        )
    }
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            ...
            let result = project.withUnsafeBytes { projectBytes in
                patchJSON.withUnsafeBytes { patchBytes in
                    supercli_native_bridge_remote_project_organization_set(
                        handle,
                        projectBytes.bindMemory(to: UInt8.self).baseAddress,
                        projectBytes.count,
    ...
    func setPreset(patch: RemotePresetPatch) async throws -> NativeRemoteEffectReceipt {
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            ...
            let result = patchJSON.withUnsafeBytes { bytes in
                supercli_native_bridge_remote_preset_set(
                    handle,
                    bytes.bindMemory(to: UInt8.self).baseAddress,
                    bytes.count,
                    &outputPointer,
    ...
    func setWorkspaceSettings(
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            ...
            var outputLength = 0
            let result = patchJSON.withUnsafeBytes { bytes in
                supercli_native_bridge_remote_workspace_settings_set(
                    handle,
                    bytes.bindMemory(to: UInt8.self).baseAddress,
                    bytes.count,
                    &outputPointer,
    ...
    func setOpener(
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            ...
            let result = body.withUnsafeBytes { bytes in
                supercli_native_bridge_remote_opener_set(
                    handle,
                    bytes.bindMemory(to: UInt8.self).baseAddress,
                    bytes.count,
                    &outputPointer,
    ...
    func installApp(appID: String) async throws -> NativeRemoteEffectReceipt {
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            ...
            var outputLength = 0
            let result = body.withUnsafeBytes { bytes in
                supercli_native_bridge_remote_app_install(
                    handle,
                    bytes.bindMemory(to: UInt8.self).baseAddress,
                    bytes.count,
                    &outputPointer,
    ...
    func installIntegration(runtimeID: String) async throws -> NativeRemoteEffectReceipt {
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            ...
            var outputLength = 0
            let result = body.withUnsafeBytes { bytes in
                supercli_native_bridge_remote_integration_install(
                    handle,
                    bytes.bindMemory(to: UInt8.self).baseAddress,
                    bytes.count,
                    &outputPointer,
    ...
    func openApp(
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            ...
            var outputLength = 0
            let result = body.withUnsafeBytes { bytes in
                supercli_native_bridge_remote_app_open(
                    handle,
                    bytes.bindMemory(to: UInt8.self).baseAddress,
                    bytes.count,
                    &outputPointer,
    ...
    func createSession(
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            ...
            let result = requestJSON.withUnsafeBytes { bytes in
                supercli_native_bridge_remote_session_create(
                    handle,
                    bytes.bindMemory(to: UInt8.self).baseAddress,
                    bytes.count,
                    &outputPointer,
    ...
    func pairingInvitation(_ requestJSON: Data) async throws -> Data {
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            ...
            var outputLength = 0
            let result = requestJSON.withUnsafeBytes { bytes in
                supercli_native_bridge_remote_pairing_invitation(
                    handle,
                    bytes.bindMemory(to: UInt8.self).baseAddress,
                    bytes.count,
                    &outputPointer,
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            ...
            let result = request.withUnsafeBytes { requestBytes in
                bytes.withUnsafeBytes { payload in
                    supercli_native_bridge_remote_resource(handle,
            ...
            let result = session.withUnsafeBytes { sessionBytes in
                type.withUnsafeBytes { typeBytes in
                    bytes.withUnsafeBytes { payloadBytes in
                        supercli_native_bridge_remote_upload_attachment(
                            handle,
                            sessionBytes.bindMemory(to: UInt8.self).baseAddress,
                            sessionBytes.count,
    ...
    func listArchivedSessions(projectID: String) async throws -> [RemoteSessionSummary] {
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            ...
            let result = project.withUnsafeBytes { bytes in
                supercli_native_bridge_remote_archived_sessions(
                    handle,
                    bytes.bindMemory(to: UInt8.self).baseAddress,
                    bytes.count,
                    &outputPointer,
    ...
    func transcriptMarkdown(
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            ...
            var outputLength = 0
            let result = session.withUnsafeBytes { bytes in
                supercli_native_bridge_remote_transcript_markdown(
                    handle,
                    bytes.bindMemory(to: UInt8.self).baseAddress,
                    bytes.count,
                    boundedEntries,
    ...
    func pluginUpdates() async throws -> RemotePluginUpdates {
        ...
        return try await Self.runBlocking(priority: .utility) {
            ...
            var pointer: UnsafeMutablePointer<UInt8>?
            var length = 0
            let result = supercli_native_bridge_remote_plugin_updates(handle, &pointer, &length)
            let output = Self.takeOutput(pointer, length: length)
            guard result == SUPERCLI_NATIVE_BRIDGE_OK else {
    ...
    func sessionMetrics(sessionID: String) async throws -> NativeRemoteSessionMetrics {
        ...
        return try await Self.runBlocking(priority: .userInitiated) {
            ...
            var outputLength = 0
            let result = session.withUnsafeBytes { bytes in
                supercli_native_bridge_remote_session_metrics(
                    handle,
                    bytes.bindMemory(to: UInt8.self).baseAddress,
                    bytes.count,
                    &outputPointer,
    ...
    private func currentHandle() throws -> supercli_native_bridge_remote_handle_t {
        lock.lock()
        defer { lock.unlock() }
        guard handle != 0 else {
            throw NativeRemoteBackendError(
                result: Int32(SUPERCLI_NATIVE_BRIDGE_ERROR_INVALID_HANDLE),
    ...
    private func markIdentityValidated(
        for expectedHandle: supercli_native_bridge_remote_handle_t
    ) throws {
        lock.lock()
        defer { lock.unlock() }
    ...
    private func takeHandle() -> supercli_native_bridge_remote_handle_t? {
        lock.lock()
        defer { lock.unlock() }
    ...
    private static func closeIgnoringError(_ handle: supercli_native_bridge_remote_handle_t) {
        var outputPointer: UnsafeMutablePointer<UInt8>?
        var outputLength = 0
        _ = supercli_native_bridge_remote_close(
            handle,
            &outputPointer,
            &outputLength
        )
    ...
    fileprivate static func discardIgnoringError(
        ...
        var outputPointer: UnsafeMutablePointer<UInt8>?
        var outputLength = 0
        _ = supercli_native_bridge_remote_output_discard(
            parentHandle,
            pageHandle,
            &outputPointer,
            &outputLength
