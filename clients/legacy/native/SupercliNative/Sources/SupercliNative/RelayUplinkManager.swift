...
//  SupercliNative
//
//  The app's client-side half of Supercli Link. The Relay uplink itself (the
//  Host WebSocket to apps/relay, tunneled requests, output streams) is owned
//  by the workspace worker (`supercli serve`). What stays here is what only
...
//  ~/.supercli/mobile/relay-entitlement.json.
//
...
/// Uplink diagnostics land in the shared `~/.supercli/hooks/trace.log`
...
enum RelayConfig {
    ...
    static let enabledDefaultsKey = "supercli.native.remoteAccessEnabled"
...
/// Client-side half of Supercli Link that stays in the app after the Swift
...
/// tunneled requests, output streams) is owned by `supercli serve`.
@MainActor
final class RelayUplinkManager: ObservableObject {
    ...
    func rejectLocalAuthority() -> String? {
        persistAuthorizationRejection(message: "Supercli Link authorization rejected")
    }
    ...
    func activationSuppressionGeneration() throws -> String? {
        try LinkAuthorityStore.suppression(home: LaunchConfig.supercliDir)?.generation
    }
    ...
    func refreshPlatformEntitlement(macID: String) async -> Bool {
        ...
        if LicenseConfig.developmentBuildLicenseBypassEnabled,
           let devToken = AppDefaults.shared.string(
               forKey: "supercli.native.relayDevToken"
    ...
    private func currentEntitlement(macID: String) async -> String? {
        ...
        if LicenseConfig.developmentBuildLicenseBypassEnabled,
           local.suppression == nil,
           let devToken = AppDefaults.shared.string(forKey: "supercli.native.relayDevToken"),
                ...
                if [400, 401, 402, 403, 404, 409, 410, 422].contains(http.statusCode) {
                    _ = persistAuthorizationRejection(
                        message: http.statusCode == 402
                            ? "Remote access requires Supercli Link"
    ...
    static func pushFailureLabel(reason: String?) -> String {
        switch reason {
        case "remote-disabled": return "Supercli Link is turned off"
        ...
        case "network": return "Could not reach Supercli Link"
