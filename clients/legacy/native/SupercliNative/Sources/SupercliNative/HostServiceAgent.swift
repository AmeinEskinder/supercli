...
//
//  The agent is the same per-user LaunchAgent shape `supercli serve install`
//  writes (packaging/service/com.supercli.serve.plist), under the app's own
...
enum HostServiceAgent {
    static let releaseLabel = "com.supercli.native.serve"
    /// Dev builds share the release bundle id but must never re-point the
    /// user's real unit at `dist/Supercli.app`; they get their own label.
    static let developmentLabel = "com.supercli.native.dev.serve"
    static let bundleIdentifier = "com.supercli.native"
    /// `SUPERCLI_NATIVE_SERVICE_LAUNCHER=direct` restores the pre-2026-09-06
    /// fork for diagnostics. Anything else (or unset) uses launchd.
    static let launcherOverrideEnvVar = "SUPERCLI_NATIVE_SERVICE_LAUNCHER"
    ...
    nonisolated static var isDevelopmentBuild: Bool {
        Bundle.main.object(forInfoDictionaryKey: "SupercliDevelopmentBuild") as? Bool == true
    }
    ...
    /// `~/Library/LaunchAgents` for the real user home (`SUPERCLI_HOME` never
