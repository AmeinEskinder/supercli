...
/// pins and presets: until the user picks a mode natively, Supercli follows
...
extension Notification.Name {
    /// Posted by SupercliStore.setAppTint after Theme.appTintHue changes, so
    /// non-SwiftUI consumers (SurfaceCache → live Ghostty panes) repaint.
    static let supercliAppTintChanged = Notification.Name("supercliAppTintChanged")
    ...
    /// everything expensive still waits for `supercliAppTintChanged`.
    static let supercliAppTintAnimationTick =
        Notification.Name("supercliAppTintAnimationTick")
    ...
    /// `supercliAppTintChanged`, which only fires when THIS window's chrome hue
    /// moves.
    static let supercliWorkspaceTintChanged = Notification.Name("supercliWorkspaceTintChanged")
    ...
    static let supercliWorkspaceListChanged = Notification.Name("supercliWorkspaceListChanged")
    ...
    static let supercliTransparencyChanged = Notification.Name("supercliTransparencyChanged")
    ...
    static let supercliTerminalFontChanged = Notification.Name("supercliTerminalFontChanged")
}
...
final class TransparencyModel: ObservableObject {
    ...
    func applyScopedPresentation(
        ...
        NotificationCenter.default.post(name: .supercliTransparencyChanged, object: nil)
    }
    ...
    private func valueChanged() {
        ...
        let work = DispatchWorkItem {
            NotificationCenter.default.post(
                name: .supercliTransparencyChanged, object: nil
            )
        }
...
final class TerminalFontModel: ObservableObject {
    ...
    /// (orgs/supercli-com discussions #9).
    ...
    func reloadFromDefaults() {
        ...
        suppressPersistence = false
        announceWorkItem?.cancel()
        NotificationCenter.default.post(name: .supercliTerminalFontChanged, object: nil)
    }
...
final class AppTintModel: ObservableObject {
    ...
    private init() {
        observer = NotificationCenter.default.addObserver(
            forName: .supercliAppTintChanged, object: nil, queue: .main
        ) { [weak self] _ in
...
/// while `.supercliAppTintChanged` fires only at completion, because each post
...
final class AppTintAnimator {
    ...
    func animate(
        ...
        let timer = Timer(timeInterval: 1.0 / 30.0, repeats: true) { [weak self] _ in
            MainActor.assumeIsolated {
                ...
                if tick % 3 == 0 {
                    NotificationCenter.default.post(
                        name: .supercliAppTintAnimationTick, object: nil
                    )
                }
                if raw >= 1 {
                    self.cancel()
    ...
    private func finish(toHue: Double?) {
        ...
        NotificationCenter.default.post(name: .supercliAppTintChanged, object: nil)
        completion?()
    }
    ...
    /// chrome — the expensive Ghostty restyle (`.supercliAppTintChanged`)
...
enum Theme {
    ...
    /// `SupercliStore.appTint`. Written on the main thread (launch + Appearance
    ...
    static func toolColorHex(forCommand command: String) -> Int? {
        SupercliRuntimeCatalog.runtime(command: command)?.tintColorHex
    }
    ...
    static func toolSpinnerColorHex(forCommand command: String) -> Int? {
        let runtime = SupercliRuntimeCatalog.runtime(command: command)
    ...
    /// one, else the ordinary foreground. Supercli Apps and plain terminals
...
struct TerminalPaneStyle {
    ...
    static func resolved(
        ...
        let runtime = SupercliRuntimeCatalog.runtime(id: runtimeID)
            ?? command.flatMap { SupercliRuntimeCatalog.runtime(command: $0) }
