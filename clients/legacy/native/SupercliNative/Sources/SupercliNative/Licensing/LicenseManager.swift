...
//
//  A Supercli Pro license is an Ed25519-signed token:
//
//      CLRTY-<payloadB64url>.<signatureB64url>
//
//  The app is free — a license is never required for local use. An active
//  license unlocks **Supercli Pro** (Supercli Remote relay access, iPhone
...
final class LicenseManager: ObservableObject {
    ...
    static let initialLoadNotification = Notification.Name("supercli.license.initialLoadComplete")

    /// Supercli Pro is unlocked by an active license (or the dev-build bypass).
    /// The app itself is free and never gated; Pro covers the device-linking
    /// features — Supercli Remote relay access, iPhone pairing, and workspaces.
    var isPro: Bool {
    ...
    var updateAuthorizationHeaders: [String: String]? {
        ...
        return [
            "X-Supercli-License": stored,
            "X-Supercli-Device-ID": Self.deviceID
        ]
    }
    ...
    func activate(_ rawKey: String) async {
        ...
        guard let payload = verify(key) else {
            lastError = "That doesn't look like a valid Supercli license key."
            return
        }
        licenseStateGeneration += 1
    ...
    nonisolated static func message(for result: [String: Any]) -> String {
        ...
        case "seat_limit":
            if let seats = result["seats"] as? Int {
                return "All \(seats) seats for this license are in use. Deactivate Supercli "
                    + "on another Mac first, or contact support."
            }
            return "All purchased seats for this license are in use. Deactivate Supercli on "
                + "another Mac first, or contact support."
        case "revoked":
            return "This license has been revoked."
        case "unknown":
            return "This key isn't recognized. If you just bought Supercli, try again in a moment."
        default:
