...
private final class PathDraggableTerminalView: TerminalView, NSDraggingSource {
    ...
    private func beginPathDrag(_ path: String, with event: NSEvent, at point: CGPoint) -> Bool {
        ...
        // not consume file URLs. Supercli's terminal target prefers `.fileURL`
...
final class GhosttyTerminalPane: NSView {
    ...
    /// at the project boundary the user recognizes in Supercli.
    ...
    ///     itself; e.g. "/bin/zsh --login" or "supercli-attach <session-id>").
    ...
    init(
        ...
        if let issue = controller.lastConfigurationIssue {
            NSLog("[SupercliNative] ghostty configuration issue: %@", issue)
        }
        ...
        findObservers = [
            center.addObserver(
                forName: .supercliTerminalFind, object: nil, queue: .main
            ) { [weak self] _ in
            ...
            center.addObserver(
                forName: .supercliTerminalFindNext, object: nil, queue: .main
            ) { [weak self] _ in
                MainActor.assumeIsolated {
                    guard let self, self.isDisplayedForFind else { return }
                    self.findNext()
            ...
            center.addObserver(
                forName: .supercliTerminalFindPrevious, object: nil, queue: .main
            ) { [weak self] _ in
                MainActor.assumeIsolated {
                    guard let self, self.isDisplayedForFind else { return }
                    self.findPrevious()
    ...
    /// (`supercli-attach`) without waiting for ARC/deinit timing. Idempotent so
    ...
    /// plus suspension of the draw that follows each wakeup, mirroring what
    /// Ghostty itself does on `NSWindow.occlusionState` changes. The wakeup's
    /// tick (draining Ghostty's app mailbox) always runs: a hidden surface
    /// whose mailbox stops draining wedges its io thread and deadlocks the
    /// main thread on the next focus (see `shouldRenderOnWakeup`).
    ...
    // a HIDDEN container and are not paused via setSurfaceVisible(false).
    // Pausing used to suspend the wrapper's wakeup→tick loop too, and a
    // surface that never ticks while its attach client floods the replay
    // wedges its IO; the next synchronous surface call from the main thread
    // (adoption on click) then deadlocked. The wrapper now ticks paused
    // surfaces as well (only the draw is skipped), so the same deadlock no
    // longer reaches detached SurfaceCache panes or a backgrounded app; the
    // hidden container already keeps warm panes out of the compositor.
    ...
    func pressReturn() {
        guard let event = NSEvent.keyEvent(
            ...
            NSLog("[SupercliNative] pressReturn: could not synthesize event")
            return
        }
    ...
    func dumpScreenText() -> String? {
        ...
        guard let raw = Mirror(reflecting: surface).children
            ...
            NSLog("[SupercliNative] dumpScreenText: raw surface not reachable")
            return nil
        }
    ...
    fileprivate static func dropReferences(
        ...
        home: URL = LaunchConfig.supercliDir
    ) -> [String] {
    ...
    private static func stableDropPath(
        _ url: URL, home: URL = LaunchConfig.supercliDir
    ) -> String {
        ...
        do {
            ...
            NSLog("[SupercliNative] failed to stabilize dropped file: \(error)")
            return url.path
        }
    }
    ...
    static func remoteFileBytes(at url: URL) async throws -> Data {
        ...
        guard metadata.st_size > 0, metadata.st_size <= 64 * 1024 * 1024 else {
            throw NSError(domain: "SupercliAttachment", code: 1, userInfo: [NSLocalizedDescriptionKey: "Files must be nonempty and no larger than 64 MB."])
        }
        try Task.checkCancellation()
        let bytes = try file.read(upToCount: 64 * 1024 * 1024 + 1) ?? Data()
        guard bytes.count == metadata.st_size else {
            throw NSError(domain: "SupercliAttachment", code: 2, userInfo: [NSLocalizedDescriptionKey: "The file changed while it was being read. Drop it again."])
        }
        return bytes
    }
    ...
    private static func saveDroppedImage(
        _ image: NSImage, home: URL = LaunchConfig.supercliDir
    ) -> String? {
        guard let tiff = image.tiffRepresentation,
              let rep = NSBitmapImageRep(data: tiff),
        ...
        do {
            ...
            NSLog("[SupercliNative] failed to save dropped image: \(error)")
            return nil
        }
    }
    ...
    TerminalSurfaceLifecycleDelegate
{
    func terminalDidAttachSurface(_ surface: TerminalSurface) {
        ...
        NSLog("[SupercliNative] surface attached")
    }
    ...
    func terminalDidDetachSurface() {
        ...
        NSLog("[SupercliNative] surface detached")
    }
    ...
    func terminalDidRingBell() {
        NSLog("[SupercliNative] bell (stub)")
    }
    ...
    private func openClickedFile(_ match: ClickablePath.Match) -> Bool {
        ...
        SupercliStore.openFileInPreferredEditor(
            path: resolved,
            line: match.line,
            column: match.column
        )
    ...
    /// Session (`SupercliStore.paneWorkingDirectory`), then replaced by the
    ...
    static func openTerminalURL(_ url: String) {
        guard let parsed = Self.sanitizedURL(from: url) else {
            NSLog("[SupercliNative] refusing malformed/unsupported url: \(url)")
            return
        }
        NSLog("[SupercliNative] open url \(parsed.absoluteString)")
        if !NSWorkspace.shared.open(parsed) {
            NSLog("[SupercliNative] NSWorkspace failed to open \(parsed.absoluteString)")
        }
    }
...
/// - it has no command or working directory and cannot launch `supercli-attach`;
...
final class RemoteGhosttyTerminalPane: NSView {
    ...
    init(
        ...
        if let issue = controller.lastConfigurationIssue {
            NSLog("[SupercliNative] remote ghostty configuration issue: %@", issue)
        }
    ...
    var dropStabilizeHome: URL = LaunchConfig.supercliDir
