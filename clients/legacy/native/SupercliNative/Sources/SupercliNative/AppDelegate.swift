...
final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, SPUUpdaterDelegate,
    ...
    private var store: SupercliStore?
    ...
    func applicationDidFinishLaunching(_: Notification) {
        ...
        // One instance per SUPERCLI_HOME: two processes on the same state dir
        // would fight over sessions, ports, and pairing. Identity-verified
        // pidfile, so a stale file never blocks a launch.
        if SupercliWorkspaceLauncher.otherInstanceOwnsCurrentHome() {
            let alert = NSAlert()
            alert.messageText = "This workspace is already running"
            alert.informativeText =
                "Another Supercli instance is already using this workspace's data. Switch to it instead."
            alert.runModal()
            NSApp.terminate(nil)
            return
        }
        SupercliWorkspaceLauncher.writeOwnPidFile()
        ...
        let store = SupercliStore(
            paneLayoutController: paneLayoutController,
            deferInitialScan: !ProcessInfo.processInfo.environment.keys.contains {
                $0.hasPrefix("SUPERCLI_TEST_") || $0.hasPrefix("SUPERCLI_SNAPSHOT")
            }
        )
        self.store = store

        // Finder "New Supercli Session Here" service (NSServices in Info.plist):
        // macOS delivers the right-clicked folder(s) to `newSupercliSession`.
        ...
        // ~/.supercli/app-ports so broadcasts and ownership probes can find it.
        ...
        if let server = HookServer() {
            ...
            NSLog("[SupercliNative] loopback listener failed to start; platform capabilities unavailable")
        }
        ...
        Task { @MainActor [weak store] in
            ...
            if SupercliFeatureFlags.mobileRemoteControlEnabled {
        ...
        // the hints). Installed here, not in SupercliStore.init, so the
        ...
        installMainMenu()
        // A service launch (SUPERCLI_LAUNCH_HIDDEN=1 — a peer instance started
        ...
        if ProcessInfo.processInfo.environment["SUPERCLI_LAUNCH_HIDDEN"] != "1" {
            showMainWindow()
        }
    ...
    func showMainWindow() {
        ...
        // store as the rest of the app, so a dev/blank SUPERCLI_HOME instance is
        // isolated here too.
        ...
        // Appearance is app-driven: SupercliStore.init already applied the
    ...
    private func installMainMenu() {
        ...
        // current project (SupercliStore.launchDefaultSession). Deliberately no
    ...
    /// `NSMessage` target for the "New Supercli Session Here" Finder service
    ...
    @objc func newSupercliSession(
        _ pboard: NSPasteboard,
        userData _: String?,
        error _: AutoreleasingUnsafeMutablePointer<NSString>?
    ) {
        // Finder "New Supercli Session Here" files a folder ON this Mac — valid
    ...
    @objc private func takeScreenshotFromMenu() {
        NotificationCenter.default.post(name: .supercliTakeSessionScreenshot, object: nil)
    }

    @objc private func findInTerminalFromMenu() {
        NotificationCenter.default.post(name: .supercliTerminalFind, object: nil)
    }

    @objc private func findNextInTerminalFromMenu() {
        NotificationCenter.default.post(name: .supercliTerminalFindNext, object: nil)
    }

    @objc private func findPreviousInTerminalFromMenu() {
        NotificationCenter.default.post(name: .supercliTerminalFindPrevious, object: nil)
    }
    ...
    private static var licenseHeadersForUpdateFeed: [String: String]? {
        ...
        headers["X-Supercli-Install-ID"] = installID
        if let version = Bundle.main.object(forInfoDictionaryKey: "CFBundleShortVersionString") as? String {
            headers["X-Supercli-App-Version"] = version
        }
        return headers
    }
    ...
    private static let installIDKey = "supercli.native.installID"
    ...
    private static var sparkleCanStart: Bool {
        ...
        guard SupercliWorkspaceContext.isDefaultInstance else { return false }
    ...
    private func hideSystemTitlebarBackground() {
        ...
        if ProcessInfo.processInfo.environment["SUPERCLI_DEBUG_TITLEBAR"] == "1" {
    ...
    /// Clean shutdown: drop our port from ~/.supercli/app-ports so hook
    ...
    func applicationWillTerminate(_: Notification) {
        ...
        SupercliWorkspaceLauncher.removeOwnPidFile()
    }
}
