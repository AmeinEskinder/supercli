...
//  SupercliShared
//
//  Wire protocol + end-to-end crypto for Supercli Remote — the Cloudflare
    ...
    public init(
        ...
        func derive(_ info: String) -> SymmetricKey {
            HKDF<SHA256>.deriveKey(
                ...
                salt: salt,
                info: Data("supercli-relay-v\(RelayProtocol.version):\(info)".utf8),
                outputByteCount: 32
            )
        }
...
public enum RelayHandshake {
    ...
    public static func transcriptMAC(
        ...
        let macKey = HKDF<SHA256>.deriveKey(
            ...
            info: Data("supercli-relay-v\(RelayProtocol.version):handshake-mac".utf8),
            outputByteCount: 32
        )
        var transcript = Data()
