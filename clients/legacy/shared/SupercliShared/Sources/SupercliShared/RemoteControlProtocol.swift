...
public enum RemoteControlProtocol {
    ...
    /// self-signed certificate the `supercli-host __remote__` WSS server pins
...
public enum RemotePairingCode {
    private static let prefix = "SUPERCLI"
...
public enum RemotePairingCrypto {
    ...
    private static func deriveKey(token: String, salt: Data, direction: Direction) -> SymmetricKey {
        HKDF<SHA256>.deriveKey(
            ...
            salt: salt,
            info: Data("supercli-pairing-v1:\(direction.rawValue)".utf8),
            outputByteCount: 32
        )
    }

    private static func associatedData(macID: String, endpoint: URL, direction: Direction) -> Data {
        Data("supercli-pairing-v1\u{0}\(direction.rawValue)\u{0}\(macID)\u{0}\(endpoint.absoluteString)".utf8)
    }
}
...
public struct RemotePairingResponse: Codable, Equatable, Sendable {
    ...
    /// TCP port of the Mac's `supercli-host __remote__` TLS server (WSS
    ...
    /// Supercli Remote (relay) credentials: per-device E2E key + relay token,
...
public struct RemoteSessionSummary: Codable, Equatable, Identifiable, Sendable {
    ...
    /// runtime's Supercli integration) or `"screen"` (the Host's screen
    ...
    /// Installed Supercli App identity, resolved by the Host against ITS
...
public struct RemoteBootstrapSnapshot: Codable, Equatable, Sendable {
    ...
    /// Current TCP port of the Mac's `supercli-host __remote__` TLS server
    ...
    /// restarts (the cert persists in `~/.supercli/remote/tls/`).
    public let remoteServerCertificateFingerprint: String?
    ...
    /// Whether this Mac has an active Supercli Pro entitlement
    ...
    /// each registered `supercli --workspace` instance), so a connected phone can
...
public struct RemoteAgentSummary: Codable, Equatable, Identifiable, Sendable {
    ...
    /// The runtime ships an Supercli integration the Host can install.
