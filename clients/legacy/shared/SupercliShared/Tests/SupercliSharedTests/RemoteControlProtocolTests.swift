import XCTest
@testable import SupercliShared
...
final class RemoteControlProtocolTests: XCTestCase {
    ...
    func testAppIconsResolveFromTheHostCatalogByIDAndByCommand() throws {
        ...
        SupercliAppIconCatalog.update([app])
        defer { SupercliAppIconCatalog.update([]) }

        let byID = SupercliToolIcon.resolving(appID: "supercli.app.markdown", providerID: nil, command: "")
        ...
        let byCommand = SupercliToolIcon.resolving(
            providerID: nil,
            command: "'/Users/me/.supercli/apps/bin/supercli-markdown' '/tmp/README.md'"
        )
        XCTAssertEqual(byCommand.id, byID.id)
        XCTAssertEqual(SupercliToolIcon.resolving(providerID: nil, command: "supercli-markdown").id, byID.id)

        // Runtime art still wins, and an unknown command stays the terminal.
        XCTAssertEqual(SupercliToolIcon.resolving(providerID: nil, command: "claude").kind, .agent)
        XCTAssertEqual(SupercliToolIcon.resolving(providerID: nil, command: "supercli-nope").id, "terminal")
    }

    func testAppSummaryDefaultsNewHandlerFieldsFromOlderHosts() throws {
        let json = #"{"id":"supercli.app.markdown","name":"Markdown","command":"supercli-markdown","mediaTypes":["text/markdown"],"installed":true}"#
    ...
    func testProjectGroupFieldsRoundTripAndRemainAdditive() throws {
        let group = RemoteProjectSummary(
            ...
            name: "Research",
            path: "/dev/supercli",
            parentProjectID: "project-supercli",
            isGroup: true,
            colorID: "violet",
            pinned: true
        )
        ...
        let legacyJSON = #"{"id":"project-supercli","name":"Supercli","path":"/dev/supercli","mcpBlocked":false}"#
        let legacy = try JSONDecoder().decode(
    ...
    func testCompactPairingCodeRoundTrip() throws {
        ...
        XCTAssertEqual(code, "SUPERCLI:1:192.168.1.20:49152:MAC-1:K7ZP2Q4RSTUVWXYZABCDEFGH23:1789996900")
    ...
    func testControllerAssistedPairingCodeRoundTrip() throws {
        ...
        XCTAssertEqual(
            code,
            "SUPERCLI:1:192.168.1.20:49152:HOST-1:K7ZP2Q4RSTUVWXYZABCDEFGH23:1789996900:INVITE-123"
        )
        let decoded = try XCTUnwrap(RemotePairingCode.decode(code))
        XCTAssertEqual(decoded.endpoint, payload.endpoint)
        XCTAssertEqual(decoded.macID, payload.macID)
    ...
    func testPairingCodeDecodeRejectsGarbage() {
        ...
        XCTAssertNil(RemotePairingCode.decode("SUPERCLI:1:192.168.1.20:notaport:TOKEN:123"))
        XCTAssertNil(RemotePairingCode.decode("SUPERCLI:1:192.168.1.20:49152:TOKEN"))
    ...
    func testCreateSessionRequestRoundTripsInitialPrompt() throws {
        let request = RemoteCreateSessionRequest(
            projectID: "project-1",
            presetID: "claude",
            worktreePath: "/tmp/supercli-worktree",
    ...
    /// SUPERCLI_PROTOCOL_DIR points at another copy (a server checkout's
    /// `protocol/`, or an extracted archive).
    private func fixtureJSON(_ name: String) throws -> [String: Any] {
        if let override = ProcessInfo.processInfo.environment["SUPERCLI_PROTOCOL_DIR"] {
        ...
        XCTFail("could not locate protocol/\(name) from \(#filePath) — set SUPERCLI_PROTOCOL_DIR at a protocol/ directory")
        return [:]
    }
}
