...
struct PluginProtocolTests {
    ...
    @Test func pluginOrderPatchAndCommandIdentityRoundTrip() throws {
        let patch = RemoteWorkspaceSettingsPatch(pluginOrder: ["codex", "supercli.app.markdown", "claude"])
    ...
    @Test func appInstallerKeepsTheRemoteHostsAbsoluteCommand() throws {
        let app = RemoteAppSummary(id: "supercli.app.markdown", name: "Markdown", command: "supercli-markdown",
                                   installCommand: "'/opt/supercli/bin/supercli' apps install 'supercli.app.markdown' --yes")
    ...
    @Test func activationPatchChangesOnePluginWithoutReplacingOtherSettings() throws {
        let patch = RemoteWorkspaceSettingsPatch(pluginActivation: .init(id: "supercli.app.markdown", active: false))
        #expect(!patch.isEmpty)
        let object = try #require(JSONSerialization.jsonObject(with: JSONEncoder().encode(patch)) as? [String: Any])
        #expect(object.count == 1)
        let activation = try #require(object["pluginActivation"] as? [String: Any])
        #expect(activation["id"] as? String == "supercli.app.markdown")
