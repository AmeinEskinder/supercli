...
final class RuntimeCatalogTests: XCTestCase {
    private func runtimeFixture(
        slug: String,
        platform: SupercliRuntimePlatform,
        ...
        SupercliRuntimeMetadata(
            ...
            usageStores: [],
            suggestedPresets: [
                SupercliRuntimeSuggestedPreset(
                    id: "\(slug)-default",
                    label: slug,
                    command: slug,
                    quickLaunch: true
    ...
    func testGeneratedCatalogPreservesLegacyOrderAndStableIdentity() throws {
        let orders = SupercliRuntimeCatalog.runtimes.compactMap(\.legacyOrder)
        XCTAssertEqual(orders, Array(0 ..< orders.count))

        let claude = try XCTUnwrap(
            SupercliRuntimeCatalog.runtime(id: "com.anthropic.claude-code")
        )
        ...
        for runtime in SupercliRuntimeCatalog.runtimes where runtime.kind == .agent {
            XCTAssertEqual(
        ...
        let kimi = try XCTUnwrap(SupercliRuntimeCatalog.runtime(id: "kimi"))
    ...
    func testCommandLookupUsesDescriptorAliasesAndExecutableBasename() {
        XCTAssertEqual(
            SupercliRuntimeCatalog.runtime(command: "/opt/homebrew/bin/codex --full-auto")?.stableID,
            "com.openai.codex"
        )
        XCTAssertNil(SupercliRuntimeCatalog.runtime(command: "npm run dev"))
    }
    ...
    func testPlatformFilteringUsesMacOSAndLinuxDescriptorFixtures() {
        ...
        XCTAssertEqual(
            SupercliRuntimeCatalog.runtimes(for: .macos, from: fixtures).map(\.slug),
            ["macos-only"]
        )
        XCTAssertEqual(
            SupercliRuntimeCatalog.runtimes(for: .linux, from: fixtures).map(\.slug),
            ["linux-only"]
        )
        ...
        XCTAssertEqual(SupercliToolIcon.forRuntime(linux).id, "com.example.linux-only")
    }

    func testSharedIconResolutionPrefersHostProviderIdentity() {
        let icon = SupercliToolIcon.resolving(
        ...
        XCTAssertEqual(
            SupercliToolIcon.resolving(providerID: nil, command: "unknown-agent"),
            .terminal
        )
    }

    func testGenericFallbackFollowsRuntimeKindNotAgentSparkle() {
        let editor = SupercliToolIcon.forRuntime(
            runtimeFixture(slug: "markdown", platform: .macos, kind: .editor)
        )
        let agent = SupercliToolIcon.forRuntime(
    ...
    func testEveryRuntimeDescriptorResolvesAnEmbeddedOrGenericIcon() throws {
        for runtime in SupercliRuntimeCatalog.runtimes {
            let icon = SupercliToolIcon.forRuntime(runtime)
        ...
        let copilot = try XCTUnwrap(SupercliRuntimeCatalog.runtime(id: "com.github.copilot-cli"))
        XCTAssertNil(copilot.iconSVG)
        XCTAssertFalse(SupercliToolIcon.forRuntime(copilot).usesRuntimeAsset)

        let openCode = try XCTUnwrap(SupercliRuntimeCatalog.runtime(id: "ai.opencode.cli"))
        XCTAssertNotNil(openCode.iconSVG)
        XCTAssertFalse(openCode.iconIsTemplate)
        XCTAssertFalse(SupercliToolIcon.forRuntime(openCode).isTemplate)
    }
}
