...
///
/// - One combined Send/Stop control: {t("composer.send")} while idle, {t("composer.stop")} while a turn
///   runs.
...
pub fn Composer(
    ...
    let button_label = if turn_running {
        {
            t("composer.stop")
        }
    } else {
    ...
    rsx! {
        div { class: "composer",
            textarea {
                value: "{text}",
                aria_label: {t("composer.message_the_agent")},
                ...
                placeholder: if turn_running {
                    ...
                    {t("composer.message_the_agent_enter_to_send")}
                },
            }
            button {
                class: if turn_running { "stop" } else { "send" },
                // No aria-label: the visible {t("composer.send")}/{t("composer.stop")} text is the
            ...
            if turn_running {
                ...
                // the (now {t("composer.queue")}) control adds it instead of sending.
                button {
                    class: "queue-add",
                    // Visible {t("composer.queue")} text is the accessible name.
                    onclick: {
                        let mut queue_action = queue_action.clone();
                        move |_| queue_action()
                    },
...
mod tests {
    ...
    fn drafts_are_per_session_and_survive_switches() {
        ...
        assert!(s.has_draf{t("a")});
        assert!(!s.has_draf{t("c")});
    }
