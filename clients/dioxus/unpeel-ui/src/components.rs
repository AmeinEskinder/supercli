    ...
    rsx! {
        div { class: "connection-bar",
            ...
            span { class: "host-name",
                {host_name.unwrap_or_else(|| {t("components.no_host")})}
            }
        ...
        div { class: "session-list", "data-testid": "session-list",
            ...
            if archived_count > 0 {
                button {
                    ...
                    if show { {t("components.hide_archived")} } else { "Show archived ({archived_count})" }
                }
            }
        }
    }
            ...
            for e in entries {
                {
                    ...
                    rsx! {
                        div {
                            key: "{e.id}",
                            class: if mine { {t("components.bubble_mine")} } else { {t("components.bubble_theirs")} },
                            pre { "{text}" }
                        }
                    }
                }
...
pub fn ApprovalCard(
    ...
    let title = approval
        .title
        .clone()
        .unwrap_or_else(|| { t("components.approval_requested") }.to_string());
    ...
    let state_text: Option<String> = match answer_state {
        ...
        Some(AnswerUiState::RateLimited { retry_in_secs }) => {
            Some({ t("approval.rate_limited") }.replace("{secs}", &retry_in_secs.to_string()))
        }
        ...
        Some(AnswerUiState::ResolvedUnknown) => {
            Some({ t("approval.resolved_unknown") }.to_string())
        }
        None => None,
    };
    rsx! {
