...
pub fn mcp_policy_sections() -> [McpPolicySection; 3] {
    [
        McpPolicySection {
            ...
            rows: vec![
                McpPolicyRow { id: "agents.enabled".into(), title: "Allow agent MCP access".into(), summary: "Sessions can call tools on approved MCP servers.".into(), enabled: true },
                McpPolicyRow { id: "agents.askOnWrite".into(), title: "Ask before writes".into(), summary: "An agent must ask before a tool writes outside its own session.".into(), enabled: true },
            ],
        },
        McpPolicySection {
            id: "browser".into(),
            ...
            rows: vec![
                McpPolicyRow { id: "browser.enabled".into(), title: "Allow browser use".into(), summary: "New sessions launch with the browser domain advertised.".into(), enabled: false },
                McpPolicyRow { id: "browser.persistLogin".into(), title: "Persist logins".into(), summary: "Keep the session browser's login state between runs.".into(), enabled: false },
            ],
        },
        McpPolicySection {
            id: "sessions".into(),
            title: "Sessions use".into(),
            summary: "Agent sessions can read other sessions and request write access to explicit targets.".into(),
            rows: vec![
                McpPolicyRow { id: "sessions.readAll".into(), title: "Read other sessions".into(), summary: "Sessions can read every other session.".into(), enabled: true },
                McpPolicyRow { id: "sessions.askOnWrite".into(), title: "Ask before writing to another session".into(), summary: "Unless that pair was already approved.".into(), enabled: true },
            ],
        },
    ]
}
                        ...
                        for (feature, enabled) in features {
                            div {
                                ...
                                div { class: "feature-text",
                                    ...
                                    if feature.experimental { span { class: "feature-badge", "Experimental" } }
                                }
                                input {
                                    r#type: "checkbox",
                                    checked: enabled,
