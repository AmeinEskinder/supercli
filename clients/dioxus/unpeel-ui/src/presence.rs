...
//!   "kind":"ws"|"poll","device":"Name (id)"|null,"last_seen":ms}]}}`.
...
/// so the launcher can toast them (Swift shows "<name> connected").
#[derive(Debug, Clone)]
...
impl PresenceStore {
    ...
    /// "no viewers". Returns display names of genuinely new arrivals for
