...
impl SessionSheetAction {
    ...
    pub fn confirmation_title(self, session_running: bool) -> Option<&'static str> {
        match self {
            SessionSheetAction::Restart => Some("Resume Session?"),
            SessionSheetAction::ResumeAgent => Some("Resume Agent?"),
            SessionSheetAction::Stop => Some("Stop Session?"),
            SessionSheetAction::Archive => Some("Stop and Archive Session?"),
...
pub fn SessionOrganizeSheet(
    session: SessionSummary,
    host_protocol: Option<HostProtocolDescriptor>,
    /// All projects from the Host's bootstrap; the "Move to" filing
        ...
        div { class: "sheet-backdrop", onclick: move |_| on_close.call(()),
            div { class: "sheet organize-sheet", onclick: move |e| e.stop_propagation(),
                div { class: "sheet-header",
                    span { class: "sheet-title", "Organize session" }
                    button { class: "sheet-close", onclick: move |_| on_close.call(()), "Done" }
                }
                div { class: "sheet-body",
                    label { class: "field-label", "Session name" }
                    input {
                        class: "text-field",
                        value: "{title}",
                    ...
                    button { class: "primary-button", onclick: do_save, "Save" }
                    if can_move && !destinations.is_empty() {
                        label { class: "field-label", "Move to project" }
                        select {
                            class: "text-field",
                            onchange: move |e| {
                                let v = e.value();
                            ...
                            option { value: "", "Keep current location" }
            ...
            div { class: "sheet organize-sheet", onclick: move |e| e.stop_propagation(),
                ...
                div { class: "sheet-body",
                    if target_is_group {
                        ...
                        label { class: "field-label", "Folder color" }
