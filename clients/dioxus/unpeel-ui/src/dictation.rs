...
/// recording{t("dictation.and")}stopped an empty recording", so the view sent the
...
impl DictationSession {
    ...
    pub fn backend_interrupted(&mut self) {
        if self.phase == DictationPhase::Recording {
            self.phase = DictationPhase::Idle;
            self.error = Some({ t("dictation.dictation_was_interrupted") }.to_string());
        }
    }
                ...
                if let Some(rest) = msg.strip_prefix("dictation:") {
                    if let Some(rest) = rest.strip_prefix("text:") {
                        ...
                        match rest {
                            ...
                            "unavailable" => session.write().note_error({
                                t("dictation.speech_recognition_isn_t_available_here")
                            }),
                            ...
                            r if r.starts_with("error:") => {
                                ...
                                let message = if kind == "microphone" {
                                    {
                                        t("dictation.microphone_unavailable_check_permission")
                                    }
                                } else {
                                    {
                                        t("dictation.speech_recognizer_failed")
                                    }
                                };
    ...
    rsx! {
        div { class: "dictation-wrap", "data-testid": "dictation-wrap",
            button {
                ...
                onclick: on_mic,
                aria_label: if phase == DictationPhase::Recording { {t("dictation.stop_dictation")} } else { {t("dictation.start_dictation")} },
                title: if phase == DictationPhase::Recording { {t("dictation.stop_dictation")} } else { {t("dictation.dictate")} },
                "🎤"
            }
            if show_pill {
                div { class: "dictation-pill", "data-testid": "dictation-status",
                    ...
                    if phase == DictationPhase::Recording {
                        button { class: "dictation-stop", onclick: on_mic, {t("dictation.stop")} }
                    } else if !session.read().transcript().is_empty() && error.is_some() {
                        button { class: "dictation-paste", onclick: on_paste_kept, {t("dictation.paste")} }
                    }
                    button { class: "dictation-cancel", onclick: on_cancel, aria_label: {t("dictation.dismiss_dictation")}, "✕" }
                }
            }
        }
    }
