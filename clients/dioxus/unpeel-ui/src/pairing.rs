...
pub fn PairingView(
    ...
    /// Ids of Hosts with a live connection. Their rows offer {t("pairing.switch")}
    /// instead of {t("pairing.connect")}: switching never tears the connection down.
        ...
        div { class: "pairing", "data-testid": "pairing-view",
            ...
            div { class: "actions",
                button {
                    ...
                    if working { "Pairing…" } else { {t("pairing.pair")} }
                }
            }
            ...
            if !hosts.is_empty() {
                h3 { {t("pairing.previously_paired")} }
                for host in hosts {
                    {
                        ...
                        rsx! {
                            div { class: "host-row", key: "{host.host_id}",
                                div { class: "host-info",
                                    ...
                                    if is_active {
                                        span { class: "host-badge", {t("pairing.viewing")} }
                                ...
                                div { class: "host-actions",
                                    button {
                                        ...
                                        if is_active { {t("pairing.viewing_2")} } else if is_connected { {t("pairing.switch")} } else { {t("pairing.connect")} }
                                    }
                                    button {
                                        class: "danger",
