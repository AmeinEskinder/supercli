                ...
                if let Some(rest) = msg.strip_prefix("ctl:") {
                    match rest {
                        ...
                        r if r.starts_with("denied:") => {
                            let reason = r.strip_prefix("denied:").unwrap_or("camera unavailable");
        ...
        div { class: "qr-scanner",
            ...
            match *state.read() {
                QrScanState::RequestingPermission => rsx! {
                    div { class: "qr-status", "Requesting camera access…" }
                },
                ...
                QrScanState::Paused => rsx! {
                    div { class: "qr-status", "Scanner paused" }
                },
                _ => rsx! {
                    div { class: "qr-status", "Point the camera at the pairing QR code" }
                },
            }
        }
    }
