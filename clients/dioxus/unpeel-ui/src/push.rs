...
impl PushRegistrationState {
    ...
    pub fn diagnostic_label(&self) -> String {
        match self {
            PushRegistrationState::NotRequested => { t("push.not_requested") }.to_string(),
            PushRegistrationState::RequestingPermission => {
                { t("push.waiting_for_notification_permission") }.to_string()
            }
            PushRegistrationState::PermissionDenied => {
                { t("push.notifications_are_denied_in_system_setti") }.to_string()
            }
            PushRegistrationState::Registering => {
                { t("push.waiting_for_a_push_device_token") }.to_string()
            }
            PushRegistrationState::Registered { environment } => {
                if environment == "production" {
                    { t("push.ready_production") }.to_string()
                } else {
                    { t("push.ready_sandbox") }.to_string()
                }
            }
    ...
    pub fn sidebar_warning(&self) -> Option<String> {
        match self {
            PushRegistrationState::PermissionDenied => Some({ t("push.notifications_are_off") }),
            PushRegistrationState::Failed { .. } => {
                Some({ t("push.notifications_aren_t_working") })
            }
            _ => None,
        }
    }
