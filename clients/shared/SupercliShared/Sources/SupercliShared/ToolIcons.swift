...
public struct SupercliToolIcon: Equatable, Hashable, Identifiable, Sendable, CaseIterable {
    public let id: String
    public let key: String
    public let label: String
    public let kind: SupercliRuntimeKind
    public let svgSource: String
    public let isTemplate: Bool
    public let fallbackSystemName: String
    public let usesRuntimeAsset: Bool

    public static var allCases: [SupercliToolIcon] {
        SupercliRuntimeCatalog.runtimes.map(forRuntime) + [.terminal]
    }
    ...
    public static func forApp(id: String, name: String, iconSVG: String?) -> SupercliToolIcon {
    ...
    public static func forRuntime(_ runtime: SupercliRuntimeMetadata) -> SupercliToolIcon {
    ...
    public static let terminal = SupercliToolIcon(
        id: "terminal",
        key: "terminal",
        label: "Terminal",
        kind: .terminal,
    ...
    private static func genericSVG(for kind: SupercliRuntimeKind) -> String {
        switch kind {
        case .agent:
    ...
    private static func fallbackSystemName(for kind: SupercliRuntimeKind) -> String {
        switch kind {
...
public enum SupercliAppIconCatalog {
    private static let lock = NSLock()
    nonisolated(unsafe) private static var byAppID: [String: SupercliToolIcon] = [:]
    nonisolated(unsafe) private static var byBinary: [String: SupercliToolIcon] = [:]

    public static func update(_ apps: [RemoteAppSummary]) {
        var nextByID: [String: SupercliToolIcon] = [:]
        var nextByBinary: [String: SupercliToolIcon] = [:]
        for app in apps {
            let icon = SupercliToolIcon.forApp(id: app.id, name: app.name, iconSVG: app.iconSvg)
    ...
    public static func icon(appID: String?) -> SupercliToolIcon? {
    ...
    public static func icon(command: String) -> SupercliToolIcon? {
