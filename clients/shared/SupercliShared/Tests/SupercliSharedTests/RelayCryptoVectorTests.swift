...
///
/// Set SUPERCLI_EMIT_RELAY_KAT=1 to print the current vectors as JSON (used to
...
final class RelayCryptoVectorTests: XCTestCase {
    ...
    func testTranscriptMACKnownAnswer() {
        ...
        // fixed inputs above; regenerate with SUPERCLI_EMIT_RELAY_KAT=1 only on
    ...
    func testEmitVectorsWhenRequested() throws {
        guard ProcessInfo.processInfo.environment["SUPERCLI_EMIT_RELAY_KAT"] == "1" else {
            throw XCTSkip("set SUPERCLI_EMIT_RELAY_KAT=1 to emit vectors")
        }
        let mac = RelayHandshake.transcriptMAC(
    ...
    // SUPERCLI_EMIT_RELAY_KAT=1 on an intentional change.
