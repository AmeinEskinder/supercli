...
//  (`supercli serve`) and answer 404 here. This listener:
//
//  - listens on 127.0.0.1 with an OS-assigned port (port 0),
//  - registers that port in ~/.supercli/app-ports on startup and removes it
//    on clean shutdown (peers and the worker's ownership probe find it
//    there; every response carries `X-Supercli-Controller-Owner: serve`),
//  - serves the worker's authenticated platform-adapter callback
//    (`/_supercli/platform-adapter/call`, per-process bearer registered over
//    `host.sock`),
...
final class HookServer: @unchecked Sendable {
    ...
    /// Handler for `/state-changed`: another Supercli wrote shared state and
    ...
    private static let debug = ProcessInfo.processInfo.environment["SUPERCLI_DEBUG"] == "1"

    /// Binds 127.0.0.1:0, starts the accept loop, and registers the
    /// assigned port in ~/.supercli/app-ports. Returns nil if the socket
    ...
    init?() {
        ...
        thread.name = "supercli.hook-server"
        thread.start()
        acceptThread = thread

        Self.registerPort(port)
        NSLog("[SupercliNative] loopback listener on 127.0.0.1:%d", Int(port))
    }

    /// Closes the listener and removes our port from ~/.supercli/app-ports.
    ...
    func stop() {
        ...
        if port != 0 {
            ...
            NSLog("[SupercliNative] hook server stopped, port %d unregistered", Int(port))
            port = 0
        }
    }
    ...
    private func handleRequest(
        ...
        if path == "/_supercli/platform-adapter/call" {
        ...
        // (supercli-core state_bus). The TUI, the CLI and this app are peers
    ...
    /// the mobile routes) answers 404: `supercli serve` owns them.
    static func shouldDispatch(_ path: String) -> Bool {
        switch path {
        case "/_supercli/platform-adapter/call",
             "/state-changed",
             "/show-window",
             "/reload-appearance":
            return true
    ...
    /// Decode the bounded callback envelope emitted by `supercli serve`.
    ...
    private func respond(_ fd: Int32, status: Int, body: String) {
        ...
        let payload =
            ...
            + "X-Supercli-Controller-Owner: \(LocalHostClientFeature.controllerOwnerHeaderValue)\r\n"
    ...
    // MARK: - Port registry (~/.supercli/app-ports)

    static var portRegistryURL: URL {
        LaunchConfig.supercliDir.appendingPathComponent("app-ports")
    }

    private static var portRegistryLockURL: URL {
        LaunchConfig.supercliDir.appendingPathComponent("app-ports.lock")
    }
    ...
    // MARK: - Trace log (~/.supercli/hooks/trace.log, shared with Tauri)
