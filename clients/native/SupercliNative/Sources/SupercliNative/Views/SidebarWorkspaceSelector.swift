...
//  registry workspaces from ~/.supercli/profiles.json,
...
//  and SSH rows remain behind RemoteHostFeature.pickerEnabled (Supercli Dev).
//
...
struct WorkspacePickerPanel: View {
    ...
    var body: some View {
        pickerContent
            ...
            // unlike `supercliAppTintChanged`, which repeats through the fade.
            .onReceive(
    ...
    /// its own app instance (`SupercliWorkspaceLauncher`, the tear-out seam's
    ...
    private func openInNewWindowAction(_ row: WorkspaceListRowModel) -> (() -> Void)? {
        ...
        let launchRecord: SupercliWorkspaceRecord
        if let record {
            launchRecord = record
        } else if isDefault {
            // The default workspace has no registry record; the launcher
            // only needs its home and a display name.
            launchRecord = SupercliWorkspaceRecord(
                id: "default",
                name: row.name,
                home: SupercliWorkspaceRegistry.realSupercliDir.path,
                createdAt: 0
            )
        } else {
            return nil
        ...
        return {
            ...
            if SupercliWorkspaceLauncher.runningPid(home: home) != nil {
                SupercliWorkspaceLauncher.showWindow(home: home)
                dismiss()
                return
            }
            do {
                try SupercliWorkspaceLauncher.launch(launchRecord)
                dismiss()
            } catch {
    ...
    private func commitRename(_ target: WorkspaceRenameTarget) {
        ...
        case .defaultWorkspace:
            renamed = SupercliWorkspaceContext.renameDefaultWorkspace(to: name)
        case .local(let id):
            SupercliWorkspaceRegistry.rename(id: id, to: name)
            renamed = true
