...
struct RemoteHostPairingSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            if store.remoteHostPairingCompleted {
                ...
                HStack(alignment: .top, spacing: 18) {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        Text("Scan this code in Supercli on the phone.")
                            .font(.system(size: 13, weight: .semibold))
                        Text(
            ...
            Text("After pairing, the phone connects to \(hostName) itself—Direct when reachable, otherwise through Supercli Link if enabled.")
                .font(.system(size: 11))
                .foregroundStyle(Theme.mutedForeground)
        }
        .padding(24)
...
struct AddWorkspaceSheet: View {
    ...
    init(store: SupercliStore, hosts: RemoteHostStore, initialMethod: Method) {
        self.store = store
        self.hosts = hosts
    ...
    private var pairingContent: some View {
                ...
                HStack(spacing: 8) {
                    ...
                    Text(
                        "Workspaces on your network appear here. On the other machine, "
                            + "run `supercli serve` (or open Supercli)."
                    )
                        .foregroundStyle(Theme.mutedForeground)
                }
                .frame(maxWidth: .infinity, minHeight: 54, alignment: .leading)
        ...
        VStack(alignment: .leading, spacing: 8) {
            Text("Pairing code")
                .font(.system(size: 12, weight: .semibold))
            Text(
                "On the other machine, run `supercli pair` — or `supercli pair --serve` "
                    + "to start serving at the same time — then enter the code it shows here. "
                    + "For another Mac running the Supercli app, its code is in "
                    + "Settings ▸ Remote Control."
            )
                .font(.system(size: 11))
                .foregroundStyle(Theme.mutedForeground)
            TextField("SUPERCLI:1:…", text: $pairingCode)
                .textFieldStyle(.roundedBorder)
    ...
    private var sshContent: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Supercli uses your system SSH configuration, keys, agent, host-key policy, ProxyJump, and VPN. Put ports and identity files in ~/.ssh/config.")
                .font(.system(size: 10))
                .foregroundStyle(Theme.mutedForeground)
                .fixedSize(horizontal: false, vertical: true)
        }
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            SecureField("Leave empty for SSH keys or agent", text: $sshSecret)
                .textFieldStyle(.roundedBorder)
            Text("Saved in this Mac’s Keychain so Supercli can reopen this workspace. It is never stored on the remote machine or passed in command arguments.")
                .font(.system(size: 10))
                .foregroundStyle(Theme.mutedForeground)
        }
        ...
        VStack(alignment: .leading, spacing: 7) {
            ...
            Text(sshInstallSuggested
                ? "Supercli could not be started. Install it with the same SSH connection, or copy the command as a fallback:"
                : "If Supercli is missing, Connect will offer to install it. Manual fallback:")
                .font(.system(size: 10))
                .foregroundStyle(Theme.mutedForeground)
            HStack {
        ...
        HStack {
            Text(
                "Supports ordinary SSH and interactive-only managed shells automatically.\n"
                    + "No service needs to run on the box — Supercli connects over SSH on demand. "
                    + "For phone access and notifications while disconnected, also run "
                    + "`supercli serve` there."
            )
                .font(.system(size: 10))
                .foregroundStyle(Theme.mutedForeground)
            Spacer()
    ...
    private func createLocalWorkspace() {
        ...
        errorMessage = nil
        do {
            let record = try SupercliWorkspaceRegistry.create(name: name)
            if workspaceTint != .none {
                ...
                AppDefaults.suite(forSupercliHome: record.home)
                    .set(workspaceTint.rawValue, forKey: SupercliStore.nativeAppTintKey)
                NotificationCenter.default.post(
                    name: .supercliWorkspaceTintChanged, object: nil
                )
            }
            NotificationCenter.default.post(
                name: .supercliWorkspaceListChanged,
                object: nil
            )
        ...
        operationTask = Task { @MainActor in
            ...
            do {
                ...
                if case .connection = error {
                    ...
                    errorMessage = "Supercli is not available on this SSH Host yet, or could not be started. Install it and connect again."
                } else {
                    errorMessage = error.localizedDescription
                }
            } catch {
