...
struct WorkspacesSettingsPanel: View {
    ...
    /// Opens Settings ▸ Remote (home of the Supercli Link section since the
    ...
    @State private var removalCandidate: SupercliWorkspaceRecord?
    ...
    private enum LocalRenameTarget: Identifiable, Equatable {
        ...
        case record(SupercliWorkspaceRecord)
    ...
    var body: some View {
        ...
        .alert(
            ...
            Button("Rename") {
                switch target {
                case .defaultWorkspace:
                    SupercliWorkspaceContext.renameDefaultWorkspace(to: renameText)
                case .record(let record):
                    SupercliWorkspaceRegistry.rename(id: record.id, to: renameText)
                case .remote(let id, _):
                    _ = store.remoteHostStore.renameHost(id, to: renameText)
                }
                refresh()
                NotificationCenter.default.post(name: .supercliWorkspaceListChanged, object: nil)
            }
        ...
        .confirmationDialog(
            ...
            Button("Remove from list", role: .destructive) {
                SupercliWorkspaceRegistry.remove(id: workspace.id, deleteData: false)
                refresh()
                NotificationCenter.default.post(name: .supercliWorkspaceListChanged, object: nil)
            }
            Button("Remove and delete its data", role: .destructive) {
                SupercliWorkspaceRegistry.remove(id: workspace.id, deleteData: true)
                refresh()
                NotificationCenter.default.post(name: .supercliWorkspaceListChanged, object: nil)
            }
            Button("Cancel", role: .cancel) {}
    ...
    private func setRowTint(_ tint: AppTint, row: WorkspaceListRowModel) {
        ...
        case let .local(record, isDefault, isCurrentInstance, _):
            if isCurrentInstance {
                ...
                AppDefaults.suite(forSupercliHome: record.home)
                    .set(tint.rawValue, forKey: SupercliStore.nativeAppTintKey)
                ...
                UserDefaults.standard
                    .set(tint.rawValue, forKey: SupercliStore.nativeAppTintKey)
                Self.pingReloadAppearance(
                    homeDir: SupercliWorkspaceRegistry.realSupercliDir
                )
            }
        case .paired(let host):
    ...
    static func pingReloadAppearance(homeDir: URL) {
        ...
        SupercliStore.announceStateChange(
            "native-overlay",
            registry: portsFile,
            ownPort: nil
        )
        ...
        Section {
            HStack(alignment: .firstTextBaseline, spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text("Extra workspaces require Supercli Link")
                        .font(.system(size: 13, weight: .semibold))
                        .foregroundStyle(Theme.foreground)
                    Text("Supercli Link adds workspaces, remote control from your iPhone, and "
                        + "Supercli Remote access from anywhere — $59 per seat per year.")
                        .font(.system(size: 12))
                        .foregroundStyle(Theme.mutedForeground)
                        .fixedSize(horizontal: false, vertical: true)
                    if let onOpenPro {
                        Button("Open Supercli Link Settings", action: onOpenPro)
                            .controlSize(.small)
                            .padding(.top, 4)
                    }
                }
    ...
    private func refresh() {
        ...
        rows = refreshedRows
        guard SupercliFeatureFlags.mobileRemoteControlEnabled else {
            sharingByHome = [:]
            return
        }
    ...
    private func sharingSummary(for row: WorkspaceListRowModel) -> WorkspaceSharingSummary? {
        guard SupercliFeatureFlags.mobileRemoteControlEnabled,
              isScoped(row),
              case .local = row.kind,
              let home = row.home
        else { return nil }
    ...
    static func buildRows(
        ...
        let registry = SupercliWorkspaceRegistry.load()
        let defaultHome = SupercliWorkspaceRegistry.realSupercliDir
        let normalizedDefault = SupercliWorkspaceRegistry.normalizePath(defaultHome.path)
        let currentHome = SupercliWorkspaceContext.isDefaultInstance
            ? normalizedDefault
        ...
        for record in registry {
            let normalized = SupercliWorkspaceRegistry.normalizePath(record.home)
            ...
            let isRunning = isCurrent
                || SupercliWorkspaceLauncher.runningPid(
        ...
        // A dev-blank instance runs from an unregistered SUPERCLI_HOME; give it
    ...
    /// launcher uses as SUPERCLI_HOME so the suite hash matches.
    private static func localStoredTint(home: String?) -> AppTint {
        AppDefaults.suite(forSupercliHome: home)
            .string(forKey: SupercliStore.nativeAppTintKey)
...
struct WorkspaceListRowModel: Identifiable {
    enum Kind {
        case local(
            record: SupercliWorkspaceRecord?,
            isDefault: Bool,
...
enum RemoteWorkspaceTint {
    private static let key = "supercli.native.remoteWorkspaceTint"
