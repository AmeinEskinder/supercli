...
/// The Supercli Browser MCP policies, rendered as Form sections inside
        ...
        Section {
            ...
            case "ready":
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text(verbatim: "agent-browser \(engine?.version ?? "") at "
                        + "\(engine?.path ?? "~/.supercli/browser/bin/agent-browser"), "
            ...
            case "disabled":
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text(verbatim: "This Host runs with SUPERCLI_BROWSER_ENGINE_INSTALL=0, so it "
                        + "never installs the engine itself. Run "
                        + "`supercli browser install` on the Host, or set "
                        + "SUPERCLI_AGENT_BROWSER_BIN to an engine binary.")
                        .font(.system(size: 11))
                        .foregroundStyle(Theme.mutedForeground)
                        .fixedSize(horizontal: false, vertical: true)
                }
            case "failed":
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text(verbatim: (engine?.error ?? "unknown error")
                        + " — run `supercli browser install` on the Host to retry "
            ...
            default:
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text(verbatim: "The Host service has not published an engine state yet "
                        ...
                        + "on its own; `supercli browser install` does the same by hand.")
                        .font(.system(size: 11))
                        .foregroundStyle(Theme.mutedForeground)
                        .fixedSize(horizontal: false, vertical: true)
                }
    ...
    private func refreshEngineStatus() async {
        while !Task.isCancelled {
            engine = HostBrowserEngineStatus.read(
                at: LaunchConfig.supercliDir.appendingPathComponent("serve.json"))
