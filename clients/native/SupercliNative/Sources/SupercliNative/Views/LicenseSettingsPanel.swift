...
//  SupercliNative
//
//  Settings ▸ Remote ▸ Supercli Link: paste a key, activate this Host, or
//  release the seat. The app is free; a license unlocks Supercli Link — the
//  device-linking features (Supercli Remote, iPhone pairing, workspaces).
//  Customer-facing "Pro" is renamed Supercli Link (2026-08-12), and the
...
struct LinkLicenseSections: View {
    ...
    private func activeSection(_ payload: LicensePayload) -> some View {
        Section {
            ...
            SettingsSectionHeader(
                title: "Supercli Link subscription",
    ...
    private func revokedSection(_ payload: LicensePayload) -> some View {
        Section {
            HStack(spacing: 8) {
                ...
                Text("This Supercli Link key is no longer active")
                    .font(.system(size: 13, weight: .semibold))
                    .foregroundStyle(Theme.foreground)
            }
            Text("The subscription for \(payload.email) has ended or been refunded. Supercli "
                + "keeps working — renew or enter a different key to use Supercli Link again.")
                .font(.system(size: 12))
            ...
            SettingsSectionHeader(title: "Supercli Link subscription")
        } footer: {
            activationFooter
        }
    }
    ...
    private var freeSection: some View {
        Section {
            ...
            SettingsSectionHeader(
                title: "Get Supercli Link",
                description: "Supercli is free on your Mac; Link is the relay above — "
    ...
    private static let proFeatures: [(icon: String, title: String, detail: String)] = [
        ...
        (
            ...
            "Pair the Supercli iOS app to steer sessions from your phone."
        ),
        (
            "person.2",
            "Workspaces",
            "Run separate Supercli instances with their own sessions and settings."
        )
    ]
    ...
    private var activationFooter: some View {
        HStack(spacing: 4) {
            Link("Get Supercli Link", destination: LicenseConfig.apiBaseURL.appendingPathComponent("link"))
