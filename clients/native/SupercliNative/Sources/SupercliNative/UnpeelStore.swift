...
//  and launches new sessions through supercli-host.
//
...
enum NativeOverlay {
    ...
    static let sessionTitlesKey = "supercli.native.sessionTitles"

    /// [Supercli session id: provider conversation id] — captured from hook
    ...
    static let providerSessionIDsKey = "supercli.native.providerSessionIDs"
    ...
    static let restartRecommendationDismissalsKey =
        "supercli.native.restartRecommendationDismissals"
    ...
    static let notifyWhenDoneKey = "supercli.native.notifyWhenDone"
    ...
    static let archivedSessionsKey = "supercli.native.archivedSessions"
    ...
    static let archivedAtKey = "supercli.native.archivedAt"
}
...
/// hot UI state, while `SupercliStore.objectWillChange` wakes every project node
...
/// `SupercliStore` made a completed `git symbolic-ref` invalidate every sidebar
...
@MainActor
final class SupercliStore: ObservableObject {
    ...
    /// titlebar bell. Backed by `<SUPERCLI_HOME>/activity-log.jsonl`.
    ...
    @Published var expandedProjectIDs: Set<String> =
        Set(AppDefaults.shared.stringArray(forKey: SupercliStore.expandedProjectsKey) ?? []) {
        didSet {
    ...
    private var expandedProjectsStorageKey = SupercliStore.expandedProjectsKey
    ...
    @Published private(set) var sidebarVisibleSessionLimit = SupercliStore.defaultSidebarStoppedLimit {
    ...
    /// this; the native app does (requested), alongside supercli.sidebar.width.
    ...
    static func resolveMenuAttentionDetection() -> Bool {
        ...
        if !SupercliWorkspaceContext.isDefaultInstance,
    ...
    func revertExperimentalToInheritedBaseline() {
        SupercliFeatureFlags.revertToInheritedBaseline()
        enabledExperimentalKeys = Set(
            AppFeature.all
                .filter { SupercliFeatureFlags.isEnabled($0) }
                .map(\.key)
        )
    }
    ...
    /// user registered through Supercli's own UI are never touched.
    ...
    /// state. Driven by the Finder "New Supercli Session Here" service and
    ...
    /// Whether sessions may create Supercli-managed worktrees
    ...
    /// Supercli Link profile (`profile_display_name` / `profile_avatar`): the
    ...
    /// `supercli-host` protocol. Populated from manifest host_protocol_version
    ...
    /// against the CURRENT probe rules (via `supercli-host
    ...
    @Published private(set) var enabledExperimentalKeys: Set<String> =
        Set(AppFeature.all.filter { SupercliFeatureFlags.isEnabled($0) }.map(\.key))
    ...
    /// Supercli-owned file. Poll their cheap Git registry on the existing 5s
    ...
    private let startupPresentationCache = StartupPresentationCache(home: LaunchConfig.supercliDir)
    ...
    private let scanCollectQueue = DispatchQueue(
        label: "supercli.scan-collect", qos: .userInitiated
    )
    ...
    nonisolated static func hasDurableResumeEvidence(
        ...
        guard let managedPath = validatedManagedStoragePath(
            ...
            supercliDir: LaunchConfig.supercliDir
        ) else { return false }
    ...
    /// preset self-test's throwaway `SupercliStore()` never starts one.
    ...
    static let sidebarCollapsedKey = "supercli.sidebar.collapsed"
    ...
    static let selectedSessionKey = "supercli.native.selectedSession"
    static let expandedProjectsKey = "supercli.native.expandedProjects"
    static let menuAttentionDetectionKey = "supercli.native.menuAttentionDetection"
    static let showSessionGalleryKey = "supercli.native.showSessionGallery"
    ...
    static let showAgentWorkspacesKey = "supercli.native.showAgentWorkspaces"
    static let commandTActionKey = "supercli.native.commandTAction"
    private static let nativePinsKey = "supercli.sidebar.pins"
    private static let nativePendingTitleWritesKey = "supercli.native.pendingTitleWrites"
    static let nativePresetsKey = "supercli.native.presets"
    static let nativeThemeKey = "supercli.native.theme"
    nonisolated static let nativeAppTintKey = "supercli.native.appTint"
    private nonisolated static let nativeCodeEditorKey = "supercli.native.codeEditor"
    private static let appPresentationReceiptsKey =
        "supercli.native.appPresentationReceipts.v1"
    ...
    // removed outright.
    private static let legacyAutoSessionCleanupDaysKey = "supercli.native.autoSessionCleanupDays"
    private static let legacyAutoSessionStopMinutesKey = "supercli.native.autoSessionStopMinutes"
    static let nativePresetOrderKey = "supercli.native.presetOrder"
    ...
    private static let legacyCLIAvailabilityKey = "supercli.native.cliAvailability"
    private static let legacyCLIDefaultsKey = "supercli.native.cliDefaults"
    private static let legacyCLIOrderKey = "supercli.native.cliOrder"
    private static let nativeProjectFolderColorsKey = "supercli.native.projectFolderColors"
    ...
    init(
        ...
        sessionSelection.$sessionID
            ...
            .store(in: &remoteScopeCancellables)
        // Publish-rate probe (SUPERCLI_DEBUG=1 only): every objectWillChange
        ...
        if ProcessInfo.processInfo.environment["SUPERCLI_DEBUG"] == "1" {
            var publishCount = 0
    ...
    func selectLocalWorkspace(_ record: SupercliWorkspaceRecord) {
        selectLocalWorkspace(home: record.home, name: record.name)
    }

    func selectLocalWorkspace(home: String, name: String) {
        rememberOutgoingScopeSelection()
        let normalizedHome = SupercliWorkspaceRegistry.normalizePath(home)
        ...
        // like SupercliWorkspaceRegistry.create does so the gateway and a later
    ...
    /// Install Supercli with an explicit user action, then run the ordinary
    ...
    func installAndAddSSHHost(target input: String, secret: String?) async throws -> SSHHostRecord {
        ...
        let normalizedSecret = secret?.isEmpty == false ? secret : nil
        do {
            try await NativeRemoteBackend.installSupercli(
                sshTarget: target,
                mode: .command,
                secret: normalizedSecret
            )
    ...
    /// — same rule as supercli-core's app_state::save choke point.
    @discardableResult
    ...
    /// The `~/.supercli` dir the CURRENT scope's filesystem/state verbs target:
    ...
    /// run there.
    private var scopedLocalSupercliDir: URL? {
        switch selectedHostScope {
        case .local:
            return LaunchConfig.supercliDir
    ...
    private var scopedAppStateFile: URL? {
        scopedLocalSupercliDir?.appendingPathComponent("app-state.json")
    }

    /// The UserDefaults suite that backs the current scope's native overlays
    /// (`supercli.native.projects`, …): this instance's own `.shared` for
    ...
    private var scopedAppDefaults: UserDefaults {
        AppDefaults.suite(forSupercliHome: selectedHostScope.scopedLocalHome)
    }
    ...
    private func editScopedAppStateAnnouncing(
        ...
        Self.announceStateChange(
            "app-state",
            registry: scopedLocalSupercliDir?.appendingPathComponent("app-ports"),
            ...
            Task { @MainActor in
                ...
                case .success(.reconcileMobileE2EKeys):
                    ...
                    do {
                        ...
                        NSLog(
                            "[SupercliNative] platform E2E Keychain reconciliation failed: %@",
                ...
                case let .success(.removeMobileE2EKey(deviceID)):
                    ...
                    do {
                        ...
                        NSLog(
                            "[SupercliNative] platform E2E Keychain removal failed: %@",
                            error.localizedDescription
                        )
                ...
                case .success(.overlaySnapshot):
                    do {
                        ...
                        NSLog(
                            "[SupercliNative] platform overlay snapshot failed: %@",
                            error.localizedDescription
                        )
                ...
                case let .success(.registerPushToken(deviceID, token, environment)):
                    ...
                    do {
                        ...
                        NSLog("[SupercliNative] platform push registration failed: %@", error.localizedDescription)
    ...
    func startLocalHostControlClient() {
        guard SupercliFeatureFlags.mobileRemoteControlEnabled else { return }
    ...
    private func startLocalHostControlRefresh() {
        ...
        let home = LaunchConfig.supercliDir.standardizedFileURL.path
    ...
    private func refreshLocalHostControlState(home: String? = nil) async {
        let home = home ?? LaunchConfig.supercliDir.standardizedFileURL.path
        do {
    ...
    private static func mintWorkspacePairingCode(
        ...
        throw lastError ?? SupercliWorkspaceError(
    ...
    func beginRemoteHostPairing() {
        ...
        guard remoteHostRuntime.supportsHostOperation(
            ...
            remoteHostPairingError =
                "This Host cannot create remote pairing invitations yet. Update Supercli on the Host."
            return
        }
    ...
    /// it on Supercli Link. Enforcement is Host-side: the uplink re-registers
    ...
    /// Scope a paired Host (outbound) to Direct-only or restore its Supercli
    ...
    func currentWorkspaceSettingsWire() -> RemoteWorkspaceSettings {
        ...
        let computerAdapterReason: String? = "Supercli computer use has been retired."
    ...
    /// `~/.supercli/session-order.json` — `{ project_id: [session ids] }`, the
    ...
    static var sharedSessionOrderURL: URL {
        LaunchConfig.supercliDir.appendingPathComponent("session-order.json")
    }
    ...
    private static func editSharedSessionOrders(
        ...
        let wrote = PresetStateFile.withExclusiveLock(on: sharedSessionOrderURL) {
            ...
            do {
                ...
                NSLog("[SupercliNative] shared session order write failed: \(error)")
                return false
            }
        } ?? false
    ...
    enum SharedMarker: String {
        ...
        /// supercli-core session_ops::set_provider_session for the merge and
    ...
    nonisolated static func sessionLifecycleLockURL(
        supercliDir: URL, sessionID: String
    ) -> URL {
    ...
    nonisolated static func acquireSessionLifecycleLease(
        ...
        acquireSessionFileLock(
            at: sessionLifecycleLockURL(supercliDir: supercliDir, sessionID: sessionID)
        )
    }
    ...
    private nonisolated static func acquireSessionFileLock(
        ...
        do {
            ...
            NSLog("[SupercliNative] failed to create Session lock directory: \(error)")
            return nil
        }
        let descriptor = open(
            lockURL.path,
    ...
    /// rule as supercli-core. Hook-capable agents have a truthful durable hook
    ...
    func openSettings(tab: SettingsTab? = nil) {
        if let tab {
            settingsTab = (tab == .mobile && !SupercliFeatureFlags.mobileRemoteControlEnabled)
                ? .agents
                : tab
        } else if settingsTab == .mobile && !SupercliFeatureFlags.mobileRemoteControlEnabled {
            settingsTab = .agents
        }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            do {
                ...
                if process.terminationStatus != 0 {
                    ...
                    NSLog("Supercli install %@ failed (exit %d): %@",
    ...
    static func inheritedDefaultTheme() -> ThemePreference? {
        guard !SupercliWorkspaceContext.isDefaultInstance else { return nil }
    ...
    func reloadAppTintFromDisk() {
        ...
        NotificationCenter.default.post(name: .supercliWorkspaceTintChanged, object: nil)
    ...
    private func effectiveScopeTintHue() -> Double? {
        ...
        case .localWorkspace(let home, _):
            ...
            // launcher's SUPERCLI_HOME on this machine, so the suite hash lines
    ...
    static func workspaceTint(home: String) -> AppTint {
        AppDefaults.suite(forSupercliHome: home)
    ...
    // (crates/supercli-serve/src/auto_archive.rs): the worker reads
    ...
    static func workspaceThemePreference(home: String) -> ThemePreference {
        let suite = AppDefaults.suite(forSupercliHome: home)
    ...
    func revertAppearanceToInheritedBaseline() {
        ...
        let theme = Self.workspaceThemePreference(home: LaunchConfig.supercliDir.path)
        if theme != themePreference {
            themePreference = theme
        }
        applyAppAppearance()
    ...
    private func writeAppAppearanceFile() {
        ...
        let url = LaunchConfig.supercliDir.appendingPathComponent("app-appearance")
        do {
            try FileManager.default.createDirectory(
            ...
            NSLog("[SupercliNative] failed to write app appearance file: \(error)")
        }
    }
    ...
    // Supercli Sessions MCP reads `mcp_orchestrators` only from app-state.json, so
    ...
    // MARK: - Supercli Link profile (Settings ▸ Remote ▸ Supercli Link)
    //
    ...
    /// Delete the "kept per project" browsing data: the Supercli-managed
    ...
    func clearBrowserProfiles() {
        ...
        if let files = try? FileManager.default.contentsOfDirectory(
            ...
            for file in files where file.lastPathComponent.hasPrefix("supercli-proj-") {
    ...
    private func mutateAppStateJSON(_ mutate: (inout [String: Any]) -> Void) {
        ...
        guard let data = try? JSONSerialization.data(
            ...
            NSLog("[SupercliNative] failed to serialize app-state.json for MCP security write")
            return
        }
        do {
            ...
            NSLog("[SupercliNative] failed to write app-state.json: \(error)")
        }
    }
    ...
    private func dispatchSessionPush(
        ...
        let title = if let customTitle, !customTitle.isEmpty {
            ...
            rawTitle.isEmpty ? "Supercli session" : rawTitle
        }
    ...
    private static let readReceiptQueue = DispatchQueue(
        label: "com.supercli.read-receipts",
        qos: .utility
    )
    ...
    private static let duplicatePresetRepairKey = "supercli.native.presetDuplicateRepair.1"
    ...
    func repairDuplicatePresetsOnce() {
        ...
        if removed > 0 {
            NSLog("[SupercliNative] presets: collapsed %d exact duplicate row(s)", removed)
            reloadSharedPresets()
        }
    }
    ...
    /// - ~/.supercli/app-sessions  (manifests appear/change/vanish; output.bin
    ///   growth maintains hook deadlines and terminal recency),
    /// - ~/.supercli/app-state.json (projects/pins/global presets),
    /// - ~/.supercli/session-order.json + project-order.json (a drag in
    ...
    private func rebuildFileWatcher() {
        ...
        let paneLayoutsURL = LaunchConfig.supercliDir
        ...
        let callback: FSEventStreamCallback = { _, info, _, eventPaths, _, _ in
            ...
            let store = Unmanaged<SupercliStore>.fromOpaque(info).takeUnretainedValue()
        ...
        guard let stream = FSEventStreamCreate(
            ...
            NSLog("[SupercliNative] FSEvents stream creation failed; timer fallback only")
            return
        }
    ...
    /// - `supercli.native.projectOrder`             = [top-level project ids]
    /// - `supercli.native.projectOrder.<projectID>` = [worktree child project ids]
    /// - `supercli.native.sessionOrder.<projectID>` = [session ids]
    ...
    /// pruned at read time.
    static let projectOrderKey = "supercli.native.projectOrder"
    ...
    static func sessionOrderKey(_ projectID: String) -> String {
        "supercli.native.sessionOrder.\(projectID)"
    }
    ...
    static func pinnedOrderKey(_ projectID: String) -> String {
        "supercli.native.pinnedOrder.\(projectID)"
    }

    /// `~/.supercli/project-order.json` — a flat rank list for every project.
    ...
    static var sharedProjectOrderURL: URL {
        LaunchConfig.supercliDir.appendingPathComponent("project-order.json")
    }
    ...
    /// Tell every other Supercli that shared state moved — the Swift half of
    /// `supercli-core::state_bus`. Same registry (`~/.supercli/app-ports`), same
    ...
    nonisolated static func announceStateChange(_ change: String, ownPort: UInt16?) {
        announceStateChange(
            change,
            registry: LaunchConfig.supercliDir.appendingPathComponent("app-ports"),
            ownPort: ownPort
        )
    }
    ...
    private func writeSessionManifestFields(
        ...
        Self.withSessionManifestLock(sessionID: sessionID) {
            ...
            do {
                ...
                NSLog("[SupercliNative] failed to sync session title to manifest: \(error)")
                return false
            }
        } ?? false
    }
    ...
    /// `~/.supercli/session-manifest-locks/<sha256(session id)>.lock` path
    /// around every manifest read-modify-write cycle.
    private static func withSessionManifestLock<Result>(
        sessionID: String,
        _ operation: () -> Result
    ) -> Result? {
        let directory = LaunchConfig.supercliDir
        ...
        do {
            ...
            NSLog("[SupercliNative] failed to create manifest lock directory: \(error)")
            return nil
        }
        let digest = SHA256.hash(data: Data(sessionID.utf8))
            .map { String(format: "%02x", $0) }
    ...
    /// Fire-and-forget `supercli-host __auto_title__ <id>`
    ...
    private nonisolated static func autoTitleFromProviderTranscript(sessionID: String) {
        ...
        do {
            ...
            NSLog("[SupercliNative] failed to spawn auto-title: \(error)")
        }
    }
    ...
    /// sessions still running an old `supercli-host`.)
    ...
    /// Rendered by `supercli-host __transcript__ markdown <id>`, which reads the
    ...
    nonisolated static func runTranscriptMarkdown(
        ...
        do {
            ...
            return ("", "Failed to run supercli-host: \(error.localizedDescription)")
        }
        ...
        if process.terminationStatus != 0 {
            ...
            return ("", "supercli-host exited with code \(process.terminationStatus).")
        }
        return (String(data: outData, encoding: .utf8) ?? "", nil)
    }

    /// End a session's cua-driver session (overlay cursor + scope state) via
    /// `supercli-host __computer_cleanup__ <id>` (computer_mcp.rs run_cleanup).
    ...
    private nonisolated static func cleanupComputerSession(
        ...
        if let supercliHome {
            ...
            env["SUPERCLI_HOME"] = supercliHome.path
            process.environment = env
        }
        do {
            try process.run()
        } catch {
            NSLog("[SupercliNative] failed to spawn computer cleanup: \(error)")
        }
    }
    ...
    /// test while production always invokes the real `supercli-host` command.
    ...
    /// `supercli-host` prefixes command errors for CLI readability. Strip only
    ...
    /// instance's SUPERCLI_HOME.
    nonisolated static func validatedManagedStoragePath(
        _ path: String?,
        supercliDir: URL
    ) -> String? {
        guard let path, !path.isEmpty else { return nil }
        let root = supercliDir.standardizedFileURL.resolvingSymlinksInPath()
    ...
    static let removedProjectsKey = "supercli.native.removedProjects"
    ...
    /// a tombstone in `supercli.native.removedProjects`; natively-added
    /// projects are actually deleted from `supercli.native.projects`.
    ...
    private func removeLocalHostProjectedProject(_ projectID: String) {
        ...
        guard wrote else {
            ...
            Self.showErrorAlert(
                ...
                message: "Supercli couldn't update this workspace's shared project state."
            )
            return
        }
    ...
    func localWorkspaceMoveTargets() -> [WorkspaceMoveTarget] {
        guard selectedHostScope.isLocalMachine,
              let sourceHome = scopedLocalSupercliDir
        else { return [] }
        let source = SupercliWorkspaceRegistry.normalizePath(sourceHome.path)
        let stamp = (try? FileManager.default.attributesOfItem(
            atPath: SupercliWorkspaceRegistry.registryURL.path
        ))?[.modificationDate] as? Date
        let defaultName = SupercliWorkspaceContext.defaultWorkspaceName ?? "Personal"
        ...
        let targets = WorkspaceListOrder.apply(
            to: Self.knownLocalWorkspaceHomesAndNames().compactMap { home, name
                ...
                let normalized = SupercliWorkspaceRegistry.normalizePath(home)
    ...
    func moveProjectToLocalWorkspace(projectID: String, dest: WorkspaceMoveTarget) {
        ...
        guard SupercliWorkspaceRegistry.normalizePath(sourceHome.path)
            != SupercliWorkspaceRegistry.normalizePath(destHome.path)
        else { return }
        ...
        Task { @MainActor [weak self] in
            ...
            let switched = WorkspaceFeature.pickerEnabled
                || SupercliWorkspaceRegistry.normalizePath(dest.home)
    ...
    private func rescopeAfterProjectMove(to dest: WorkspaceMoveTarget) {
        let destNorm = SupercliWorkspaceRegistry.normalizePath(dest.home)
        ...
        if let row = WorkspaceSwitching.orderedRows(store: self).first(where: {
            $0.home.map { SupercliWorkspaceRegistry.normalizePath($0) } == destNorm
        }) {
    ...
    private static func defaultsSuite(forWorkspaceHome home: String) -> UserDefaults {
        let normalized = SupercliWorkspaceRegistry.normalizePath(home)
        let defaultHome = SupercliWorkspaceRegistry.normalizePath(
            SupercliWorkspaceRegistry.realSupercliDir.path
        )
        if normalized == defaultHome { return .standard }
        return AppDefaults.suite(forSupercliHome: home)
    }
    ...
    private func openClickedFileInEditor(_ match: ClickablePath.Match, path: String) {
        guard selectedHostScope.isLocalMachine else {
            Self.showRemoteVerbFailure(
                ...
                message: "Choose an Supercli App for this file type to open it from a remote workspace."
            )
            return
        }
    ...
    func setExperimental(_ enabled: Bool, for feature: AppFeature) {
        SupercliFeatureFlags.setEnabled(enabled, for: feature)
        if enabled {
    ...
    var selectedHostAdvertisesComputerUse: Bool {
        ...
        return SupercliFeatureFlags.computerUseControllable(
    ...
    /// Rename the Supercli worktree project. This deliberately does not rename
    ...
    func promptRenameWorktreeProject(_ projectID: String) {
        ...
        alert.informativeText = branch.map {
            """
            This changes the name shown in Supercli. The git branch stays \
            "\($0)".
            """
        } ?? "This changes the name shown in Supercli."
    ...
    static func paneSidebarItem(
        ...
        let runtimeName = SupercliRuntimeCatalog.runtime(command: command)?.label
    ...
    /// share the direct `supercli-attach` data plane with Local scope; a true
    ...
    /// ~/.supercli/app-state.json, so we never write it; native mutations
    ...
    /// The preset the Supercli Sessions MCP launches for a new session of `cli`:
    ...
    static func seamlessRemoteInstallCommand(_ install: String) -> String {
        var lines = [
            ...
            "if ! grep -q 'supercli agent PATH' \"$HOME/.profile\" 2>/dev/null; then "
                + "{ echo '# supercli agent PATH'; "
    ...
    private static let nativeProjectsKey = "supercli.native.projects"
    ...
    /// Finder "New Supercli Session Here" service entry point: reuse-or-add the
    ...
    /// In-memory projects for hook verification (SUPERCLI_TEST_LAUNCH with a
    /// `path:` spec). Never persisted anywhere, so other Supercli instances
    ...
    var titlebarSegments: [String] {
        ...
        guard let project = titlebarProject else {
            return hostPrefix.isEmpty ? ["Supercli"] : hostPrefix
        }
    ...
    func refreshTitlebarBranch() {
        ...
        // Local-machine scopes — this instance AND other Supercli workspaces
    ...
    private func resolveLocalTitlebarBranch(path: String) {
        guard SupercliStore.isGitRepo(path: path) else {
            titlebarBranchPath = nil
            titlebarBranchState.update(name: nil, isWorktree: false)
            return
        }
    ...
    /// Writes a launch file and spawns supercli-host detached, then polls for
...
extension SupercliStore {
    ...
    private static func currentInstanceNormalizedHome() -> String {
        SupercliWorkspaceContext.isDefaultInstance
            ? SupercliWorkspaceRegistry.normalizePath(
                SupercliWorkspaceRegistry.realSupercliDir.path
            )
            : SupercliWorkspaceRegistry.normalizePath(LaunchConfig.supercliDir.path)
    }
    ...
    private static func knownLocalWorkspaceHomesAndNames() -> [(String, String)] {
        var homes: [(String, String)] = [(
            ...
            SupercliWorkspaceContext.defaultWorkspaceName ?? "Personal"
        )]
        for record in SupercliWorkspaceRegistry.load() {
            homes.append((
                SupercliWorkspaceRegistry.normalizePath(record.home),
                record.name
            ))
        }
        return homes
...
struct ScanDiskSnapshot {
    ...
    fileprivate var projectOverrideStamps: [String: SupercliStore.FileStamp] = [:]
}
...
final class ScanSnapshotCollector: @unchecked Sendable {
    ...
    private var manifestCache:
        [String: (stamp: SupercliStore.FileStamp, manifest: HostedSessionManifest?)] = [:]
    ...
    func collect(
        ...
        for dirName in names where !dirName.hasPrefix(".") {
            ...
            guard let stamp = SupercliStore.statFile(manifestPath) else {
                manifestCache.removeValue(forKey: dirName)
                continue
            }
            ...
            if manifest.state == "running",
               let outputStamp = SupercliStore.statFile(dirPath + "/output.bin") {
                snapshot.outputSignals[dirName] = UInt64(clamping: outputStamp.size)
            }
            if access(
                dirPath + "/" + SupercliStore.SharedMarker.archived.rawValue, F_OK
            ) == 0 {
                snapshot.archivedMarkerDirs.insert(dirName)
            }
            if SupercliStore.hasDurableResumeEvidence(
                manifest: manifest,
                dirPath: dirPath
            ) {
                snapshot.resumableStateDirs.insert(dirName)
            }
            if let overrideStamp = SupercliStore.statFile(
