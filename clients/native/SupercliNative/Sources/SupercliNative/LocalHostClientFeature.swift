...
/// workspace worker (`supercli-host __serve__`). There is no in-app Host any
...
enum LocalHostClientFeature {
    ...
    static func resolveForLaunch(
        ...
        realHome: URL = LaunchConfig.realSupercliDir
    ) -> Task<Void, Never> {
        let log: @Sendable (String) -> Void = { message in
            NSLog("[SupercliNative] %@", message)
...
/// Launch diagnostics land in `~/.supercli/hooks/trace.log` next to the hook
/// and worker lines so one file tells the story of a launch.
enum LaunchTrace {
    nonisolated static func append(_ line: String, home: URL = LaunchConfig.supercliDir) {
