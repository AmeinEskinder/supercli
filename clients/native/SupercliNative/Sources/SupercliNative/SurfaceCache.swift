...
/// cache never starts a replacement `supercli-attach` while the old one is
...
final class SurfaceCache: ObservableObject {
    ...
    private struct PaneRecord {
        ...
        /// `app-sessions` directory this pane's `supercli-attach` runs against:
        ...
        let sessionsDir: URL
        /// The hosted PTY this pane's `supercli-attach` was started against
    ...
    private let themeReadQueue = DispatchQueue(label: "supercli.provider-theme", qos: .utility)
    ...
    init() {
        ...
        appTintTickObserver = NotificationCenter.default.addObserver(
            forName: .supercliAppTintAnimationTick, object: nil, queue: .main
        ) { [weak self] _ in
            MainActor.assumeIsolated {
        ...
        transparencyObserver = NotificationCenter.default.addObserver(
            forName: .supercliTransparencyChanged, object: nil, queue: .main
        ) { [weak self] _ in
            MainActor.assumeIsolated {
                self?.reapplyAppTint()
            }
        ...
        terminalFontObserver = NotificationCenter.default.addObserver(
            forName: .supercliTerminalFontChanged, object: nil, queue: .main
        ) { [weak self] _ in
            MainActor.assumeIsolated {
                self?.reapplyAppTint()
            }
    ...
    /// can reclaim this exact record, preventing a second `supercli-attach`
    ...
    private func reapplyAppTint(visibleOnly: Bool = false) {
        ...
        for (id, record) in panes {
            if visibleOnly {
                ...
                // only Supercli-themed canvases — provider-themed backgrounds
    ...
    private func rebuildThemeWatcher() {
        ...
        guard let stream = FSEventStreamCreate(
            ...
            NSLog("[SupercliNative] provider theme FSEvents stream creation failed")
            return
        }
