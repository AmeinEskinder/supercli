...
final class MenuBarController: NSObject {
    private let store: SupercliStore
    ...
    private static let workspaceTag: String? = {
        guard let name = SupercliWorkspaceContext.displayName else { return nil }
    ...
    private func configureButton() {
        ...
        if let tag = Self.workspaceTag {
            button.toolTip = "Supercli sessions — \(tag)"
        } else {
            button.toolTip = "Supercli sessions"
        }
    ...
    private func refreshButton() {
        ...
        case .unread(let dark):
            // Settled but unread (jobs done): the Supercli mark with a colored
        ...
        case .idle:
            // Idle (no sessions running): the Supercli brand mark, always present.
