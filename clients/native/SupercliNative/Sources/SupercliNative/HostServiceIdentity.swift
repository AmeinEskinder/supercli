...
//  once per launch. After a Sparkle update the bundled `supercli-host` service
...
enum HostServiceIdentity {
    ...
    enum Decision: Equatable {
        ...
        /// service such as `supercli serve install`'s launchd job).
    ...
    /// Same stamp `supercli_core::session_host::current_host_build_id` computes
    ...
    /// supercli-host image. Anything else is a recycled pid and is never signaled.
    ///
    ...
    nonisolated static func processMatchesRecord(pid: Int32, startedAtUnixMs: UInt64) -> Bool {
        guard pid > 1, kill(pid, 0) == 0,
              let actual = SupercliStore.processStartTimeMs(pid)
        else { return false }
        ...
        return isSupercliHostImage(path: path, processName: processName(pid))
    }
    ...
    nonisolated static func isSupercliHostImage(path: String?, processName: String?) -> Bool {
        if let path, !path.isEmpty {
            return path.hasSuffix("/supercli-host")
        }
        return processName == "supercli-host"
    }
