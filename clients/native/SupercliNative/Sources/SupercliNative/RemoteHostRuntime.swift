...
//  into SupercliStore's Local projects/sessions or SurfaceCache.
//
...
enum RemoteHostTransport: Sendable {
    ...
    /// `supercli-attach` data plane. The transport refuses the legacy in-child
    ...
    fileprivate var usesDirectSessionDataPlane: Bool {
        ...
        case .localService, .localGateway:
            // Both homes are on this disk: the surface runs `supercli-attach`
    ...
    fileprivate var continuityKey: RemoteHostContinuityKey {
        ...
        case let .localGateway(supercliHome, _, _):
            return .workspaceHome(supercliHome)
        case let .localService(supercliHome, _, _):
            return .workspaceHome(supercliHome)
        case .direct, .link:
...
final class RemoteHostRuntime: ObservableObject {
    ...
    func seedSnapshot(_ seed: RemoteBootstrapSnapshot) {
        ...
        SupercliAppIconCatalog.update(seed.availableApps ?? [])
        snapshot = seed
    ...
    init(
        ...
        backendFactory: @escaping BackendFactory = { transport in
            ...
            case let .localGateway(supercliHome, _, expectedHostID):
                try NativeRemoteBackend(
                    localGatewayHome: supercliHome,
                    expectedHostID: expectedHostID
                )
            case let .localService(supercliHome, _, expectedHostID):
                try NativeRemoteBackend(
                    localGatewayHome: supercliHome,
                    expectedHostID: expectedHostID,
    ...
    static var defaultForceLinkForDevelopment: Bool {
        ...
        return UserDefaults.standard.bool(forKey: "supercli.native.forceLink")
    }
    ...
    func connectPairedHost(
        ...
        // A Direct-only Host (removed from the Supercli Link enrollment list)
...
extension RemoteHostRuntime {
    ...
    private func requireConnection(
        ...
        guard supportsHostOperation(capability) else {
            throw RemoteHostVerbError(
                operation: operation,
                message: "This Host does not support \(operation) yet. Update Supercli on the Host.",
                outcomeIsUnknown: false
            )
        }
