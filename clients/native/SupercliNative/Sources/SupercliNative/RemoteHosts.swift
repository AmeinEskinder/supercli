...
enum RemoteHostFeature {
    static var pickerEnabled: Bool {
        if Bundle.main.object(forInfoDictionaryKey: "SupercliDevelopmentBuild") as? Bool == true {
            return true
        }
        return SupercliFeatureFlags.isEnabled(.remoteWorkspaces)
    }
}
...
struct KeychainSSHHostSecretStore: SSHHostSecretStoring {
    private static let service = "com.supercli.native.ssh-host"
...
enum RemoteHostPairingError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidCode:
            "That is not an Supercli pairing code."
        case .incompatibleProtocol:
...
enum SSHHostSetupError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .missingIdentity:
            "The remote Supercli Host did not provide a stable identity. Update Supercli on the Host and try again."
...
struct KeychainRemoteHostCredentialStore: RemoteHostCredentialStoring {
    private static let service = "com.supercli.native.remote-host"
...
final class RemoteHostStore: ObservableObject {
    private static let recordsKey = "supercli.native.remoteHosts"
    private static let sshRecordsKey = "supercli.native.sshHosts"
    private static let selectedHostKey = "supercli.native.selectedRemoteHost"
    private static let controllerIDKey = "supercli.native.remoteControllerID"
    ...
    /// Supercli Link fallback. Same narrows-only storage convention as the
