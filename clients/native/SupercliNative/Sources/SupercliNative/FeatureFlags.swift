...
struct AppFeature: Identifiable, Hashable {
    ...
    init(
        ...
        self.defaultsKey = "supercli.experimental.\(key)"
...
enum SupercliFeatureFlags {
    ...
    static var mobileRemoteControlEnabled: Bool {
        if ProcessInfo.processInfo.environment["SUPERCLI_DEV_MOBILE_REMOTE"] == "1" {
            return true
        }
        let key = "supercli.dev.mobileRemoteControl"
    ...
    /// Mac-as-client: connect this Supercli to another Supercli's remote server
    /// and attach to its sessions. Experimental; pairs with the Rust-side
    /// SUPERCLI_REMOTE_ATTACH=1 gate on the attach CLI.
    static var remoteSupercliClientEnabled: Bool {
        if ProcessInfo.processInfo.environment["SUPERCLI_REMOTE_ATTACH"] == "1" {
            return true
        }
        return AppDefaults.shared.bool(forKey: "supercli.dev.remoteSupercliClient")
    }
}
...
enum WorkspaceFeature {
    static var pickerEnabled: Bool {
        pickerEnabled(
            localWorkspacesEnabled: SupercliFeatureFlags.isEnabled(.workspaces),
