...
final class DesktopNotifier: NSObject, ObservableObject, UNUserNotificationCenterDelegate {
    ...
    /// Tapping a banner selects that session. Wired by `SupercliStore`.
        ...
        UNUserNotificationCenter.current().requestAuthorization(
            ...
            Task { @MainActor in
                ...
                if let errorDescription {
                    NSLog("[SupercliNative] notification authorization failed: %@", errorDescription)
                }
            }
        }
    }
        ...
        UNUserNotificationCenter.current().requestAuthorization(
            ...
            Task { @MainActor in
                ...
                if let errorDescription {
                    ...
                    NSLog(
                        "[SupercliNative] test notification authorization failed: %@",
                        errorDescription
                    )
                } else if granted {
        ...
        UNUserNotificationCenter.current().add(request) { @Sendable [weak self] error in
            ...
            Task { @MainActor in
                ...
                if let errorDescription {
                    self.lastTestDiagnostic = .failed(errorDescription)
                    NSLog(
                        "[SupercliNative] test notification scheduling failed: %@",
                        errorDescription
                    )
                } else {
    ...
    func notify(title: String, body: String, sessionID: String, kind: String) {
        ...
        UNUserNotificationCenter.current().add(request) { error in
            if let error {
                NSLog(
                    "[SupercliNative] desktop notification scheduling failed: %@",
    ...
    func notifyWorkspaceAttention(
        ...
        UNUserNotificationCenter.current().add(request) { error in
            if let error {
                NSLog(
                    "[SupercliNative] workspace notification scheduling failed: %@",
                    error.localizedDescription
                )
            }
        }
    ...
    // Show the banner even while Supercli is frontmost (the caller already gated
