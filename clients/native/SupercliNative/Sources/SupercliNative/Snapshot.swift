...
//    SUPERCLI_SNAPSHOT=/path.png   render window.contentView to PNG after 3s
//    SUPERCLI_SNAPSHOT_QUIT=1      terminate after writing
//    SUPERCLI_SNAPSHOT_DELAY=<s>   override the 3s delay
//    SUPERCLI_SELECT_SESSION=<id>  auto-select a session before the snapshot
//    SUPERCLI_TEST_LAUNCH=<project-id>|<command>
//                                 exercise the real in-app launch path
//                                 (launch file -> supercli-host -> manifest
//                                 poll -> select + attach)
//    SUPERCLI_EXPAND_PROJECTS=<id,id,...>
//                                 expand projects (animated) at t=0.5s
//    SUPERCLI_SHOW_ALL_SESSIONS=<id,id,...>
//                                 trigger "Show N more" (reveal all session
//                                 rows) for those projects at t=1.0s
//    SUPERCLI_SCROLL_SIDEBAR=<px>  scroll the sidebar list down <px> points
//                                 at t=1.4s (clamped; verifies rows melting
//                                 into the top progressive blur)
//    SUPERCLI_DISABLE_TOP_BLUR=1   omit the sidebar's progressive top blur
//                                 (perf A/B baseline; lives in SidebarView)
//    SUPERCLI_OPEN_WORKTREES=<project-id>
//                                 expand the project so its inline worktree
//                                 folder rows show, at t=0.5s
//    SUPERCLI_COLLAPSE_SIDEBAR=1   collapse the sidebar (animated) at t=0.5s
//    SUPERCLI_OPEN_SETTINGS=1|presets|advanced
//                                 open settings at t=0.5s (sidebar nav
//                                 slides in, content pane swaps to panel)
//    SUPERCLI_CLOSE_SETTINGS=<s>   close settings again at t=<s> (verify
...
//  cacheDisplay. Use SUPERCLI_SNAPSHOT_WINDOW=1 (composited capture) for
...
enum Snapshot {
    static func armIfRequested(
        window: NSWindow, store: SupercliStore, cache: SurfaceCache? = nil
    ) {
        let env = ProcessInfo.processInfo.environment
        guard let path = env["SUPERCLI_SNAPSHOT"], !path.isEmpty else { return }

        // SUPERCLI_DUMP_SCREEN=/path.txt: write the selected session's
        ...
        if let dumpPath = env["SUPERCLI_DUMP_SCREEN"], !dumpPath.isEmpty, let cache {
            let delay = env["SUPERCLI_SNAPSHOT_DELAY"].flatMap(Double.init) ?? 3
        ...
        if let sessionID = env["SUPERCLI_SELECT_SESSION"], !sessionID.isEmpty {
        ...
        // SUPERCLI_SELECT_SESSIONS=<id,id,...>: select each session in turn
        ...
        if let ids = env["SUPERCLI_SELECT_SESSIONS"], !ids.isEmpty {
        ...
        // SUPERCLI_TEST_PROJECT_PATH=<dir>: register (and expand) an
        ...
        if let path = env["SUPERCLI_TEST_PROJECT_PATH"], !path.isEmpty {
        ...
        if let spec = env["SUPERCLI_TEST_LAUNCH"], !spec.isEmpty {
        ...
        if let ids = env["SUPERCLI_EXPAND_PROJECTS"], !ids.isEmpty {
        ...
        // SUPERCLI_SHOW_ALL_SESSIONS reveal hook has nothing left to reveal.)

        // SUPERCLI_SCROLL_SIDEBAR=<px>: scroll the sidebar's scroll view
        ...
        if let raw = env["SUPERCLI_SCROLL_SIDEBAR"], let px = Double(raw) {
        ...
        // SUPERCLI_SCROLL_SETTINGS=<px>: scroll the settings content pane's
        ...
        if let raw = env["SUPERCLI_SCROLL_SETTINGS"], let px = Double(raw) {
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.6) {
                ...
                scroll.reflectScrolledClipView(scroll.contentView)
                NSLog(
                    "[SupercliNative] settings scroll: doc=%.0f viewport=%.0f offset=%.0f",
                    doc.bounds.height, scroll.contentView.bounds.height, target
                )
            }
        }

        // SUPERCLI_OPEN_ARCHIVE=<project-id> — open that project's archived
        // sessions page at t=0.5s (content-pane swap, same as the project
        // context menu's "Archived sessions").
        if let projectID = env["SUPERCLI_OPEN_ARCHIVE"], !projectID.isEmpty {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
        ...
        if let projectID = env["SUPERCLI_OPEN_WORKTREES"], !projectID.isEmpty {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                withAnimation(SidebarMotion.accordionOpen) {
        ...
        // original `supercli.sidebar.collapsed` default afterwards.
        if let raw = env["SUPERCLI_COLLAPSE_SIDEBAR"], raw == "1" || raw == "0" {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                store.sidebarCollapsed = raw == "1"
            }
        }

        // SUPERCLI_OPEN_SETTINGS=1|presets|advanced — open settings on the
        // given tab (default presets): the sidebar slides to the settings
        // nav and the content pane swaps to the panel.
        if let raw = env["SUPERCLI_OPEN_SETTINGS"], !raw.isEmpty {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                // "license" was the standalone Supercli Link tab until it merged
        ...
        // SUPERCLI_CLOSE_SETTINGS=<seconds> — close settings at t=<s>, so a
        // snapshot can verify the open→close round trip leaves the
        // workspace (and the retained terminal surface) intact.
        if let raw = env["SUPERCLI_CLOSE_SETTINGS"], let at = Double(raw) {
            DispatchQueue.main.asyncAfter(deadline: .now() + at) {
                store.closeSettings()
            }
        }

        // SUPERCLI_DEBUG_RENAME_SESSION=<session-id> — put that session's
        ...
        if let sessionID = env["SUPERCLI_DEBUG_RENAME_SESSION"], !sessionID.isEmpty {
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.2) {
                store.editingSessionID = sessionID
            }
        }

        // SUPERCLI_DEBUG_CONFIRM_REMOVE=<session-id>[@archive] — put that
        ...
        // SUPERCLI_OPEN_ARCHIVE).
        if let spec = env["SUPERCLI_DEBUG_CONFIRM_REMOVE"], !spec.isEmpty {
            let parts = spec.split(separator: "@", maxSplits: 1).map(String.init)
            let surface: SupercliStore.RemoveConfirmSurface =
        ...
        // SUPERCLI_DEBUG_CONFIRM_REMOVE_PROJECT=<project-id> — put that
        // project's row into the inline "Remove project?" confirm state at
        // t=1.2s so a snapshot can photograph it. UI state only.
        if let projectID = env["SUPERCLI_DEBUG_CONFIRM_REMOVE_PROJECT"], !projectID.isEmpty {
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.2) {
                store.requestRemoveProject(projectID)
            }
        }

        // SUPERCLI_TEST_RESTART=<session-id>[@<seconds>] — programmatic
        ...
        if let spec = env["SUPERCLI_TEST_RESTART"], !spec.isEmpty {
            let parts = spec.split(separator: "@", maxSplits: 1).map(String.init)
        ...
        // SUPERCLI_TEST_WORKTREE=<parentPath>|<worktreePath>|<branch> —
        ...
        if let spec = env["SUPERCLI_TEST_WORKTREE"], !spec.isEmpty {
        ...
        // SUPERCLI_DEBUG_OPEN_PRESETS=1 — open Settings on the Presets tab.
        // Older detail/detail-off values still open the same single-screen
        // inline editor; there is no preset drill-in route anymore.
        if let presetDebug = env["SUPERCLI_DEBUG_OPEN_PRESETS"], !presetDebug.isEmpty {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
        ...
        // SUPERCLI_SNAPSHOT2=/path.png (+ SUPERCLI_SNAPSHOT2_DELAY=<s>, default
        ...
        let secondPath = env["SUPERCLI_SNAPSHOT2"]
        let secondDelay = env["SUPERCLI_SNAPSHOT2_DELAY"].flatMap(Double.init) ?? (delay + 0.5)
        DispatchQueue.main.asyncAfter(deadline: .now() + delay) {
            write(window: window, to: path)
            if env["SUPERCLI_SNAPSHOT_QUIT"] == "1", secondPath == nil {
                NSApp.terminate(nil)
            }
        }
        if let secondPath, !secondPath.isEmpty {
            DispatchQueue.main.asyncAfter(deadline: .now() + max(secondDelay, delay + 0.1)) {
                ...
                if env["SUPERCLI_SNAPSHOT_QUIT"] == "1" {
                    NSApp.terminate(nil)
                }
            }
        }
    ...
    /// SUPERCLI_TEST_REORDER=1 (+ SUPERCLI_TEST_PROJECT_PATH=<dir> with ≥2
    ...
    static func runReorderSelfTestIfRequested(store: SupercliStore) {
        let env = ProcessInfo.processInfo.environment
        guard env["SUPERCLI_TEST_REORDER"] == "1",
              let projectPath = env["SUPERCLI_TEST_PROJECT_PATH"], !projectPath.isEmpty
        else { return }
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {
            ...
            let sessionKey = SupercliStore.sessionOrderKey(projectID)
            let pinnedKey = SupercliStore.pinnedOrderKey(projectID)
            let worktreeProjectKey = SupercliStore.projectOrderKey(forParent: projectID)
            // Snapshot the keys as found so the user's real overlay (if
            // any) is restored byte-for-byte.
            let savedProjectOrder = defaults.stringArray(forKey: SupercliStore.projectOrderKey)
            ...
            let savedPins = defaults.data(forKey: "supercli.sidebar.pins")
            defer {
                if let savedProjectOrder {
                    defaults.set(savedProjectOrder, forKey: SupercliStore.projectOrderKey)
                } else {
                    defaults.removeObject(forKey: SupercliStore.projectOrderKey)
                }
                ...
                if let savedPins {
                    defaults.set(savedPins, forKey: "supercli.sidebar.pins")
                } else {
                    defaults.removeObject(forKey: "supercli.sidebar.pins")
                }
                store.rescan()
                NSLog(
                    "[reorder-test] DONE: %@",
            ...
            let worktreePathA = URL(fileURLWithPath: projectPath)
                .appendingPathComponent(".supercli-test-worktree-a")
                .path
            let worktreePathB = URL(fileURLWithPath: projectPath)
                .appendingPathComponent(".supercli-test-worktree-b")
                .path
            ...
            @MainActor func worktreeIDs(of store: SupercliStore) -> [String] {
            ...
            @MainActor func regularIDs(of store: SupercliStore) -> [String] {
            ...
            let reloadedPins = SupercliStore()
    ...
    /// SUPERCLI_TEST_RENAME=1: headless round-trip test of the native rename
    /// overlay (supercli.native.sessionTitles): rename → merged label wins →
    ...
    /// written.
    static func runRenameSelfTestIfRequested(store: SupercliStore) {
        guard ProcessInfo.processInfo.environment["SUPERCLI_TEST_RENAME"] == "1" else { return }
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {
            var failures = 0
            func expect(_ condition: Bool, _ message: String) {
                if !condition { failures += 1 }
    ...
    /// SUPERCLI_TEST_PRESETS=1: headless round-trip test of the native
    ...
    static func runPresetSelfTestIfRequested(store: SupercliStore) {
        guard ProcessInfo.processInfo.environment["SUPERCLI_TEST_PRESETS"] == "1" else { return }
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            ...
            AppDefaults.shared.removeObject(forKey: SupercliStore.nativePresetsKey)
            store.rescan()
            expect(
            ...
            expect(
                AppDefaults.shared.object(forKey: SupercliStore.nativePresetsKey) == nil,
    ...
    private static func write(window: NSWindow, to path: String) {
        // SUPERCLI_SNAPSHOT_WINDOW=1: composited capture (vibrancy, traffic
        // lights) via CGWindowList — needs screen-recording permission.
        if ProcessInfo.processInfo.environment["SUPERCLI_SNAPSHOT_WINDOW"] == "1",
           let cg = CGWindowListCreateImage(
            ...
            if let png = rep.representation(using: .png, properties: [:]) {
                ...
                NSLog("[SupercliNative] composited snapshot written to \(path)")
                return
            }
        }

        guard let view = window.contentView else {
            NSLog("[SupercliNative] snapshot: no content view")
            return
        }
        let bounds = view.bounds
        guard let rep = view.bitmapImageRepForCachingDisplay(in: bounds) else {
            NSLog("[SupercliNative] snapshot: no bitmap rep")
            return
        }
        view.cacheDisplay(in: bounds, to: rep)
        guard let png = rep.representation(using: .png, properties: [:]) else {
            NSLog("[SupercliNative] snapshot: png encode failed")
            return
        }
        do {
            try png.write(to: URL(fileURLWithPath: path))
            NSLog("[SupercliNative] snapshot written to \(path)")
        } catch {
            NSLog("[SupercliNative] snapshot write failed: \(error)")
        }
    }
}
