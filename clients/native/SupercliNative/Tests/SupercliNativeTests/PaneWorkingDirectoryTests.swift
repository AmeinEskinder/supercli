...
final class PaneWorkingDirectoryTests: XCTestCase {
    func testScopedSessionWithKnownCwdResolvesARelativePath() {
        let cwd = SupercliStore.paneWorkingDirectory(
    ...
    func testSessionWithoutCwdFallsBackToItsProjectPath() {
        XCTAssertEqual(
            SupercliStore.paneWorkingDirectory(
                sessionCwd: nil,
        ...
        XCTAssertNil(SupercliStore.paneWorkingDirectory(sessionCwd: nil, projectPath: nil))
    }
    ...
    func testHostPublishedCwdSurvivesTheSnapshotIntoTheSessionEntry() throws {
        ...
        XCTAssertEqual(SupercliStore.sessionEntry(fromRemote: with).cwd, "/Users/me/Dev/flatsome")
        ...
        XCTAssertNil(SupercliStore.sessionEntry(fromRemote: without).cwd)
    }
