...
final class ClickablePathTests: XCTestCase {
    ...
    func testAbsolutePath() {
        let m = match("/Users/x/Dev/supercli/AGENTS.md:3", column: 4)
        XCTAssertEqual(m, .init(path: "/Users/x/Dev/supercli/AGENTS.md", line: 3, column: nil))
    }

    func testTildePathStaysUnexpanded() {
        // Expansion happens at resolve time, not extraction.
        let m = match("~/.supercli/app-state.json", column: 2)
        XCTAssertEqual(m, .init(path: "~/.supercli/app-state.json", line: nil, column: nil))
    }
