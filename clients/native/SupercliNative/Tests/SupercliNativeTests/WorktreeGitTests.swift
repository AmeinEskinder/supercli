...
final class WorktreeGitTests: XCTestCase {
    func testRunGitDoesNotBlockWhenCommandWritesLargeStderr() throws {
        let repo = FileManager.default.temporaryDirectory
            .appendingPathComponent("supercli-worktree-git-\(UUID().uuidString)")
        ...
        let result = WorktreeGit.runGit(
            repo: repo.path,
            ["-c", "alias.supercli-spam=\(noisyAlias)", "supercli-spam"]
        )
    ...
    private func git(_ repo: URL, _ args: [String]) -> String {
        let identity = [
            "-c", "user.email=test@supercli.test", "-c", "user.name=Supercli Test",
