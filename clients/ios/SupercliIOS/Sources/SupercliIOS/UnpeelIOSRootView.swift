...
public struct SupercliIOSRootView: View {
...
enum IOSSidebarTheme {
    ...
    static func toolColor(for command: String) -> Color {
        ...
        if let hex = SupercliRuntimeCatalog.runtime(command: command)?.tintColorHex {
    ...
    static func toolSpinnerColor(for command: String) -> Color {
        if let runtime = SupercliRuntimeCatalog.runtime(command: command),
...
private struct SidebarWorkspaceHeader: View {
    var store: RemotePreviewStore
    /// True while the connection tunnels through Supercli Remote — surfaced as
    /// the "via Supercli Remote" subtitle.
    ...
    /// disconnected, "via Supercli Remote" on the relay path. A healthy Direct
    ...
    private var subtitle: String? {
        ...
        if usingRelay { return "via Supercli Remote" }
        return nil
    }
