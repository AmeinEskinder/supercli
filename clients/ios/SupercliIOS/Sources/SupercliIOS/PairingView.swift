...
//  SupercliIOS
//
//  Pair this phone with a workspace running Supercli. The workspace's
...
struct PairingView: View {
    ...
    private var workspacesContent: some View {
        VStack(spacing: 20) {
            ...
            // Supercli on THIS phone, not a particular workspace.
            securitySection
            if #available(iOS 26.0, *) {
                dictationSection
            }
    ...
    private var notificationSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("A ready APNs token is sent to each reachable paired workspace. "
                + "Supercli Link then delivers needs-input and opted-in finished alerts.")
                .font(.caption)
                .foregroundStyle(.white.opacity(0.5))
        }
        .padding(14)
            ...
            Toggle(isOn: Binding(
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text("Require \(AppLockManager.methodLabel())")
                        .foregroundStyle(.white)
                    Text("Locks Supercli when you leave the app.")
                        .font(.caption)
                        .foregroundStyle(.white.opacity(0.5))
                }
            }
        ...
        VStack(alignment: .leading, spacing: 10) {
            if RemoteConnectionStore.devBridgeAvailable {
                ...
                Label("Not paired", systemImage: "wifi.slash")
                    .font(.subheadline)
                    .foregroundStyle(.white.opacity(0.7))
                Text("Open Supercli on your Mac, switch to the workspace you want, and show the pairing code in Settings ▸ iPhone. Each workspace pairs separately.")
                    .font(.footnote)
                    .foregroundStyle(.white.opacity(0.5))
            }
        }
    ...
    private func handlePayload(_ raw: String) {
        ...
        guard let payload = RemotePairingCode.decode(raw) else {
            errorMessage = "That doesn't look like an Supercli pairing code."
            return
        }
