...
# MUSE_PLUGIN_*/PLUGIN_* survive), so the Supercli identity exported into the
...
# shaped; the path patterns stop at whitespace, which every Supercli-managed
# home satisfies.
recover_parent_env() {
  ps eww $PPID 2>/dev/null | grep -oE "$1=$2" | head -1 | cut -d= -f2-
}
[ -n "${SUPERCLI_SESSION_ID:-}" ] \
  || SUPERCLI_SESSION_ID="$(recover_parent_env SUPERCLI_SESSION_ID '[0-9a-fA-F-]+')"
[ -n "${SUPERCLI_APP_PORT:-}" ] \
  || SUPERCLI_APP_PORT="$(recover_parent_env SUPERCLI_APP_PORT '[0-9]+')"
[ -n "${SUPERCLI_SESSION_DIR:-}" ] \
  || SUPERCLI_SESSION_DIR="$(recover_parent_env SUPERCLI_SESSION_DIR '[^[:space:]]+')"
[ -n "${SUPERCLI_APP_PORT_REGISTRY_FILE:-}" ] \
  || SUPERCLI_APP_PORT_REGISTRY_FILE="$(recover_parent_env SUPERCLI_APP_PORT_REGISTRY_FILE '[^[:space:]]+')"
[ -n "${SUPERCLI_HOOK_TRACE_FILE:-}" ] \
  || SUPERCLI_HOOK_TRACE_FILE="$(recover_parent_env SUPERCLI_HOOK_TRACE_FILE '[^[:space:]]+')"
[ -n "${SUPERCLI_RUNTIME_GENERATION:-}" ] \
  || SUPERCLI_RUNTIME_GENERATION="$(recover_parent_env SUPERCLI_RUNTIME_GENERATION '[0-9]+')"
...
# event/tool/post metadata unless SUPERCLI_HOOK_TRACE_VERBOSE=1 explicitly opts
...
# SUPERCLI_MUSE_PARENT_COMMAND_OVERRIDE exists only as a test seam.
muse_parent_command() {
  if [ -n "${SUPERCLI_MUSE_PARENT_COMMAND_OVERRIDE:-}" ]; then
    printf '%s' "$SUPERCLI_MUSE_PARENT_COMMAND_OVERRIDE"
  else
