...
metadata_fields_json() {
  ...
  printf '%s' "$INPUT" | awk '
    {
      rest = $0
      while (match(rest, /"(session_id|provider_session_id|providerSessionID|providerSessionId|thread_id|threadID|threadId|conversation_id|conversationID|conversationId|transcript_path|transcriptPath|provider_transcript_path|providerTranscriptPath|tool_name|prompt_text)"[[:space:]]*:[[:space:]]*"([^"\\]|\\.)*"/)) {
...
case "$EVENT_TYPE" in
  BeforeAgent) EVENT_TYPE="UserPromptSubmit" ;;
  AfterAgent) EVENT_TYPE="Stop" ;;
  AfterTool) EVENT_TYPE="HookSeen" ;;
...
case "$EVENT_TYPE" in
  UserPromptSubmit|Stop|PermissionRequest) record_last_hook_event "$EVENT_TYPE" "" ;;
esac
