...
metadata_fields_json() {
  ...
  printf '%s' "$INPUT" | awk '
    {
      rest = $0
      while (match(rest, /"(session_id|provider_session_id|providerSessionID|providerSessionId|thread_id|threadID|threadId|conversation_id|conversationID|conversationId|transcript_path|transcriptPath|provider_transcript_path|providerTranscriptPath|tool_name|prompt_text|sessionId|toolName)"[[:space:]]*:[[:space:]]*"([^"\\]|\\.)*"/)) {
        field = substr(rest, RSTART, RLENGTH)
        rest = substr(rest, RSTART + RLENGTH)
        key = field
        sub(/^"/, "", key)
        sub(/".*/, "", key)
        if (key == "sessionId") { key = "session_id"; sub(/^"sessionId"/, "\"session_id\"", field) }
        if (key == "toolName") { key = "tool_name"; sub(/^"toolName"/, "\"tool_name\"", field) }
...
case "$EVENT_TYPE" in
  sessionStart|postToolUse) EVENT_TYPE="HookSeen" ;;
  agentStop) EVENT_TYPE="Stop" ;;
  sessionEnd)
    case "$(json_string_value reason || true)" in
      abort|user_exit) EVENT_TYPE="StopCancelled" ;;
      error|timeout) EVENT_TYPE="StopFailure" ;;
      *) EVENT_TYPE="Stop" ;;
    esac
    ;;
  userPromptSubmitted) EVENT_TYPE="UserPromptSubmit" ;;
  permissionRequest|preToolUse) EVENT_TYPE="PermissionRequest" ;;
  *)
    printf '{}\n'
    exit 0
    ;;
...
COPILOT_TOOL_NAME="$(json_string_value tool_name || true)"
[ -n "$COPILOT_TOOL_NAME" ] || COPILOT_TOOL_NAME="$(json_string_value toolName || true)"
case "$EVENT_TYPE" in
  UserPromptSubmit|Stop|StopCancelled|StopFailure|PermissionRequest)
    record_last_hook_event "$EVENT_TYPE" "$COPILOT_TOOL_NAME"
    ;;
esac
...
if [ -n "$SUPERCLI_SESSION_ID" ]; then
  ...
  # fan-out covers a second Supercli instance (e.g. a dev build next to the
...
printf '%s copilot-hook session=%s port=%s event=%s tool=%s post=%s\n' \
  "$(date '+%Y-%m-%d %H:%M:%S')" \
  "${SUPERCLI_SESSION_ID:-}" \
  "${SUPERCLI_APP_PORT:-}" \
  "${EVENT_TYPE:-}" \
