...
`SUPERCLI_HOME` and Host identity** with fully separate Sessions, projects,
...
(`AppFeature.workspaces`, env `SUPERCLI_DEV_WORKSPACES=1`; legacy
`SUPERCLI_DEV_PROFILES=1` is also accepted); managed in Settings ▸ Workspaces
(`WorkspacesSettingsPanel.swift`). The feature's shipped UserDefaults key
remains `supercli.experimental.profiles`.
...
scope: a real `supercli-attach` surface on that workspace's own
`<home>/app-sessions/<id>/session.sock` (`LaunchConfig.attachCommand(sessionID:
sessionsDir:)`, launched with the workspace's `SUPERCLI_HOME`), with on-disk
...
`SupercliWorkspaceLauncher`.

Core pieces (`SupercliWorkspaceRegistry.swift`):

- **Legacy storage contract**: the registry remains at the **real**
  `~/.supercli/profiles.json`, its array key remains `profiles`, and workspace
  homes remain under `~/.supercli/profiles/<slug>`. These shipped names are
  compatibility identifiers, not current product terminology. Never resolve
  the registry through `LaunchConfig.supercliDir`: every instance must see one
  ...
  the installed markers) lives in the **machine home** — the real `~/.supercli`
...
- **Launch** (`SupercliWorkspaceLauncher.launch`): direct `Process` exec of
  `Bundle.main.executableURL` with `SUPERCLI_HOME` in the env — **never
...
- **Relay entitlements**: `relay_bindings` (the website D1, migration
  ...
  seat.
- Hook isolation: sessions get `SUPERCLI_APP_PORT_REGISTRY_FILE` and
  `SUPERCLI_HOOK_TRACE_FILE` injected (`integrations/mod.rs`) so a workspace's
  hook broadcasts/traces stay in its own home instead of the real `~/.supercli`.
...
`SUPERCLI_HOME`. Scope changes clear transient drop/focus/reveal state before
...
`SupercliWorkspaceLauncher`, move one scope and its layout to one owning window,
...
### Workspaces from the CLI (`supercli --workspace`, `supercli workspaces`)

The CLI is a peer control surface over the same registry and homes
(Rust side: `crates/supercli-cli/src/workspaces.rs`, added 2026-08-13):

- **`supercli --workspace NAME [command]`** — resolves NAME (workspace name,
  case-insensitive, or slug = home dir basename) against the real
  `~/.supercli/profiles.json`, then sets `SUPERCLI_HOME` in-process **before any
  dispatch** — so it works for `supercli serve` and every headless verb
  (`supercli --workspace work ls`). Spawned hosts inherit the env, which is what
  keeps sessions, state, hook broadcasts (`SUPERCLI_APP_PORT_REGISTRY_FILE` /
  `SUPERCLI_HOOK_TRACE_FILE`), and pairing identity inside the workspace home.
  ...
  erroring.
- **`supercli workspaces [list | add <name> | remove <name>]`** — manages the
  shared registry. `add` mirrors the app's create exactly: unique slug
  (`slugify` parity with `SupercliWorkspaceRegistry.swift`), permanent home
  minted under `~/.supercli/profiles/<slug>`, atomic `0600` write, identical JSON
  ...
  supports `--json`.
- **`supercli --workspace NAME settings ...` and `presets ...`** — mutate that
...
- The registry path is always the **real** `~/.supercli/profiles.json` —
  `workspaces.rs` must never resolve it through `app_paths::supercli_home()`,
  which honors `SUPERCLI_HOME` (that indirection would fork the registry the
...
- No feature gate on the CLI: the flag is pure env plumbing over the
  already-ungated `SUPERCLI_HOME` mechanism (the app's
  `AppFeature.workspaces` gate is UI visibility, not a capability
  boundary). `supercli serve` also has no per-home single-instance guard —
