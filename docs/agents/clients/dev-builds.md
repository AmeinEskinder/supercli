...
(exposed as `bun run dev:native`). It builds + signs `dist/Supercli.app` and
launches it.
...
> `/Applications/Supercli.app`. No `rm -rf`, no `cp`, no `ditto`, not even
...
> `dist/Supercli.app` (needing to be the only instance is covered below). This
...
> require the installed `/Applications/Supercli.app` to be running. Do not launch
...
Dev builds live at `clients/native/dist/Supercli.app` (`bun run dev:native` builds,
signs, quits any already-running **Supercli Dev** from that dist bundle, then
launches it with `open -n` so an installed app with the shared bundle id
cannot steal the launch). The installed `/Applications/Supercli.app` is never
...
builds land in the *same* `dist/Supercli.app` path: after any `bun run release:mac`
(including `--dry-run`), `dist` holds a release-flavored bundle — plain
"Supercli" name, dark icon, **Sparkle feed baked in** (it can self-update out
...
`bun run dev:native` and confirm the menu bar reads **Supercli Dev**. What to
...
- **Phone-facing changes (`/mobile/*` routes, pairing, remote server):** the
  ...
  rediscovers it via Bonjour/persisted port (same `~/.supercli` state, same
  macID — no re-pairing). Afterward, relaunch `/Applications/Supercli.app` only
  if it was running before the test.
- **Clean-state testing:** `bun run dev:native:blank` (isolated `SUPERCLI_HOME`,
  own UserDefaults suite) runs independently of the installed app.

Confirm which binary is serving with
`pgrep -fl "Supercli.app/Contents/MacOS/SupercliNative"` (the path shows dist vs
/Applications), and that a new route made it into a build with e.g.
`strings clients/native/dist/Supercli.app/Contents/MacOS/SupercliNative | grep <route>`.

### iOS app: build & deploy

The iOS client (`clients/ios/SupercliIOS`) is an xcodegen project — after adding or
renaming Swift files, run `xcodegen` (in `clients/ios/SupercliIOS`) so the `.xcodeproj`
...
> `SUPERCLI_DEVELOPMENT_TEAM` (`DEVELOPMENT_TEAM` follows it) — don't pass a
> team id on the CLI; override per-build with `SUPERCLI_DEVELOPMENT_TEAM=…` if
...
cd clients/ios/SupercliIOS
xcodebuild -project SupercliIOS.xcodeproj -scheme SupercliIOSApp \
...
xcodebuild -project SupercliIOS.xcodeproj -scheme SupercliIOSApp \
  ...
  -derivedDataPath /tmp/supercli-ios-device -allowProvisioningUpdates \
...
- **Cross-team upgrade rejection:** a phone that has Supercli signed by a
...
- Keep the device derivedDataPath separate (`/tmp/supercli-ios-device`) so
...
- **"No profiles for … were found" + "No Accounts" (headless can't regenerate
  ...
  ```sh
  SRC=/tmp/supercli-ios-device/Build/Products/Debug-iphoneos/Supercli.app/embedded.mobileprovision
...
`SupercliIOSRootView`), the way the sidebar/preset drawers and the bell sheet do.

### Blank / first-run dev instance

To exercise first-run behavior (or any clean-state behavior) without touching
your real `~/.supercli`, use `clients/native/dev-blank.sh` (exposed as
`bun run dev:native:blank`). It does the same stable-signed build, then launches
the `SupercliNative` executable directly (not `open`, so the env is inherited)
with `SUPERCLI_HOME` pointed at a throwaway state dir — so the app boots as if
...
`SUPERCLI_HOME` overrides the Supercli state dir in **both** the app
(`LaunchConfig.supercliDir`) and the spawned host (`app_paths::supercli_home`); the
...
intentionally avoids the `SUPERCLI_TEST_*`/`SUPERCLI_SNAPSHOT*` prefixes, which arm
the snapshot harnesses. Default is a fresh `mktemp` dir per run; pass
`SUPERCLI_HOME=~/supercli-dev` to reuse a fixed dir, plus `RESET=1` to wipe it
first.

Crucially, `SUPERCLI_HOME` also isolates **UserDefaults**. The native app keeps
...
`AppDefaults.shared` (`LaunchConfig.swift`), which returns a per-`SUPERCLI_HOME`
