...
login shell exactly as typed; the Host exports only Supercli's generic session
environment (`SUPERCLI_SESSION_ID`, `SUPERCLI_SESSION_DIR`, `SUPERCLI_APP_PORT`,
`SUPERCLI_HOST_BIN`, the port registry and trace paths, the Apps bin on `PATH`,
...
`supercli integrations install <runtime>` or the `integrations.install` Host
...
and only for configuration Supercli already edited. Every integration registers the same stable shim,
`~/.supercli/bin/supercli-mcp`, as the MCP server: it execs
`${SUPERCLI_HOST_BIN:-<install-time supercli-host>} __mcp_gate__ unified`, and
...
Provider processes hosted by Supercli never inherit an outer Herdr pane
...
child-env hygiene (the Herdr supervisor integration this once protected
 against a race with the interactive terminal UI's aggregate `custom:supercli`
...
- Emits lifecycle and permission events through the Supercli hook server
...
- Registers the MCP shim as the user-scope server `supercli` in
  `~/.claude.json` (the file `claude mcp add --scope user` writes), pruning
  the Supercli-owned `supercli-mcp`/`supercli-sessions`/`supercli-browser` names
...
- **Primary lifecycle source is native Codex hooks**, not just notify. The
  ...
  Supercli-managed `SessionStart`, `UserPromptSubmit`, and `PermissionRequest`
...
- Codex hook commands guard missing scripts and are reconciled whenever hook
  assets install: live hooks for side-by-side `SUPERCLI_HOME` instances are
  preserved, while obsolete Supercli entries are pruned. This prevents a deleted
...
- `~/.codex/config.toml` gets the top-level `notify = ["bash", <normalizer>]`
  reporter (set only when absent or already Supercli-owned) as the
  turn-completion (Stop/idle) source and a compatibility bridge for Codex
  builds without the `hooks` feature, plus `[mcp_servers.supercli]` pointing
...
- The Codex package's notify normalizer maps raw event `type`s onto Supercli states before calling the provider-neutral transport: `agent-turn-complete`/`task_complete`/`turn_aborted` → Stop, `task_started`/`exec_command_begin` → Start, `request_permissions`/`exec_approval_request`/`apply_patch_approval_request`/`approval-requested` → PermissionRequest.
...
- The shared notify reporter is global; Amp itself reads plugins per project
  ...
  plugin is written on request: `supercli integrations install amp --project
...
- Installs the notify plugin into OpenCode's own global plugin directory
  (`${XDG_CONFIG_HOME:-~/.config}/opencode/plugin/supercli-notify.js`); it
  no-ops outside an Supercli session
...
- Installs the shared hook script globally; Copilot reads hooks per
  repository, so `.github/hooks/supercli-notify.json` is written on request:
  `supercli integrations install github-copilot --project DIR`
...
- Installs the generic event-posting script as `~/.supercli/hooks/grok-hook.sh`
- Writes Grok-native hooks to `~/.grok/hooks/supercli.json` (global hooks are
  always trusted, so no project-trust step is needed)
- Maps Grok lifecycle events onto Supercli state: `SessionStart` → HookSeen
...
- Grok also natively scans `~/.cursor/hooks.json` and `~/.claude/settings.json`
  for compatibility. Those Supercli hooks are Claude/Cursor-shaped:
  ...
  Claude/Cursor Supercli hook scripts therefore no-op when `GROK_SESSION_ID`
  is set, and the hook server treats `session_start` as HookSeen. Native
  `supercli.json` is the lifecycle source. Grok runs exactly as the user
  types it: Supercli no longer overlays `GROK_HOME` or toggles its
...
- Installs `~/.supercli/hooks/kimi-hook.sh` and reconciles Supercli-managed
...
- Forwards Kimi's provider-created `session_id` and exact current
  `wire.jsonl` or legacy `context.jsonl` path to Supercli
...
- Current Kimi Code receives the MCP shim as a persistent `supercli` entry in
  `~/.kimi-code/mcp.json` (Supercli-owned legacy entries are pruned, user
...
- Installs managed native global event files under `~/.cline/hooks` plus
  `~/.supercli/hooks/cline-hook.sh`; the hooks no-op unless
  `SUPERCLI_SESSION_ID` is present and coexist with other supported filename slots
...
- Reads semantic `<id>.messages.json` transcripts (messages, reasoning, tools,
  model, and usage)
- Merges the MCP shim as `supercli` into Cline's own user MCP settings
...
- Nothing to install: Pi reports no lifecycle and registers no MCP, so
  `supercli integrations install pi` is refused. `pi` runs exactly as typed;
  Resume uses Pi's own `--continue`. Older launches that recorded a managed
  `--session-dir` beneath the Supercli home keep it across resume and cleanup.

Antigravity (Google's `agy`, antigravity.google/product/antigravity-cli;
community request orgs/supercli-com discussions #13):
...
- MCP: the integration merges the `supercli` stdio entry (the shim, no `env`)
...
- MCP (2026-08-21): fx loads MCP servers only from the persistent global
  ...
  the integration merges a managed `supercli` entry pointing at the MCP shim.
  ...
  otherwise, and that inheritance is what carries `SUPERCLI_SESSION_ID` into
  each session's gate process. Outside a granted hosted Session — including
  fx runs outside Supercli — the gate serves a valid endpoint with no tools.
...
- Hooks run only as **native-plugin capabilities** (no settings-file hook
  ...
  `muse plugins install` + `muse plugins approve supercli` (idempotent; skipped
  via a content-digest marker while muse's `plugins/installed.json` lockfile
  still lists the plugin). Supported events cover exactly Supercli's lifecycle:
...
- Plugins are experimental in muse and load only with
  `MUSE_EXPERIMENTAL_PLUGINS=1` in muse's environment. Supercli no longer
...
- Hook payloads are Claude-compatible stdin JSON (`hook_event_name`,
  ...
  recovers the `SUPERCLI_*` identity from its parent muse process via
...
- The plugin manifest registers the MCP shim as its `supercli` server. Muse
...
new Supercli build. Downloadable third-party adapters are planned separately and
...
3. Put scripts and plugins in `assets/hooks/`. The installer is the whole
   ...
   `supercli_runtime_generation` in both the event and durable seed.
