...
- [x] S2: Before/after concurrency tables (1/4/8/16, n≥2000 each) show
      throughput increasing through 16 and p99 <500ms at concurrency 8.
      VERIFIED 2026-09-25: Grouped 283.7→446.9→711.2→792.8/s; p99 at c8=39.3ms.
      Direct (before) collapses with 306/321 errors at c8/c16 (unsafe).
- [x] S2: Grant store uses group-commit writer (single thread, batch fsync);
      crash safety preserved (audit fsync before grants.json temp+fsync+rename+dir fsync).
      VERIFIED 2026-09-25: 12/12 grant tests pass.
- [x] S2: Grant audit hash chain (`grant-audit.jsonl`) with `grant_created`
      entries (actor, scope, tool); startup reconciliation verifies `grants ⊆ chain`;
      unaudited grants quarantined. VERIFIED 2026-09-25.
- [x] S2: `unpeel migrate` moves grants from app-state.json to grants.json
...
- [x] S4: 50x kill -9/restart chaos at concurrency 8 passes (no lost grants,
      no stuck locks, chains verify). VERIFIED 2026-09-25: 50/50 passed.
- [ ] S5: cargo-fuzz 30min each on pairing/sealed-envelope and config parsers;
      executions/crashes/fixes reported.
- [x] S6: STRIDE threat model refreshed for Phases 9-13. VERIFIED 2026-09-25:
      Updated for v2 (grant audit chain, group-commit writer, long-poll, quarantine).
- [ ] S7: 2-hour release soak at concurrency 4 passes.
- [x] Doctor --bundle excludes review payloads and command text (planted
      assertion passes). VERIFIED 2026-09-25: `collect_logs` skips
      action-reviews.jsonl and grant-audit.jsonl; test with planted command
      text passes.
