...
| 134 | [APPS] | Git App: Changes tab with status glyphs and unified diffs | partial | `crates/apps/diffs/src/app.rs`, `ui.rs`, `git.rs`; gpuidart: `clients/supercli-app/lib/screens/gitpaneview.dart`, `lib/widgets/git_widgets.dart` (stub `StubGitDataSource` by default; Host routes now live in `crates/supercli-core/src/host_git.rs`, `HostClient.gitStatus/gitDiff/...`, `GitPaneController` binds callbacks — shell mount pending, GAP-A1) | `test/apps_panes_test.dart` (21 tests); `docs/internal/proofs/proof-screenshots/parity-apps-git.png` (snapshot-raster proof, not native window) |
...
| 137 | [APPS] | Git App: Fetch/Pull/Push control (fast-forward only, no force) | partial | `crates/apps/diffs/src/git.rs` (`RemoteAction::Fetch`, pull, `push --no-force --no-mirror`); gpuidart: `clients/supercli-app/lib/screens/gitpaneview.dart` (Fetch/Pull/Push buttons + `git.fetch/pull/push` `UiAction`s and callbacks; Host routes `POST /mobile/git/{fetch,pull,push}` live (`host_git.rs`, 10 Rust tests); `GitPaneController` binds callbacks — shell mount pending, GAP-A1) | `test/apps_panes_test.dart` (remote actions exposed) |
...
| 148 | [WEB] | Hosted installer endpoints (`superc.li/install.sh`, per-App `install/<app>/install.sh`) with channel selection (alpha/beta/stable) | done | `scripts/install.sh`, `scripts/install-app.sh` | — |
| 149 | [WEB] | Help/docs/download links (superc.li/docs, /download/mac, /ios) referenced from the apps | partial (component) | `lib/web/help_links.dart` (`HelpLinks`, `HelpLinksView`), `docs/web-links.md` | `test/help_links_test.dart` (6); link surface contract, not live-verified |
| 150 | [DESKTOP] | App shell: resizable vibrancy sidebar, custom titlebar, ⌘B toggle | partial (component) | `lib/app.dart` mounts SidebarView + TerminalArea(PaneLayout) + McpApprovalPanel + ToastCenter in `bin/main.dart`; HostClient bootstrap feeds sidebar/approvals; toasts from events | `test/app_shell_test.dart` (6 tests), `test/app_test.dart`; proof: `docs/internal/proofs/proof-screenshots/parity-appshell.html` (real UiNode tree; native window BLOCKED on libxkbcommon-x11.so.0) |
