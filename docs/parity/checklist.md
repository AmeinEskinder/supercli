...
| 149 | [WEB] | Help/docs/download links (unpeel.com/docs, /download/mac, /ios) referenced from the apps | partial | `clients/supercli-app/lib/web/help_links.dart` (`HelpLinks` URL contract + `HelpLinksView` embeddable UiNode tree), `docs/web-links.md` (URL contract; domain now supercli.com) | `test/help_links_test.dart` (6 tests); hosted docs/download pages are contract (`expected`), live check not possible from sandbox |
